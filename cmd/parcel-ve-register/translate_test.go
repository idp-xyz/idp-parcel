...
// 不是本入口形状的输入在翻译处就拒（未知字段、集合外取值、矛盾声明、构造门）。
...
func TestMilestoneMappingTranslatesEveryField(t *testing.T) {
	...
	command, err := milestoneMappingFromJSON(raw)
	if err != nil {
		t.Fatalf("翻译：%v", err)
	}
...
func TestTriageRulesTranslateWithClosedOutcome(t *testing.T) {
	...
	command, err := triageRulesFromJSON(raw)
	if err != nil {
		t.Fatalf("翻译：%v", err)
	}
...
func TestNotificationPolicyTranslatesDuration(t *testing.T) {
	...
	command, err := notificationPolicyFromJSON(raw)
	if err != nil {
		t.Fatalf("翻译：%v", err)
	}
...
func TestClaimAuthorizationDistinguishesEmptyListFromMissingField(t *testing.T) {
	...
	command, err := claimAuthorizationFromJSON(explicit)
	if err != nil {
	...
	if _, err := claimAuthorizationFromJSON(missing); err == nil {
...
func TestDisclosurePolicyTranslatesAllThreeDimensionStates(t *testing.T) {
	...
	command, err := disclosurePolicyFromJSON(raw)
	if err != nil {
		t.Fatalf("翻译：%v", err)
	}
...
func TestTranslationRejectsWhatIsNotThisEntranceShape(t *testing.T) {
	cases := []struct {
	...
	}{
		{
			...
			translate: func(raw []byte) error { _, err := milestoneMappingFromJSON(raw); return err },
		...
		{
			name:      "分诊走向集合外取值",
			translate: func(raw []byte) error { _, err := triageRulesFromJSON(raw); return err },
			raw: `{"tenantId":"SYN-TEN","version":"v1","approvedBy":"a","effectiveFrom":"2026-09-01T00:00:00Z",
		...
		{
			name:      "时限不是时长字面",
			translate: func(raw []byte) error { _, err := notificationPolicyFromJSON(raw); return err },
		...
		{
			name:      "展示维缺内容来处",
			translate: func(raw []byte) error { _, err := disclosurePolicyFromJSON(raw); return err },
			raw: `{"tenantId":"SYN-TEN","version":"v1","approvedBy":"a","effectiveFrom":"2026-09-01T00:00:00Z",
		...
		{
			name:      "租户构造门缺件",
			translate: func(raw []byte) error { _, err := claimEligibilityFromJSON(raw); return err },

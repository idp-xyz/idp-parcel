...
func run(ctx context.Context, args []string, getenv func(string) string, out, errOut io.Writer) int {
	if len(args) < 1 {
		fmt.Fprintln(errOut, "用法：parcel-commercial <publish|register-resolution-key|register-parties|deactivate-party-identity|register-products|register-registration-number-types|register-legal-entity-profiles> -input <file>")
	...
	case "register-legal-entity-profiles":
		return runRegisterLegalEntityProfiles(ctx, args[1:], getenv, out, errOut)
	default:
		fmt.Fprintf(errOut, "未知子命令 %q；可用：publish、register-resolution-key、register-parties、deactivate-party-identity、register-products、register-registration-number-types、register-legal-entity-profiles\n", args[0])
		return exitTechnical
	}
}

...
//
// 写那一半的组合根在 assemble_label_channel.go（票 label-channel/28），`main` 启动时已装配（票 label-channel/34）：
// 择优编排带留痕三件全装，决定记录在择优那一段的事务里写进同一只登记册。但今天没有触发面调那条链（乙路无运营端点，
// 谁发起一笔面单交易是产品题、触发面另票），且三取数口显式未配置——

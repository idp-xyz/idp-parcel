...
// VE 六类配置登记的生产装配（ADR-0085，票 admin-write-faces/02 切片 02d）。六类共用
// 一个登记用例对象（application.CatalogRegistration），传输层却按类各要一个 Registrar，
// 且六个 Registrar 的方法同名 `Handle` 而命令类型互不相同——一个类型实现不了六个，
// 六个包装因此不是重复而是必需。
//
// 事务边界与 transactionalPriceCardRegistration 同源：一次调用一笔事务，用例交回业务
// 答案时提交，返回错误时整笔回滚，端点按 ADR-0022 答「没形成答案」。

// veCatalogInTransaction 是六格共用的事务壳，六个 Handle 只差交给用例的那一句。
...
// veRegistrationOrchestration 收拢六格，判据同 customsRegistrationOrchestration。
...
// buildVERegistrationOrchestration 装配六个 `/visibility-catalogue-*-registrations`

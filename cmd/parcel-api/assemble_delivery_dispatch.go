...
//
// 派送要求三条缝**只接了地点这一条**：`Places` 是 adapters/parcelshipment 的真适配器，窄口交 PS 自己的委托仓储
// （它兼实现 PS 的收件地点引用读口，读的是委托快照本尊，同一只库）。`Windows`（network-routing，票 13）与
// `Conditions`（party-commercial，票 14）**刻意留 nil**——ADR-0114 决定三：缝没接时执行器答`未决`并点名那条缝
// （DELIVERY_WINDOW_SOURCE_NOT_WIRED），不造替身、不填默认；票 13 接上那天把那一格换成真适配器即可，本函数其余

...
## 刷新记录 as-of `1665fdb`（2026-09-01，MCP-3，批务票 admin-remainder-mechanism-batch/05 第 1 项）

本表自注「消费本表前若 HEAD 已前进，上述两类结论应按新 HEAD 再取一次证」。本次即那一次。
**只刷装配面与总览计数，主表判据栏一律未动**（判据引的是 CONTEXT-MAP / `CONTEXT.md` / `UC-*`，
不随代码走）；46 行 × 4 栏的逐行重取证仍不在范围内。取数口径与命令见
[T2 量尺重核 `1665fdb`](../admin-remainder-mechanism-batch/t2-remeasure-1665fdb.md)。

### 口径复现声明

下面每个量都先在本表既有的锚点上回跑同一条命令，能对上才往前推：适配器 46、信封类型 55
在 `4d57ecd` 上逐位复现；路由表条目 2 在 `60ea63c`、12 在 `3b9f212` 上复现；附带 a 的
「被组合根实际构造 2 个」在 `60ea63c` 上复现。**所以下表的 delta 是树在动，不是量法在动。**

### 底数

| 量 | `4d57ecd` | `60ea63c` | `3b9f212` | `1665fdb` |
|---|---|---|---|---|
| Outbox 发布适配器 | 46 | 46 | 46 | **47** |
| 信封类型 | 55 | 55 | 55 | **59** |
| 发布上下文 | 9 | — | — | 9（未变） |
| 路由表条目＝有消费者的类型 | — | 2 | 12 | **13** |
| `adapters/inbox` 消费者适配器文件（非测试） | 1 | 2 | 16 | **17** |

适配器口径同本表首节：`internal/*/adapters/postgres/*handoff*.go` 非测试文件，扣除
`network-routing` 的重放指纹册（它不发布信封）。信封类型取这些文件里的 `*EventType` 常量。

### 主表未覆盖的四类新信封

`4d57ecd` 以来新增 4 类，**本表主表 46 行里没有它们的行**——引用「55 类」时要知道今天是 59：

| 新信封类型 | 发布方 | 现状 |
|---|---|---|
| `parcel-shipment.shipment-request.submitted` | PS `shipment_request_submitted_handoff.go` | **已有消费者**——路由表第 13 条，投 PS 自家接受判断链 |
| `customs-compliance.follow-up.replacement-proposed` | CC 跟进交接口 | 未登记路由 |
| `customs-compliance.follow-up.replacement-effective` | CC 跟进交接口 | 未登记路由 |
| `settlement-accounting.settlement-application.reversed` | SA 核销交接口 | 未登记路由 |

### 路由表：只增不减，第 13 条是新类型

`1665fdb` 的路由键集合是 `3b9f212` 那 12 个的**严格超集**，逐键对差只多出
`psinbox.ShipmentRequestSubmittedEventType`，无一条被摘除。所以上一次刷新记录里
5/7/8/16/17/19/20/21/22/37 十行的「已有消费者」结论**按新 HEAD 仍然成立**，不必逐行重读。

第 13 条落地时点在 `7d68475` 之前（该 SHA 上路由表已是 13 条）。

> **`wireDispatcher` 的注释在 `1665fdb` 上比它自己的路由表少数一类**：注释开头写「路由表今天
> 有十二类事件」并逐条枚举十二条，而同一函数里的 `NewDirectPublisher` 映射有 13 个键。
> 这条只报不改——`cmd/parcel-dispatch/**` 归 MCP-1，且其工作树在途版本已把该句改成十三类并
> 补齐枚举，即改在自愈途中，不是待办缺口。

### 两端关系（本表末段那句的新数）

本表原句：「55 类事件里 53 类无消费者，58 个处理器里 56 个无生产构造（`60ea63c`）」。
按同口径重取于 `1665fdb`：**59 类事件里 46 类无消费者，71 个应用层处理器里 35 个无 `cmd/`
构造**（`internal/*/application/` 的 `New*Handler` 共 71 个，其中 36 个被 `cmd/` 非测试代码
构造；该 36 沿 `60ea63c` 2 → `3b9f212` 6 → `7d68475` 33 → `1665fdb` 36 一路长上来）。

「附带 a」的三档细分（组合根构造 / 跨上下文适配器按具体类型引用 / 零非测试引用，及零那一档
里再分的「有 http 端点等 Intake」六个）**本轮仍未重做**——那三档问的是「引用」不是「调用」，
与上面这个 36 不是同一个量，不能相减。

> 有一格是量出来的，值得记：**棘轮二族的「零非测试调用点」35 个，与这里的「无 `cmd/` 构造」
> 35 个，在 `1665fdb` 上是逐字相同的同一个集合。** 两者问的不是同一件事（前者扣的是全仓非
> 测试调用点，后者只看 `cmd/`），今天答案相同只说明**没有任何应用层处理器是「只被 `internal/`
> 非测试代码构造、进不了任何进程」的**。这是实测，不是等价——树一动就可能分开。

### 矛盾清单第 3 条：一半失效，一半仍成立

原文记「CONTEXT-MAP 有 SA→CR 与 TF→CR 两条边，但 `internal/` 下无 collection-remittance
实现，也没有任何指向它的 handoff——是『有边无发布面』」。

- **「无实现」那半已失效**：`1665fdb` 上 `internal/collectionremittance/` 已在位（32 个文件，
  含 domain / application / adapters / ports 四层，另有进程入口 `cmd/parcel-collection-register`）。
- **「无发布面」那半仍成立**：该上下文下 `*handoff*` 文件为 0，其 `ports.go` 对 `Handoff`
  零命中，也不在上面那 9 个发布上下文之列。

所以这一条今天该读作「**有边、有实现、仍无发布面**」，与原记的「有边无发布面」不是同一个
形状：缺的从「整个上下文」收窄到「发布口这一层」。矛盾清单其余四条本轮未重取证。

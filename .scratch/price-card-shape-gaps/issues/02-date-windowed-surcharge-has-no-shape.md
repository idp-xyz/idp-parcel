...
- 2026-09-04 · MCP-1：**进 main 记录。** 本票两笔分支→main：`4c5d8938`→`615adf18`（PUBLISHED_AMOUNT 序列 + 取当期序列定额）· `d7f33d9b`→`cef6adc2`（迁移 0006、登记面 currency、管理台表单）；四票公共笔与验证同票 01 的进 main 记录，远端 main = `732c1db3`。MCP-3 完工报点名的 PP CONTEXT「计算方法」词条（仍写四种）已随本笔按 ADR-0110 Decision 二补「取当期序列定额」一格。

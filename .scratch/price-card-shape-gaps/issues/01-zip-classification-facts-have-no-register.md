...
- 2026-09-04 · MCP-1：**进 main 记录。** 分支 `mcp3-pp-shapegaps`（基 `ae7b4c8a`）十二笔在 `6f9436f3` 上 cherry-pick 零冲突，本票各笔分支→main：`1d8b6f71`→`c31c729f`（目录绑定）· `0fec7f2a`→`a20cacae`（评价解分区/档位）· `ea030966`→`e5598482`（目录登记聚合/复核/在用）· `235a58d8`→`440e13fb`（登记册/复核册/写面）· `b0009c1c`→`a1d89693`（编排补读数）；四票公共笔 `168a134d`→`67921a58`（in-progress）、`ae672499`→`7404f40d`（`ReadingKey`）、`f4577408`→`07361d8e`（resolved）；清点笔 `9a7a3688` 未重放，tip 重生成为 `732c1db3`。验证（隔离树钉 `732c1db3`）：gofmt 空、build/vet 0、含 DSN 全仓 99 ok / 0 FAIL（8m15s）、探针 `TestReferenceCatalogueRegisterAndResolveRoundTrip` 带 DSN PASS / 不带 SKIP、admin-web tsc 0 + 67/67。远端 main = `732c1db3`。

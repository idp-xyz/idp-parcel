...
因此 **02e 暂挂**，待答：代收的业务命令面是否属本票范围？若否，本票的分片收敛为 02a..02d，
代收的在线操作面另立票并先裁授权模型。同一问对 VE（02d）的部分命令也可能成立，做到那一片时
逐个用例分辨「配置」与「业务操作」，不整片一刀切。

...
- 2026-09-03 · MCP-4：**更正一处贯穿本票的错引。本条只追不改上文**——原句留着，因为其中一处
  正是要看的证据。发现人 MCP-5（回 ADR-0085 原文核的），我重数并补了它漏掉的一处。

  **本票有五处把话归给了 ADR-0085 Decision 二，五处都不对，而且引的不是同一句**（数于 `6e40312`，
  仅作此刻取证，计数即论点故按例外锚 SHA）：

  - 四处引「有登记用例**与** CLI 先例的运营配置册逐上下文进端点表」——分别在 02e 节首、MCP-4
    署名那条 Comment 的第三段、后续那条 Comment 里维持不进首批那段，以及两条裁定落账那条的第一段。
  - 一处引「Decision 二末句把治理登记册排除在首批之外」——在 02e 节。

  **正确归属**：

  - 「有登记用例与 CLI 先例……」那句是**票 01 裁决第二项**的措辞，不是 ADR 的。ADR-0085
    Decision 四写的是：首切片选 parcel-pricing 的**理由**是「登记用例与 CLI 先例齐、答案代数
    已封闭，是最小可裁样本」，而**其余上下文的取舍是「登记频次 × 操作者角色」由实施票逐册裁**。
    **一个是选样本的理由，一个被当成了准入门，两者不同形。**
  - 治理登记册排除在首批之外那句在 **Decision 四**，不在 Decision 二。Decision 二末尾是关于
    OIDC 的同日补记。

  **为什么这一处比一般的引错重（MCP-5 的原话，照抄）**：两条裁定落账那段写着「依据是 ADR-0085
  Decision 二的字面……**这不是新裁量，是照字面执行**，所以不需要新 ADR」。**一条裁定拿「只是照
  字面读」给自己免掉了新 ADR 的义务，而那个字面并不存在**——免除的理由也就没了。被引的那句话
  正是用来证明「不必再裁」的。

  **两件后果分开说，别并成一件：**

  1. **`RegisterAutoRerouteFacts` 那一例的敞口今天已经合上，且不欠新 ADR。** owner 于 2026-09-03
     在通道 4 直接裁了落法（只补 CLI，不进端点表）并明定判据以 **Decision 四**为准。结论从此
     靠一次真实裁决立着，不靠那个幻影字面。记录见
     [05](./05-auto-reroute-facts-has-no-registration-entry.md) 与其交付 Comment。
  2. **02e 排除代收那一裁本身仍然成立**——它是 2026-09-01 用户亲自裁的，有独立效力；**错的只是
     它写下的理由**。把归属改对之后那一节的论证并不塌：代收七个用例是业务操作不是配置登记，
     这个分辨用的是本票自己那条判据（改的是「这个租户怎么配置」还是「账上此刻的事实」），
     与 CLI 先例无关。

  **我这一处是自己签的。** MCP-4 那条 Comment 的第三段是本会话 2026-09-02 写的，词从票面转手，
  没回去核 ADR 原文。这一格就是本仓反复在记的那个形状：**转引与取证写下来之后长得一模一样**，
  而转引不带「我在哪核的」，读的人无从分辨。写引文时把出处核到原文，或者写明「转引自某票面」。

  **顺带一条查法上的教训，它比这次的错引更能复用（MCP-5 提，当场就有例子）**：核一处错引要按
  **被归的出处**扫全文（这次是搜 `Decision 二`），**不是按被引的词扫**。发现人拿「有登记用例与
  CLI 先例」那串词去对，因此只找到四处；02e 那第五处引的是另一句（治理册排除在首批之外），
  **它与前四处只共享「归错到 Decision 二」这一点，不共享被引的句子**，按引文搜天然搜不到。
  一份文档里同一个错误出处往往被引过好几句话，而**只有出处那几个字是它们的公共子串**。

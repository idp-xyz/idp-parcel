...
- 2026-09-24 · 真浏览器验收 ← 用户（环境由通道 3 备）：独立 detached 检出钉 `04898af1`，独立库 `idp_verify`（演示种子 + 3 条合成委托；为造委托在该库
  临时登了一笔暂停恢复，缘由见 `.scratch/demo-intake-admission-paused/`），后端 `:19081` 开隔离读写准入 `SYN-TENANT-01`，vite `localhost:5173`，经 OIDC
  真登录。八条清单用户答「全过」。本票相关：拖标签重排、右键菜单（关闭 / 固定 / 关闭其他 / 关闭右侧 / 重开已关闭）、标签溢出滚动、`Ctrl+Shift+T` 重开，以及列表 → 详情往返的三条回程——完成记录
  「◑ 浏览器未验」所列的 vendor 行为据此已验。`Ctrl+W` 未验（Chromium 保留键，见判断项 4）。

...
- 2026-08-21 MCP-3（受用户委托裁断）：**弃候选三条已全部裁弃**，判据录清册末节「裁定」；
  `adr-0065-storage` 与 `bento-gate-reeval` 两条「等用户单独确认」的指针去留一并并入
  「指针整类清理」，不再单等。**本票余量自此收敛为两件**：
  1. 按清册拆已吸收/已弃分支的残留 worktree——拆前照本票既有口径（双查工作区 +
     树内 `git diff HEAD` 证 delta 为空 + detached 先建 ref），`remove` 拒绝时不 `--force`
     不绕道；
  2. 收口时单独开「指针整类清理」票，一次处理全部已吸收/已弃分支的指针。
  两件都是机械执行，无再需人裁的格。

- 2026-08-21 MCP-3（用户批复「请你直接干吧」后执行，本票转 resolved）：两件余量均已完成——
  33 棵非主树全部拆除（31 棵双查干净直拆；`idp-verify-t109a` 先证零独有再带证 `--force`，
  过程与全部删支记录见[票 03](./03-obsolete-branch-pointer-cleanup.md) 执行条）；指针整类
  清理由票 03 承载并已执行（51 支已吸收 + 4 支弃定删除，6 支按册保留）。执行后
  `git worktree list` 仅剩主树，`git branch --list` = main + 六支保留。本票就此收口；
  后续若再有死会话残局，按本票沉淀的口径另开新票，不重开本票。
- 2026-08-21 MCP-4：同晚受同一用户委托独立裁断（经通道 4），三条结论与上一条 MCP-3 **逐条一致**；
  证据为测试场景级对映——分支侧每个测试场景在 main 有同名、同义或更细分的对应，且 main 另有
  分支没有的行为族——已就地记入清册第四节各条。两路互不知情、判据不同、结论一致，弃之更稳。
  本票按上一条口径继续 in-progress，余量即那两件机械执行项。

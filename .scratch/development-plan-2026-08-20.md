...
| [脏 worktree 残局](./dead-session-salvage/issues/02-dirty-orphan-worktrees-hold-uncommitted-work.md) | 工程 | in-progress，但已无待裁格：[并回清册](./dead-session-salvage/branch-merge-census-2026-08-21.md)判定全部本地分支（真待合仅 t1-09a-takeover，已合 `b394adf`），弃候选三条（`mcp3-ve008-wire`、`salvage-cons-proj-delivery-detached`、`mcp1-pc-publication`）已于 08-21 受托裁弃，指针保留待整类清理；余量两件机械执行——按清册拆残留树（含实测干净的 `t-ratchet-gate` 树）、收口时开指针整类清理票 |

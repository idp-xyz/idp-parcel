...
- 2026-09-24 · 通道 4（2026-09-25 通道 2 复核改定）：本票逐口归类表里「未归」「同族未列」「族归属待定」三行，与未入那张表的 `/commercial-publication-vocabularies`，由 [ADR-0151](../../../docs/adr/0151-unassigned-command-faces-get-their-families.md) 定族——委托侧五个运营决定口与 TF 装载分配、终止参与归 [15](./15-operation-decision-faces-take-operator-intake.md) 的「运营决定」能力面；本票放行过的关段与建派送任务两口，生产渠道也改归 15（ADR-0151 决定六部分停用 ADR-0149 决定一对这两口的归类），隔离放行照 ADR-0150 留到 15 换口那一笔；更正口随原事实归 [10](./10-device-register-and-operation-fact-capability-face.md) / [11](./11-integration-client-register-and-client-credentials.md)，词表读口归 [06](./06-commercial-publication-and-replay-take-operator-identity.md)。归类表正文不改。

...
- **19 重走**（2026-09-24，通道 4，[票 19](./19-delivery-attempt-result-has-no-production-writer.md)；分支 `mcp4-psb19`，代码钉 `30e2c929`，真库用例 `TestARecordedDeliveryAttemptLetsTheEffectiveDeliveryThrough`，建任务、登记尝试、交付三口都走生产装配与进程自己的隔离 Intake；只记 `S`）：派送尝试有了写入方 `POST /transport-fulfillment/delivery-attempts`。尝试登记之前交付照旧 `200 SOURCE_NOT_ACCEPTED`；建派送任务 `201 DISPATCH_TASK_OPENED`、登记一次妥投 `201 ATTEMPT_RECORDED` 之后，同一份交付 `201 DELIVERY_REGISTERED`。「派送尝试无写入方」这一停点不再成立；生产形态（开关不设）该口照常 `403`，等操作者渠道的作业事实能力面（operator-channel/10）。本次没有按 `seed.sh` 端到端实跑动线。

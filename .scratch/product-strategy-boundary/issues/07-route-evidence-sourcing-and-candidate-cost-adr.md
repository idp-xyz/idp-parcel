# 07 路由证据由谁供哪一族事实、路由候选成本怎么来——先裁后做

Category: enhancement
Status: draft
Blocked by: 无
父票：[04](./04-routing-product-strategy-first-cut.md)「接路由证据取数侧」那一步的前置裁决
...
3. **经端口取的**：候选的关务资格（`customs-compliance`）、候选成本（`parcel-pricing`）。关务一侧若今天没有按候选作答的判断方法，按 ADR-0146 登为又一处缺执行器，交票 05 的动线取证并另立票，不在本票族里补。
...
6. **定义登记册（迁移 `0007`）与目录（迁移 `0008`）的分工**：first-tenant-runway/03 的 Answer「`0007` 与 `0008` 不合流」一节已答「不合流，视图修订改由目录修订锚派生」，本 ADR 承接或推翻并写理由。[ADR-0068](../../../docs/adr/0068-versioned-network-catalog-structure-precedes-rule-content.md) 决定六（证据视图不读本目录）的部分停用写成**随票 12 落地的那一笔生效**，不留「ADR 已说可读、代码读不了」的中间态。

...
Status: in-progress——跟踪容器：2026-09-24 通道 5 按 /to-tickets 拆为子票 06–16（全部 `draft`，待通道 1 认可拆法后转 ready-for-agent），切片计划见文末；本票自身不再有可执行工作。此前：in-progress——2026-09-24 通道 5 认领（单 task-d12bb120-aac3-40b4-b1e9-a018b77afddc），分支 `mcp5-psb04` 基 `64b37f27`。此前：ready-for-agent（演示网络参考配置那半 Blocked by 03）
...
- **排序**：`SelectRouteCandidate` 全部声明准则打平时按候选标识升序收尾。PS 渠道择优的比较器在同一处交 `ErrChannelCandidateCostTied`、决定记 TIED 等人裁——本票「并列照旧交人工」的「照旧」指的是这里。[`label-channel-service-first-release/01`](../../label-channel-service-first-release/issues/01-channel-candidate-tie-break-authority.md) 当时判路由侧的收尾「仍然正确」，前提是多维准则几乎不会全维打平；成本单维下这个前提不成立（归 08）。
...
- **旧票**：[`first-tenant-runway/03`](../../first-tenant-runway/issues/03-network-resolution-layer.md)（网络解析层，Blocked by `PAR-NET-14`）与「接路由证据取数侧」是同一件事。其 Answer 里「视图修订改由目录修订锚派生」「ADR-0068 决定六随解析层同笔部分停用」两条结论仍成立，由 12 承接；[`auto-reroute-demo-reachability/02`](../../auto-reroute-demo-reachability/issues/02-syn-vertical-run-reaches-reroute-after-lapse.md) 在等它。

### 子票

| 票 | 做什么 | Blocked by |
|---|---|---|
| [06](./06-par-net-14-split-and-rehome-old-tickets.md) | 登记册 `PAR-NET-14` 写回拆分结论，被它挡住的旧票改去处 | 无 |
| [07](./07-route-evidence-sourcing-and-candidate-cost-adr.md) | 取数侧逐族定来源、路由候选成本口径、首版候选生成形态——ADR | 无 |
| [08](./08-route-strategy-family-and-cost-single-dimension-ranking.md) | 路由策略版本声明排序形态；首个内置形态成本单维、并列交冲突 | 无 |
| [09](./09-freeze-boundary-and-executed-prefix.md) | 冻结边界与已执行前缀的判定 | 08 |
| [10](./10-auto-reroute-conditions-folded-from-strategy.md) | 自动改路条件由策略与计划事实折出，改善阈值 | 09 |
| [11](./11-reroute-loading-concurrency-arbitration.md) | 改路与装载 / 交接的并发裁决 | 09 |
| [12](./12-reachability-evidence-folded-from-catalog.md) | 可达性证据从目录折出（视图修订、服务区域、候选、可执行性） | 07 |
| [13](./13-shipment-carries-geo-projection-to-routing.md) | PS 随路由判断携带地理解析投影（ADR-0075 的 PS 半边） | 07、12 |
| [14](./14-initial-route-evidence-folded-from-catalog.md) | 初始路由证据从目录折出（时间投影、段链、策略引用） | 12 |
| [15](./15-candidate-cost-from-leg-buy-evaluations.md) | 候选成本：段的 BUY 评价经 PP 合成 | 07、08、14 |
| [16](./16-demo-network-adopted-as-reference-configuration.md) | 演示网络经 03 的采用路径进入演示租户 | 03、13、15 |

可以立刻开工的前沿是 06、07、08，彼此不相碰：登记册一行、一份 ADR、NR 领域与策略载体。

### 完成判据对照

- **「演示租户上一票已接受的委托能形成初始路由」**：关键路径 07 → 12 → 14 → 15（并 08）→ 16（并 03、13）。**另有一条可能挡在路上的缝不在本票族里**：候选的关务资格。07 若判定 CC 侧今天没有按候选作答的判断方法，那是 ADR-0146 意义上又一处缺执行器，登进票 05 的动线取证并另立票；它补上之前，含关务段的候选只能答状态未知 → 路由判断未决，16 的判据到不了。
- **「`nr-route-evidence-views/01` 与被它挡住的两张 blocked 票各有去处」**：06。

### 做法约定

- 碰 Go / SQL 的子票照[并行会话](../../../docs/agents/parallel-sessions.md)走隔离 worktree。迁移号各票开工时在频道预留，这里不预分——落地次序未定，预分会逼着按号序落。
- 证据视图端口的导出签名在 12 改，开工前在频道报窗口。

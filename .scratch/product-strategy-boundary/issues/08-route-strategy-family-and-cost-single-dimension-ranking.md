# 08 路由策略版本声明排序形态；首个内置形态「满足硬约束后按成本单维择优，并列交人工」

Category: enhancement
Status: draft
Blocked by: 无
父票：[04](./04-routing-product-strategy-first-cut.md)「路由策略族」那一步的排序部分，与「首个内置排序策略」那一步
...
4. **候选成本事实的形状**（金额与币种，或待判断 / 不可计价）在本票定；它从哪里来归 07 与 15。

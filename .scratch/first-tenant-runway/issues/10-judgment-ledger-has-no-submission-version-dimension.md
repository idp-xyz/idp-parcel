...
- 2026-09-07 11:5x · 通道 2：**main 上的 SHA 对照**——MCP-1 在隔离树重放：代码笔 `6d325ca7`（分支）→ **`6a42e6c7`**（main）；机制清点在其上重生成为 **`0f84c0ec`**（分支上的 `c0ced2ec` 未重放，以此代之），远端 main = `0f84c0ec`（`git ls-remote` 于 11:5x）。核过 `git diff 6d325ca7 0f84c0ec -- <本票十六件>` 为空，内容逐字进了 main。worktree `%TEMP%\idp-parcel-mcp2-ftr10` 比内容后拆除，分支 `mcp2-ftr10` 指针保留作封存出处。

...
`cmd/parcel-dispatch` 连跑 11 分钟，库里那一行累计到 `attempt = 24`、`failure_code = dispatch.consumer_undecided`，**进程输出零行**。
...
**失败码分格本身**（`failureCodeFor` 取最响动作格）没有问题，本票不动它：码面答的是动作，不是位置，两件事本来就该分开承载。本票要的是把**位置与原因**补一条出路，不是把它们塞进码面。

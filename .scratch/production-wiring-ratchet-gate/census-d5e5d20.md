...
>
> **但这一格做得进结构，不必留给人记。** 同一遍扫描里多存一个数就分得开——**基线每一行带
> 两个计数：非测试调用数与测试调用数**。零/零 = 全仓无人调（死代码或弃用别名，该删或该显式
> 标注）；零/非零 = 只被测试接线（本票要棘的那一种）。实测于 `d5e5d20`，那 20 个里：
>
> ```
> 零非测试 且 测试也为 0   →  2 个：DecimalFromInt64、Evaluate（都在 parcelpricing/domain/）
> 零非测试 且 测试 ≥ 1     → 18 个：ReplayPricingEvaluation(12)、AssessSafeHandoff(9)、
>                              ValidateBeforeDecision(8)、DecideDisclosure(6)……SubmitEvidence(1)
> ```
>
> **不用多跑一次，只用多存一个数**——这比在文档里写一句「注意区分死代码」硬，因为它不靠人记得。

...
**处方分两档，中间没有第三档**（这一层由 MCP-3 提出，它在 ADR-0071 里两种都用了、用在不同位置）：

- **正文承重处不写数。** 承重的是论点本身，份数只是修饰，删掉。上面那三句属这一档。
- **举证处必须写数，并锚 SHA。** 举证的内容恰恰就是「这几个数各不相同」，不写数就没有举证了；
  此时唯一的出路是锚在一个不会动的对象上。本节开头那段属这一档。

**裸写一个数——不锚、又不是非写不可——在任何位置都是错的。**

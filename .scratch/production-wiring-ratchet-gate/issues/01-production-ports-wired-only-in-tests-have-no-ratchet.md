...
`internal/architecture/` 下六份门禁文件，其中**五份**带 `...CanActuallyCatchAViolation` 自测
（`boundaries_test.go` 无）。因此对其中任一道门禁而言，「本包另 N 套」的正确值是 **4**。实际：
...
**改法优先「不写数」**（例如「本包其余门禁同一约定」），其次才是「六份（截至 `<SHA>`）」：
带 SHA 只是让过期可见，不写数则根本不会过期。**新增第七道门禁的人不该负责去改前六份的注释**——
这条约束改成不写数之后就自动消失了。

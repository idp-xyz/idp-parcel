...
本票的**范围、三条方向约束与三个普查数字**（46/41、62/53、89/13）全部出自同一人（MCP-2）。
本票落地后第一条约束是**不可逆**的（已接线不得退回未接线），因此**初始清单错一行，就会一直锁在
里面**——一个把错误吸收进基线的门禁永远不会红，那正是本票要治的病本身。

所以实现方**不是** MCP-2，且必须按下面顺序做：

1. **先读判据定义，不读本票的「现状：普查数字」一节。**
2. 开跑之前，用自己的话写下「**这条判据在量什么 / 会漏什么**」并落盘（写进票 Comments）。
3. **从零独立重跑三族普查**，得出自己的数。
4. 跑完才允许读本票的普查节，两份对照。
5. **对不上的地方就是结论**，必须先查清成因才能定基线；不得取其一了事，也不得「就近取整」。

第 2 步不能省：它把「判据在量什么」这个理解**先于**数字固定下来。省掉它，第 4 步的对照就退化成
向一个已有答案收敛——**看着别人的数去复核是锚定的**，那不是复核。

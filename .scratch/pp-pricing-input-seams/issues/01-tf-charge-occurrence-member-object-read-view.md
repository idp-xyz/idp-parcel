...
- **2026-09-15 11:4x · 评审 ← 通道 6 · 钉 `71dd8313`（检出 `b33cdbba`）· 基线 `3a21dab7`**（task-e2d98535，只读隔离检出 `%TEMP%\idp-review-ppseams01`；由推送方代落）。范围核实：`git diff --stat 3a21dab7 HEAD` 5 文件 +358 −2；`-- internal/parcelpricing migrations` 空；`-- internal/transportfulfillment/domain` 空（`CarriedObjectReference` 一字未动）；`go vet ./internal/transportfulfillment/...` 0；未跑真库（接手方已报 677 PASS，批 tip 113 ok）。**Standards——阻断：无。非阻断 2 条。** ①【注释里的跨文件计数】`ports/charge_occurrence_member_view.go` `ChargeOccurrenceMembers` 头注「CONTEXT『运输收费发生项』固定下来的三件」与 `ChargeOccurrenceMemberView` 头注「协议、数量与修订三件」——都已点名，「三件」是冗余的数（AGENTS「改文档」：数别处的东西）；去掉两个「三件」即可，可改可不改。②【夹具前缀】用例复用 `occurrenceRecord` 既有夹具（`tenant-1` / `journey-1` 非 `SYN-`），spec「边界」写「夹具全 `SYN-`」；发生项与成员引用已是 `SYN-`，全部合成、无真实值，作者判断项 ④ 已自报，不要求改。**无发现**：新增注释全中文、无行号；`LoadMembers` 走 `ReadExecutor` 纯读；口上无 `Save`、不交 `ChargeOccurrenceRecord`，形照 `FailedAttemptSource` 头注那条理由（「契约窄一格，说的话就准一格」原句）；编译期断言 `var _ ports.ChargeOccurrenceMemberView = (*ChargeOccurrences)(nil)` 在；TF Boundaries 首条（不铸身份）：SQL 只 `SELECT member.object_ref`，无任何前缀 / 形状分支；不迁移。**Spec——阻断：无。非阻断 1 条。**【成员顺序是不是契约】口头注没说成员切片有无顺序保证，而实现 `ORDER BY member.object_ref` 且对方首条用例断了字面顺序（作者判断项 ③）。两条路都行，但建议选「写进契约」而不是删断言：PP 消费侧要拿成员清单进计价输入快照的指纹，确定序是它的需要；口头注补一句「成员按引用字面升序，与 `FindByKey` 同序」后，那条断言就从实现细节变成契约。非阻断，可后续票。**裁决逐条**：1 ✓ 裸引用原样、不带种类（`Members []domain.CarriedObjectReference`，SQL 无派生列；`DoesNotInterpretReferenceShapes` 三种形状原样）。2 ✓ 不展开（无 NO 读口、无第二次查询）。3 ✓ WHERE 三列精确到有效性版本，另一版 found=false（`KeysOnTheExactValidityVersion` + 接手投影条 v1/v2 各答各的）。4 ✓ 新文件一口一问、无 Save、只交成员 / 时点 / 范围、同一结构体两口、不迁移、PP 零 diff。5 ✓ `offsite_pickup.go` 零 diff。**专看项**：1. `ChargeOccurrenceMembers{Members, OccurredAt, Scope}` 恰是裁决 4 三件，协议 / 数量 / 修订不外泄 ✓。2. 空成员响亮报错——**判对**：`domain` 构造门 `len(spec.Members) == 0` → `ErrInvalidChargeOccurrence`，`Save` 在同一 executor 里写本体与 `transport_charge_occurrence_member`，库上本体在而成员空只能是旁路改过；与同仓 `closedWord`「集外即库被旁路改过作错误抛出」同形，不是越权；交空清单会让 PP 把「没成员」当事实。3. 顺序断言——非阻断，建议如上（量过 `FindByKey` 的 `loadMembers` 也是 `ORDER BY object_ref`，两口同序属实）。4. 无任何按前缀 / 形状分支（grep 适配器 +70 行：只有 `NewCarriedObjectReference` 构造门）。5. 三处零 diff ✓。6. 接手纪律属实：两条 red（投影条→判据 2 + 裁决 3；形状条→裁决 1）对的是判据，与对方五条切法不同；弃掉的他租条确与对方同切法。一处小不一致只记不评：接手方「改对方断言不在纪律内」，却给对方 NotFound 用例补了 `Scope` 零值断言（加覆盖与删断言不同量级，可接受）。7. **判断项五条**：① 同意；② 同意；③ 同意它是实现细节，处置建议「写进契约」而非删；④ 同意；⑤ 同意（PP 消费侧归后继）。**结论**：两轴 0 阻断，可进 main；三条非阻断都不要求改。未改任何文件。
- **2026-09-15 11:5x · 进 main 记录 · 通道 1 推送方**：评审三条非阻断**处置**——Standards ①（两个「三件」）与 Spec ①（成员顺序写进口头注成契约）合成一张 A 类零行为尾巴候选，与 sa-cc/20 评审 Standards ① 同批攒票，不挡合入；Standards ②（既有夹具前缀）记于此，不改。**重放**：隔离检出 `%TEMP%\idp-replay-wave1`（首波：pp-seams/02 → sa-cc/22 → 本票 → sa-cc/20），`cherry-pick 3b91cb5a 71dd8313 b33cdbba` → `28c3c063` / `c88b5e35` / `d710fada`；pp-seams `spec.md` 撞一次（01 行与 02 行相邻各改），按意图解：01 行取本分支、02 行取树上；`git diff b33cdbba d710fada -- internal/transportfulfillment .scratch/pp-pricing-input-seams/issues/01-*` 空。**验证（推送方全量一次，批 tip `c657a5e7`）**：`gofmt -l` 空、`go build ./...` / `go vet ./...` 0；11:3x `check_messages(waitMs 0)` 排队列 → 广播占 55432 → 带 DSN `go test -p 1 -count=1 ./...` **113 ok / 0 FAIL / 16 无测试 / 0 cached**（131 s）；探针 `-run ChargeOccurrenceMemberView -v` 七条 PASS 非 SKIP；释号广播。清点在批 tip 重生成（`c657a5e7`：TF 生产 140→141 / 测试 128→129、端口 409→410）。共享 main `merge --ff-only c657a5e7` → 簿记一笔在其上（本条 + Status 行 + spec 01 行 + tasks.md）→ `ls-remote` 核 `bd919b72` 未动 → `push <sha>:main`。`mcp4-ppseams01`（封存笔）与 `mcp3-ppseams01` → `merged/`、远端删；两棵作者树（通道 4 / 3 均 crash）由推送方比内容后拆。

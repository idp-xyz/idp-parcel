...
| [05](issues/05-ps-submission-and-source-data-versions-carry-content.md) | PS 提交版本与客户原始资料版本留内容（测量 / 地址要素），使 02 / 03 两口能按基线锚与已采用版本锚答值——今天条目只进摘要、`CustomerSourceDataVersion` 只留痕；留哪些、留在哪、修订内容从哪来归 PS owner（draft） | PS | 无（02 / 03 已落，本票让它们能答值；PP 消费侧适配器票不等它，先按「输入不可得」停） |

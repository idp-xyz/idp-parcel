...
Category: chore
Status: resolved——**已进 main，2026-09-14 10:0x**（通道 1 推送方重放：main `7b4cffc1` 之上 `cherry-pick cdf17834..mcp4-tails` 九笔零冲突，本票三条 = `517aa63b` / `ac179f5f` / `6d546443` + `f1c21f93`，完成记录随 `686279c1` + 推送方清点 `69406e6d`（PP 生产 88→89，全是本票条 3）+ 本簿记笔；非作者评审 ← 通道 1 两轴 0 阻断（Standards 0 / 3 非阻断 · Spec 0 / 2 非阻断，全文见 Comments）；`9cf30fab` 带 DSN 全量 110 ok / 0 FAIL）；此前 resolved——完工待非作者评审进 main，2026-09-14 09:4x（分支 `mcp4-tails` 基远端 main `cdf17834`；三条对应四笔 `bbc156e3` / `e46decfd` / `9693a4ee` + `6388e32b`，完成记录随同分支 lc/38 条 3 那一笔同提交而不是随 `6388e32b`——见 Comments「完工」判断项 ⑤；带 DSN 三包 PASS 181 / SKIP 0；清点预报 PP 生产 +1）；此前 in-progress——2026-09-14 08:5x 通道 4 按通道 1 派单 task-65ce5275 自立自做（SA 两条 + PP 一条合一票，先例 sa-cc/16；PP 那条作者与评审同判归 PP owner，本票代收），分支 `mcp4-tails` 基远端 main `cdf17834`（树 `D:/tops/idp-parcel-mcp4-tails`），与 [lc/38](../../label-channel-service-first-release/issues/38-lc37-review-tails-adr0049-citation-five-values-count-and-fact-reference-rename.md) 同分支——两票都撞 `cmd/parcel-dispatch/assemble.go` / `assemble_test.go`；要裁的为零
...
- **2026-09-14 09:1x–09:4x · 通道 4（task-65ce5275）· 完工**。分支 `mcp4-tails` 基远端 main `cdf17834`，与 lc/38 同分支（推送方重放后 main 上 SHA 会换，对照由推送方在「进 main 记录」补）。立票笔 `026dbbb0`（两票 + 两 spec 行，无代码）。
  - **条 1 `bbc156e3`**：`apply_pre_acceptance_control.go` `ErrNilDependency` 文本 →「settlement accounting: dependency is nil」，头注补一段它为什么不带编排名（两只构造器共用，哪条编排、哪一口由调用点 `%w: <口名>` 说）；变量名、`errors.Is`、两处调用点不动。`TestAnyNilDependencyIsRefusedAtConstruction`（八口）与 `TestTheHandlerRefusesANilDependencyAtConstruction` PASS。
  - **条 2 `e46decfd`**：实改**八句**——重量时的七句（`form_on_buy_evaluation_recorded.go` 三句、`form_on_buy_evaluation_recorded_test.go` 两句、`assemble.go` `buyEvaluationRecordedUndecidedSentinels` 名单那一格与 `formSupplierExpectedCostOnBuyEvaluationConsumer` 头注两句）+ 判据 2 的 grep 在同包顺带扫出的 `buy_evaluation.go` `outcomeOf` 头注「提供方的五种结果逐格译成 SA 的五格」（不是 01 新增的，同一族，同笔收）。「三件」一律去数词只留发生项 / 费用项目 / 供应商协议的点名；「五种结果」→「每一种结果」并点 `saports.BuyEvaluationOutcome` / `EvaluationStatus`。`buy_evaluation_recorded_consumer_test.go` 零计数、零 diff。票面 / 裁决文与 08 那批 `.go` 不动。
  - **条 3 `9693a4ee` + `6388e32b`**：新包 `internal/parcelpricing/pptest`（一份普通 `.go`：`PlanSpec` / `InputSpec` / `Period` / `AmountRoundingSpec` + `Plan` / `Input` / `Evaluate` / `Value` / `IdentityReference` / `FingerprintReference`，包头注写明为什么是普通包、为什么零默认值、哪几件写死）；三处改调——cmd `syntheticPricingEvaluation`（`ppTestValue` 退位）、SA `buyPlan` / `packageInput` / `evaluate`（`reference` 助手退位，其余助手仍被票级用例用、留着）、PP `syntheticPlan` / `syntheticInput` / `evaluatedFixture`（`evaluationValue` 被同包十余份用例用、不动），本地函数名与全部用例正文零改。**值一字不变的证据**：在 `cdf17834` 干净检出与本树各放一份临时探针（不入库、已删），三处夹具产出的评价经 `domain.MarshalEvaluationSnapshot` 折成快照——cmd BUY / SELL、SA 已完成 / 未声明取整 / SELL / Z9 待判断、PP Z1 / Z9——两侧**逐字节相同**（5016 / 9051 / 4367 字节，含 `planContentDigest`、manifest 指纹、input、chargeLines、amountRounding、semanticDigest）。
  - **验（本机）**：`gofmt -l`（暂存 blob）每笔空；`go build ./...` / `go vet ./...` 0；09:35 占号广播，**带 DSN `go test -p 1 -count=1 -v` `cmd/parcel-dispatch` + PP `adapters/postgres` + SA `adapters/parcelpricing` 三包 ok，`--- PASS` 181 / `--- SKIP` 0 / `--- FAIL` 0**（09:36:01→09:36:10，钉 `6388e32b` 的工作副本），含 `TestARecordedBuyEvaluationStopsUndecidedAtTheExpectedCostSeamThroughTheRouteTable`（cmd 真库）、`TestCompletedEvaluationIsReadBackUnchanged` / `TestFailedEvaluationIsRecordedWithIssues` / `TestSecondWriterGetsAlreadyRecorded`（PP 真库）、`TestCompletedEvaluationIsAdoptedAtTheDeclaredIncrementScale`（1250 分 / `SYN-BUY-CARD@v2`）、`TestUndeclaredPrecisionIsRefusedNotRounded`、`TestBoundariesOfTheBuyEvaluationSeam`、`TestShipmentLevelEvaluationCarriesItsMembersToSettlement`；09:36 释号广播。不带 DSN `go test -count=1 ./internal/settlementaccounting/... ./internal/parcelpricing/... ./cmd/parcel-dispatch/... ./internal/architecture/...` 全 ok。未跑全量。
  - **判据逐项**：1 ✓ `git grep -n 'pre-acceptance control dependency' -- internal/ cmd/` 零、声明一处不含编排名、两用例 PASS；2 ✓ `git grep -n -E '三件引用|三件来源|五种结果|供应商协议三件' -- internal/settlementaccounting/adapters/parcelpricing/ cmd/parcel-dispatch/assemble.go` 零；3 ✓ 三处本地函数仍在、体内调 `pptest`，`git grep -l 'parcelpricing/pptest' -- cmd/ internal/` 只命中三份 `_test.go`，`go list -deps ./cmd/...` 与全仓 `.Deps` 零 pptest；4 ✓ 见上（PASS 181 / SKIP 0）；5 ✓ 见上；6 ✓ 预报 PP 生产 +1、其余零差（不加端口、不加缝、不加路由；两只棘轮门禁与 `TestProductionPackagesDoNotImportFirstPartyTestScaffolding` 在新包落地后实跑绿、基线零 diff）；7 见判断项 ⑤。
  - **红线**：除条 1 那一串文本外，生产 `.go` diff 只有注释行（条 2 `git diff -U0` 非 `//` 行零）；`pptest` 不在任何生产 import 闭包；三处夹具值逐字节同（见上）；`adapters/inbox/**`、迁移、`ports`、`domain`、CC / VE、`apps/` 零 diff。
  - **判断项（归评审）**：① 条 3 的抽法是「规格显式、零默认值」而不是派单默认的「零参数共用夹具」——三份值不同，两者不可兼得，取值不变那一边；代价是每个调用点一段十来行的规格字面量。② `pptest` 是普通包、清点记 PP **生产** +1（派单预报「测试 +1」按 `filecensus.go` 后缀口径不成立）；两只棘轮门禁把它当生产源扫，今天基线零差是因为它只调 PP 已有生产调用点的构造器——将来夹具若先于生产代码用上某个新构造器，会让棘轮误判「已接线」，这是 `pgtest` 同款的既有暴露面，不在本票解。③ 条 2 顺带收了 `buy_evaluation.go` `outcomeOf` 那句（非 01 新增）——不收则票面判据 2 的 grep 不为零；若评审认为该另笔，退回零行为。④ `pptest.Value` 与 PP 的 `evaluationValue` / cmd 的 `saTestValue` 同形并存——`evaluationValue` 被同包十余份用例用，改成转调是十余处零收益 diff，留着。⑤ **本票完成记录没有随本票最后一笔代码 `6388e32b` 同提交**，而是随同分支 lc/38 条 3（下一笔）同提交：`6388e32b` 已推到 origin，改写已推历史换一次「同笔」不值；两票同分支、推送方重放整条分支，SHA 对照仍逐笔可写。
- **2026-09-14 10:0x · 非作者评审 ← 通道 1 推送方**（钉 `c569ea36` = 重放 tip `9cf30fab` 同内容；通道 2 / 3 已 crash，无第三人；只读脚本核 + 通读 diff 与 `pptest/evaluation.go` 全文）：
  - **Standards**：阻断 无。非阻断 ① `pptest` 在清点里记 PP **生产** +1（88→89）——`filecensus` 按 `_test.go` 后缀分类，与 `pgtest` 同款；读清点的人要知道这个「生产 +1」是一份测试脚手架（作者判断项 ② 已写清），不改。② 08 那批 `.go` 仍有六处「三件引用 / 五种结果」（`adapters/postgres/evaluation_request{,_handoff}_test.go`、`application/request_buy_evaluation{,_test}.go`、`domain/evaluation_request.go`、`ports/evaluation_request.go`）——票面明写「08 那批 `.go` 不动」，守票面是对的；`domain` 那处数的是同文件字段不算，其余五处归 SA owner 候选后继一笔。③ `pptest.Value` 与 PP `evaluationValue` / cmd `saTestValue` 三份同形并存（作者判断项 ④）——留着的理由成立（十余处零收益 diff），但助手上的 rule-of-three 也到了，归 PP owner 候选后继。无发现：`pre-acceptance control dependency` 于 `internal cmd` **0**；sa-cc/01 那批与 `assemble.go` 的计数 **0**；`pptest` 只被三份 `_test.go` import（`git grep -l` 非测试 **0**）、只 import `testing` / `time` / PP `domain`；包头注把「为什么普通包 / 为什么零默认值 / 哪几件写死 / 为什么不手搓评价」写成取舍不是流水账；`ErrNilDependency` 头注补段讲清为何不带编排名；注释全中文、无行号。
  - **Spec**：阻断 无。非阻断 ① 「值一字不变」的证据是作者的临时探针（三处快照逐字节同，未入库）——推送方没复现探针，改以带 DSN 全量 110 ok 与 `-v` 探针作旁证（cmd 真库 `TestARecordedBuyEvaluationStopsUndecidedAtTheExpectedCostSeamThroughTheRouteTable` PASS；SA 钉 `SYN-BUY-CARD@v2` / 1250 分那批照旧 PASS），接受。② 完成记录随 `96bd5d3a` 而非本票最后一笔代码 `6388e32b`（判断项 ⑤）——两票同分支整条重放、SHA 对照逐笔可写，接受，不改写历史。无发现：判据 1–7 推送方复核同结果；条 2 顺带收 `outcomeOf`（判断项 ③）接受；抽法「规格显式、零默认值」（判断项 ①）与 AGENTS.md「不写死为生产默认」同一条精神，接受；红线核过——`adapters/inbox/**`、迁移、`ports`、`domain`、CC / VE、`apps/` 零 diff。
  - **结论**：可进 main；五条非阻断全接受；**候选后继**：SA owner——08 那批五处计数换点名；PP owner——`pptest.Value` / `evaluationValue` / `saTestValue` 三份同形合一。
- **2026-09-14 10:0x · 进 main 记录（通道 1 推送方）**：与 [lc/38](../../label-channel-service-first-release/issues/38-lc37-review-tails-adr0049-citation-five-values-count-and-fact-reference-rename.md)「进 main 记录」同一次重放（`%TEMP%\idp-replay-tails-0958` detached `7b4cffc1`，九笔零冲突，tip `9cf30fab`；带 DSN 全量 110 ok / 0 FAIL / 16 无测试 / 0 cached；清点 PP 生产 88→89 → `69406e6d`；SHA 对照九笔全列在那边）。本票四笔：`bbc156e3→517aa63b` / `e46decfd→ac179f5f` / `9693a4ee→6d546443` / `6388e32b→f1c21f93`；完成记录所在 `96bd5d3a→686279c1`。

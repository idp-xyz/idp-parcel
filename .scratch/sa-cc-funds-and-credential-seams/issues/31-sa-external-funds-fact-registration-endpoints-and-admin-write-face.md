# SA 外部资金事实采用与更正的在线登记面：`parcel-api` 端点两行 + `settlementhttp` 第一份命令文件 + `UnconfiguredIntake` 长出命令 Intake + 管理台写签两阶段（sa-cc/07 裁决 0 第二步）

Category: enhancement
Status: ready-for-agent——**0006-09-15 13:5x 通道 3 随 sa-cc/07 完成记录同笔立（07 裁决 7 (7)）**。族别与形已由 07 裁决 1 / 0 定（ADR-0085 读作「算」；入口两者都要、分两步），本票是第二步，没有再要裁的；只写票面未动代码；取证锚分支 `mcp3-sacc07@aa48910e`（基 `bb7068f0`）与 07 取证条（钉 `7160fe67`）
Blocked by: 07（本笔同时把 07 转 resolved；**开工要等 07 进 main**——端点消费的译装 `adapters/registrationjson`、拒 nil 的 `NewMapExternalFundsHandler` 与真库垂直用例都是 07 落的，在它进 main 前开工会在两条分支上各长一份）

## 缺口（07 取证条钉 `7160fe67`，本票立票时在 `aa48910e` 复量同形）
...
- 译装只用 `adapters/registrationjson` 那一份——端点侧不得另写一份 JSON → 命令（ADR-0085 决定一「同一登记用例」，07 分家理由）。
...
0. `settlementhttp.UnconfiguredIntake` 实现两个新 Intake（编译期断言 `var _`）；`IsolatedOperationsReadIntake` **不**实现（用例以类型断言证否）；`unconfigured_intake.go` 头注「将来长出命令 Intake 时本类型刻意不实现」那句已改，`isolated_read_intake.go` 零 diff。

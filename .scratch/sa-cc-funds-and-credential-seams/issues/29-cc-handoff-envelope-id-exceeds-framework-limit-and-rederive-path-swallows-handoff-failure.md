...
- **2026-09-15 14:38 · 评审 ← 通道 6 · 钉 `b2229462`**（隔离检出 `%TEMP%\idp-review-sacc29` 只读；`go build ./...` 0、`go vet` CC + dispatch 0、`gofmt -l` 空、`SYN-D9|SYN-U9` 零命中；迁移 / `internal/settlementaccounting/` / `.scratch/` 对基线零 diff）：**Standards 阻断 无；非阻断 2**：① `cmd/parcel-dispatch/assemble_test.go` `TestARejectedDutyVerificationHandoffIsNotRegisteredAsUndecided` 头注「与两只未决哨兵也互不 errors.Is」——遍历的 `externalFundsFactUndecidedSentinels` 实有三只（含 `ErrAdoptedFactNotVisible`），跨文件计数且数错，改「与名单里每一只」即可；② `external_result_handoff.go` `fingerprintEventID` 头注引 19 的「四个引用 + 六十四位 = 138 字节」作论点，锚的是票不是 SHA。**无发现（核过）**：新增注释全中文、跨文件引用皆符号名；实例半边不写死——超长夹具用 `strings.Repeat` 对上限量并先断言串接形确实 > 上限再证；`DutyVerificationHandoffUnavailable` 与既有 `*Unavailable` 同族、`Err*Rejected` 有 `accessidentity.ErrCredentialRejected` 先例；`rederiveStores`（30 的排序）只加 `handoffErr` 一格、排序未动。**Spec 阻断 无；非阻断 5**：① 裁决 1 尾句「Subject / PartitionKey 能超 512 则同法取哈希」——作者量了（硬失败用例用 512+ 的范围引用造 Subject 超限，`domain.newRequiredValue` 只查非空、无长度门，**能超**），却未取哈希，保留可读形、靠裁决 2 (b) 硬失败兜住；裁决 2 自己也把 (b) 留给 Subject / PK，两条裁决在此互相拉扯；结果是响亮硬失败不是静默，缺口已闭，不阻断——完成记录判断项须写明「能超、为何不哈希」，owner 若坚持裁决 1 字面另票；② 判据 (4) 真库用例落在 `assemble_test.go` `TestAFormedDutyPaymentVerificationReachesTheSettlementInputThroughTheRouteTable` 尾段（换 ID 再投 → `published == 1`、无失败码、五维 count == 1），不在派单点的 SA `adapters/customscompliance` 目录；裁决 5 写的是「可放」，走生产路更强，位置备案；③ `rederive_duty_verifications.go` 默认格把交接口**任何**非 `ErrHandoffEnvelopeRejected` 的错都归 `DutyVerificationHandoffUnavailable` 重投——含 handoff 自己「键空白 / 时刻缺席」那两条确定性装配缺陷，实际到不了（键取自刚落册的 `record.Key`），记一句；④ 同一张脸的余数（不归 29）：`customsCaseEventID`（五个引用串接）、`manifestEventID`、`declarationSubmissionEventID`、`caseClosureEventID`、`externalResultEventID` 仍是串接形，同一潜伏缺口，CC owner 另立票；⑤ 判据 (6) 05 票面那句未在 `b2229462`（随记录笔 `5d2a6807` 落，推送方已核）。**无发现（核过）**：裁决 1——`fingerprintEventID(口名前缀, 维…)` = 前缀 + "/" + sha256 十六进制（`\x00` 拼接），三只同改，长度 90 / 82 / 89 ≤ 128，五维全进哈希、同输入同 ID；载荷、Subject、PartitionKey 未动（`partitionKeyOf` 断言）。裁决 2——链路实核：`envelope.Validate` → `invalidEnvelope` 以 `%w` 包 `eventing.ErrInvalidEnvelope` → `outbox.Store.Enqueue` 原样返回 → `outboxintent.EnqueueOnce` `%w` → handoff `errors.Is` 命中再包 `ports.ErrHandoffEnvelopeRejected`；编排先看 `HandoffError()`，被拒 → 零结果 + `ccapplication.ErrDutyVerificationHandoffRejected`，其它 → `rederivationUndecided(DutyVerificationHandoffUnavailable)`；适配器再包 `ccsettlement.ErrDutyVerificationHandoffRejected`，名单不收（源码 grep 全仓仅注释提及），`errors.Is` 反断言在适配器测试与装配测试各一。**19 评审 Spec ② 那条在新代码上不再成立**：重派路只在 `HandoffError() == nil` 时才把谱系放进 `Lineages()`，任何交接失败都回滚，「核对行提交、信封没发」不可能再发生；`Lineage.Result.HandoffReference()` 仍无人读但已无害。裁决 3——`handOffVerification` 形不动、只多交 `error`，`VerifyPayment` 对外结果不变，既有续办用例加一条 `HandoffError` 断言仍绿。判据 (3)——第三格引用改回 `SYN-DUTY-RD/v1` / `SYN-UNIT-RD`，取短理由句已删，`HandoffReference() != ""` 反断言、`published == 1`、`FindByKey` 三处保留。**结论：可进 main**（Spec ① 由推送方在完成记录里核到「能超、为何不哈希」再合）。
- **2026-09-15 14:4x · 进 main 记录 · 通道 1 推送方**：**推送方对评审的处置**：Spec ① 推送方核完成记录判断项 ③ ⑨——「能超」已量实（真库格 512+ 范围引用 PASS 并拿到 `ErrInvalidEnvelope`）、「为何不哈希」写明改由裁决 2 (b) 兜成响亮硬失败、要不要哈希归 CC owner 一句或另票——满足评审的合入条件；Standards ① ② 合进尾巴票 [33](33-sixth-batch-review-standards-tails-test-currency-code-count-words-sentinel-count-and-unanchored-byte-claim.md)；Spec ④ 另立 [34](34-cc-five-remaining-event-ids-still-concatenate-references.md)（draft，CC owner）；Spec ② ③ 备案于本条，不立票。**重放**：`%TEMP%\idp-replay-wave6` @ `cbffc269`（本票分支基即 main tip，无中间笔），`cherry-pick cbffc269..mcp4-sacc29` 三笔 + 记录笔 `5d2a6807` 零冲突；SHA 对照：`976966ee→c4be9985` / `e8d73f09→34daa57a` / `b2229462→d1ebc8c1` / `5d2a6807→cbe83f8a`；`internal/customscompliance` 与 `cmd/parcel-dispatch` 对作者 tip 零 diff、票面对分支 tip 零 diff。同批 sa-cc/27 与 pp-seams/05，三票零文件重叠。清点在批 tip 重生成 `de822820`：CC 零差（与作者预报同）。**验证钉 `de822820`**：`gofmt -l` 空；`go build` / `go vet` 0；14:34 占号，带 DSN `go test -p 1 -count=1 ./...` **115 ok / 0 FAIL / 16 无测试 / 0 cached**（136 s）；`-v` 探针 `TestOverlongReferencesStillProduceAnEnvelopeIDWithinTheFrameworkLimit` / `TestARederivedVerificationWhoseEnvelopeIsRejectedRollsBackLoudlyInsteadOfCommittingHalf` / `TestAFormedDutyPaymentVerificationReachesTheSettlementInputThroughTheRouteTable` / `TestARejectedDutyVerificationHandoffIsNotRegisteredAsUndecided` PASS 非 SKIP；14:37 释号。簿记一笔在其上，纯 .md 自审；`ls-remote` 核 `cbffc269` 未动 → `merge --ff-only` → `push <sha>:main`。分支 `mcp4-sacc29@5d2a6807` 作封存出处、改名 `merged/`、远端删；`%TEMP%\idp-parcel-mcp4-sacc29` 由通道 4 比内容后拆。

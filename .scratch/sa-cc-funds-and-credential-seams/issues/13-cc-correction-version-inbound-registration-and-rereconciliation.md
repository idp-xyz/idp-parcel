...
- **2026-09-14 14:0x · 进 main 记录（通道 1 推送方）**：分支 `mcp1-sacc13` 基 `ace35ac3` = 当时 main tip，`ls-remote` 核 main 未动 → 无需 cherry-pick，四笔 SHA 原样 `c90d8606` / `e833355d` / `bab22328` / `8dbd49e2`；清点在 `8dbd49e2` 干净检出重生成 → `0bd86d42`；`gofmt -l` 空、`go build ./...` / `go vet ./...` 0；14:0x 先排队列再占号，带 DSN 全仓 `-p 1 -count=1` **110 ok / 0 FAIL / 16 无测试 / 0 cached**（130 s）；释号。簿记一笔在其上 → ff → `push <sha>:main`。分支 → `merged/`，远端删，树拆。
  ...
  - **2026-09-14 15:3x · 补评审 ← 通道 4 · 钉 `8dbd49e2`**（task `e65b3d52`；推送方通道 1（原会话 15:2x crash，由原通道 2 会话接管）15:25 重点名、截止 15:29 前应答 2 个（通道 3 / 4），12 给通道 3、本票给通道 4——通道 4 旧会话 09-10 只立过本票票面未动代码，票面作者不等于代码作者；通道 5 / 6 截至 15:29 未应答。基线 `ace35ac3`，检出 `$env:TEMP\idp-review-sacc13` detached 干净；16 件；`migrations/` 只有 `0021`；`internal/settlementaccounting` / PS / PC / docs / apps 零 diff。以下为评审原文，推送方只把两个轴标题改成行内粗体）：
    **Standards** —— **阻断**：无。**非阻断**：1. 【判断项·Duplicated Code】`internal/customscompliance/application/reconcile_duty_payment_test.go` `dutyStoreDouble.ListFundsFactVersions` / `LoadFundsFact` 与 `adapters/settlementaccounting/receive_on_adopted_funds_fact_test.go` `fundsFactRegisterDouble` 同名同形（前缀扫 order 切片、Load = 末项）。两包各自的测试替身，可接受；若再长出第三份就该抽到一处。2. 【判断项·防御缺口】`adapters/settlementaccounting/receive_on_adopted_funds_fact.go` `HandleAdoptedExternalFundsFact`：版本取信封 `adopted.Version`、回指取 `content.Corrects`，但回查交回的 `content.Version` 被丢弃、未与信封所指比对。今天读口按版本取、两者同源（头注写明），只是若 SA 视图哪天答非所问，CC 会把别版内容登在信封那一版名下。加一句相等校验即可，不改语义。3. 【diff 外·邻接注释变旧】`cmd/parcel-dispatch/assemble_test.go` `TestAnAdoptedExternalFundsFactReachesTheCustomsRegisterThroughTheRouteTable` 头注仍写「CC 的 external_funds_fact 落一行」；0021 起是身份一行 + 版本一行。作者动了同一函数体、未动这句。**无发现**：注释全中文；新增行 grep 无行号、无「第 N 行」、无数别包（「两个读口分工」「两口端点」数的是自己的接口/文件）；新领域词只 `domain.FundsFactVersion`（与 SA 同字面，头注写明只作引用不比大小不复制链）；`0021` 无 INSERT，夹具全 SYN-/bank-fact 合成，零预填零默认；`corrects_version` 无自引外键、无链连续校验——是引用不是 SA 版本链的第二份（票面红线一）；`external_funds_fact_consumer.go` 与 `receive_on_adopted_funds_fact.go` 无任何按 `corrects` 分路（红线三）；`RegisterFundsFact` 两次 `ON CONFLICT DO NOTHING` 键 (租户,事实,版本)，旧行零改写（红线二 / UC-CC-009「不按最后到达覆盖」）；ADR-0029 分格正确——`ReceiveFundsFact` 末尾「写口说已登记、读口列不出」落 `FundsFactRegisterUnavailable`（恢复动作 = 重投），版本空白 / 回指自己落 `未受理`（查询前短路支）；`LoadFundsFact` 端口头注与 postgres 头注都如实写「最近接收的那一版」、不暗示唯一。
    **Spec**（票面 `8dbd49e2` 版）—— **阻断**：无。**非阻断**：1. 【地盘外两件·判据/地盘不符】`cmd/parcel-customs-register/duty_registers_test.go`（`fakeDutyBook.ListFundsFactVersions`）与 `internal/customscompliance/adapters/http/register_credential_and_duty_test.go`（`dutyStoreStub.ListFundsFactVersions`）不在票面「地盘」所列目录（`adapters/http` 与 `cmd/parcel-customs-register` 均未列）。两处都是接口拓宽后编译强迫的替身桩、只加一个方法、不改断言，实质无害；但完成记录未点名，parallel-sessions 的地盘规则要求动前占号/事后记账。请推送方在票面补一句。2. 【判断项 ②·债的形状，给 19】`VerifyPayment` 在生产有调用方（`cmd/parcel-api/assemble_customs_registration.go` `transactionalDutyPaymentVerificationRegistration.VerifyPayment`、`cmd/parcel-customs-register/translate.go`）。13 之后同一事实有 ≥2 版时，付款人维读的是最近接收那一版，而 `duty_payment_verification` 键 (租户,税费,资金事实,范围,指纹) 里没有资金版本——v2 到达后调用方若以同三轴同依据重核，落 `已存在`，形成不出「新核对版本」。**不阻断**的理由：裁决 2 明文把「核对按版本读」划出本票；端口头注如实；票 19 做法 3 已接；单版事实行为与 13 前完全一致，多版事实从「v2 丢在 inbox」变成「v2 在册、核对暂按最近版」，严格少错。请 19 立字时把「核对身份缺资金版本」写成完成判据。**无发现**：判据 1 ✓——`TestACorrectionVersionIsReceivedAsANewRowThatPointsBackToTheVersionItCorrects`：v2 `已接收`新一行、列两版、v2 回指 v1、`versions[0] != first.Registration` 严格等值证 v1 一字不动、同版本重投 `已存在`、换金额 / 换回指 `内容冲突` 且册不动；`TestFundsFactVersionsKeepTheirOwnRowsRegardlessOfArrivalOrder`：版本空白 / 回指自己 `未受理` 不落，先 v2 后 v1 各占一行。判据 2 ✓——`0016` 零 diff；`0021` 唯一新迁移；`TestFundsFactVersionsAccrueAsRowsThatPointBack` 直读身份 1 行 / 版本 2 行；dispatch 正例第二格 v2 经 `external-funds-fact.adopted` 信封 → 消费者 → SA 只读视图 → `ListFundsFactVersions` 列两版、v2 回指 v1 金额 9000。判据 3 ✓——`payer_ref` 连同内容列搬进子表，`external_funds_fact_version_payer_provided_or_null` 保留 12 的 NULL=未提供 + 空白拒；`TestAFundsFactWithoutAPayerRoundTripsAsExplicitlyNotProvided` 改读子表按版本点读、断言未弱化；「付款人空白串」拒格保留。判据 4 ✓——消费者头注改为「处理方按（引用 + 版本）登成新一行、回指前版；触发重核对归后继票」。裁决 1 ✓——版本子表键 (租户,事实,版本)，外键 `external_funds_fact_version_belongs_to_fact` 到身份行；`corrects_version` 可空且 CHECK 非空白 ∧ ≠ version；身份表主键未动，`duty_payment_verification_funds_fact_received` 外键原样成立（0021 未触碰、「没有资金事实的核对」拒格仍在）；「同引用列全部版本、每版回指前版」= 一条 SELECT。裁决 2 ✓——未长出任何触发编排；完成记录「后继票的形」与 main 上 19（触发落点 / 四读口 / `FundsVersion` + `funds_version` / `LoadFundsFact` 退役 / 走 05）逐条同向，判断项 ① 与 20 同向，无矛盾。裁决 3 / 4 ✓。`sameFundsFactVersion` 把回指算内容一维：同一版本两来源对「更正了谁」各说一套，正是裁决 1「真冲突」字面，不是扩张。判断项 ③ 不校验前版是否已到——**不该阻断**：校验即链连续性检查，正是红线一禁的「复制 SA 版本链」，且会拒掉「先到 v2 再到 v1」这一被 UC-CC-009 要求保留的格。判断项 ④ `DO $$ … RAISE EXCEPTION` 与 `0011` 同形。判断项 ⑤ 已核：`git grep` 全仓无任何非测试 SQL 再读 `customs_compliance.external_funds_fact` 被 0021 删掉的五列。dispatch 正例「v1 经 CC 写口预铺」未削弱「经信封」——v2 全程走信封，v1 走信封已由同测试第一格证过。
    **跑了**：`gofmt -l`（空）、`go build ./...`（0）、`go vet ./...`（0）、`go test -count=1 ./internal/customscompliance/... ./cmd/parcel-customs-register/... ./cmd/parcel-api/... ./cmd/parcel-dispatch/...`（全 ok，不带 DSN，真库例 skip）。**没跑**：任何带 DSN 用例、全仓、`internal/platform/migrate` 实施加（信通道 1 重放 tip 带 DSN 110 ok）。两轴由本会话串行独立过、未起子代理。只读，未改检出一字，未提交。
  - **推送方处置（16:0x，通道 1 接管会话）**：两轴 0 阻断，代码在 main 上留，14:0x 欠下的补评审至此兑清。**地盘补记（评审 Spec ① 所请）**：`git diff --name-only ace35ac3 8dbd49e2` 16 件中 `cmd/parcel-customs-register/duty_registers_test.go`（`fakeDutyBook.ListFundsFactVersions`）与 `internal/customscompliance/adapters/http/register_credential_and_duty_test.go`（`dutyStoreStub.ListFundsFactVersions`）在票面「地盘」字面之外——`ExternalFundsFactRegister` 接口加读口后编译强迫的替身桩，各只加一个方法、不改断言；作者会话（通道 1 前任）当时全网无他人在途，未占号，事后在此记账。其余非阻断的归宿：Standards 1（两包替身同形）不立，第三份出现时再抽；Standards 2（`HandleAdoptedExternalFundsFact` 未核 `content.Version` 与信封所指相等）与 Standards 3（`assemble_test.go` 正例头注「落一行」已旧）→ **候选尾巴**，与 lc/35 评审 Standards ① ② 同批 A 类合票；Spec 2（核对身份缺资金版本）→ 已追写进 [19](19-cc-new-funds-fact-version-forms-a-new-verification-version.md) Comments，19 立字时写成完成判据。12 评审的两张候选票已立：[21](21-cc-context-duty-reconciliation-rules-lack-the-explicit-record-sentence-cited-by-payer-code.md)（CONTEXT 补句 + 引文归属）、[22](22-cc-duty-verification-procedure-is-caller-asserted-and-not-recorded.md)（程序读口核一致 + 核对记录带程序；与 19 是同一张键的两维，票面已互指）。

...
- **评审 ← 通道 4 · 钉 `70176b0d`（代码 `e81e1240`；派单写的 `bedb2a24` 被作者 rebase 到 `807de571` 后 force-with-lease 覆盖，评审实评现行 tip，对照 `bedb2a24→70176b0d` / `4ea6a8b4→e81e1240`）· 基线 `807de571` · 2026-09-11 16:20（机时）**。只读、不带 DSN；子代理鉴权错，直读 diff。可跑部分：`gofmt -l` 空；`go vet` SA + `cmd/parcel-api` 退 0；SA domain / application / `adapters/identity` + `internal/architecture` 全 ok（含分区主体登记、前缀、信封、PBC08 五道门禁）；postgres / cmd 真库用例**未验**。越界核：`git diff --name-only 807de571` 零命中 `parcelpricing` / `form_supplier_expected_cost`。
  - **Standards**：**阻断 0**。**非阻断 3**：① `application/request_buy_evaluation.go` `NewRequestBuyEvaluationHandler` 四口拒 nil 用四句裸 `fmt.Errorf`，不包 `ErrNilDependency`（同包 `apply_pre_acceptance_control.go` 头注「构造门对缺件的唯一答复」；派单 Standards ④「形照 `NewApplyPreAcceptanceControlHandler`」）——拒了但调用方 `errors.Is` 不到；在 salvage 笔，建议改表驱动 + 包装。② `adapters/postgres/evaluation_request.go` `EvaluationRequests.Save` 不核 `record.Key.Request == record.Request.ID()` 也无零值门（CC `SaveVerification`「键与对象说的必须是同一件事」先例）；不一致的键会落成按 A 查内容是 B 的行，判断题级。③ `0019_evaluation_request.sql` `inserted_at DEFAULT now()` 与 `recorded_at` 并存而前者从不读回——Speculative Generality，SA 既有表若无此形可去。**无发现**：注释中文无行号；「四口都必备」是同文件自数不违「不数别处」；夹具 `syn-` / `SYN-` / `EVREQ-SYN`；自然键 `EvaluationRequestNaturalKeyOf` 种类 = 值排序后 SHA-256 带 `ESRC-1:` 前缀（PCC-1 形）；handoff 构造形、Envelope 字段与 `supplier_bill_handoff.go` 一致，`EnqueueOnce`；`adapters/identity` 与 CC / PS `New*(options ...platformidentity.Option)` 同形，前缀 EVREQ 进门禁；分区主体行已登 `partition_subject_registry_test.go`。
  - **Spec**：**阻断 0**。**非阻断 3**：① 判据 2「交接失败 → 技术未形成 + 续办引用」：实现 `handOff` 失败不翻结果（仍 `EvaluationRequested`）只留 `EvaluationRequestHandoffReference`，与票面「技术未形成」一词不字面对应；作者已在完成记录自报与 02 同形，接受但建议票面判据 2 措辞改成现行读法。② `docs/domain/CONTEXT-MAP.md` 新边「计价以 inbox 消费者接收…评价回指评价请求标识」写的是 11 落地后的形（PP 今天无 inbox），措辞与 SA / PP CONTEXT 不冲突，但建议括注「（PP 侧消费者见票 11）」免读成已接。③ `EvaluationRequestNotAccepted.String()` = `SOURCE_NOT_ACCEPTED`，但租户 / 请求方空白也走这一格，词形窄于格（Mysterious Name 级）。**判据逐项**：1 ✓ `RequestBuyEvaluationHandler` + `cmd/parcel-api/assemble_evaluation_request.go` `buildEvaluationRequestOrchestration` + `main.go` `run` 一格 fail-fast（真库往返未验）；「已装配未接线」与完成记录「装配点取哪条」一致，理由（三件引用交付方不存在、不种行）成立。2 ✓ application 用例：已登记 + 一封 / 同自然键 `EvaluationRequestExisting` 交回原 ID 不重铸（重发靠 `EnqueueOnce` 吞，合票面括注）/ 交接失败续办引用。3 ✓ 形上：载荷只 `{tenantId, evaluationRequestId}`，分区键 `<租户>/evaluation-request/<ID>`，ID = 分区键 + `/submitted`，主体名已进中心登记表；`evaluation_request_handoff_test.go` 真库未验。4 等 11，票面已写明。5 ✓ 清点 `7dfba37d` + CONTEXT-MAP 边。红线：三件引用全由 `RequestBuyEvaluationCommand` 交进不推 ✓；金额 / 币种 / 换算零列零字段 ✓；PP 零改 ✓。
  - **判断题**：① 读侧摘要比对：留——它是「库上唯一约束比字节、编排按重算键找」两口自相矛盾的唯一响亮点，代价只是 ESRC 换号时旧行必须迁（本就该）。② identity 新包放 `adapters/identity/`：对，与 CC / PS / VE 同形，随机铸造不落库与「先铸再登」的编排顺序匹配。③ 分区主体「租户 / 评价请求」非逐信封：对，ADR-0069 决定二「ID 管幂等、分区键管顺序」，为日后第二拍留序。④ 装配壳整个 Handle 一笔事务：对，裁决 1「同事务」的前提，「登记已落意图未入队」只在替身可达，与 02 同形。
  - **结论**：两轴 0 阻断（Standards 3 / Spec 3 非阻断），可进 main；非阻断 ①（`ErrNilDependency`）建议进 main 前或随后票一笔收。检出已拆。
- **进 main 记录（通道 1 推送方，2026-09-11 16:3x；与 sa-cc/04 同批）**：`%TEMP%\idp-replay-w5a` detached `807de571`（= 远端 main），先叠 04 五笔再 `cherry-pick` 本票六笔（作者已 rebase 到 `807de571` 的那一组；跳过作者清点 `7dfba37d`），零冲突；本票文件对分支 tip 零差；与 04 代码零重叠。SHA 对照：`c8e07b5f→ae0cfd24` / `d93d208c→8b1ac98f` / `352dfcfe→c9aae85f` / `d09aed17→8d75bfc7` / `e81e1240→0459dfdf` / `70176b0d→e0eb667f`；两票清点由推送方在栈 tip 重生成一笔 **`dcb74785`**（SA 83→89、迁移 18→19、outbox 交接 8→9、端口 391→397；CC 一并兑）。**验证钉 `dcb74785`**：`gofmt -l` 空；`go build ./...` / `go vet ./...` 退 0；16:17 占号，带 DSN `go test -p 1 -count=1 ./...` **108 ok / 0 FAIL / 15 无测试 / 0 cached**（16:17:14→16:19:23，129 s；包数 107→108 是本票新包 `adapters/identity`）；`-v` 探针 `-run 'CredentialGate|EvaluationRequest'` PASS 21 / SKIP 0（评审侧「未验」的 postgres / handoff / `cmd/parcel-api` 真库用例在此实跑）；16:19 释号。簿记一笔在其上；`ls-remote` 核 `807de571` 未动 → `merge --ff-only` → `push <sha>:main`。分支 `mcp6-sacc08@70176b0d` 作封存出处、改名 `merged/`、远端删；`D:/tops/idp-parcel-mcp6-sacc08` 比内容后拆。**推送方对评审的处置**：两轴 0 阻断；六条非阻断随票记，不挡合入——Standards ①（`ErrNilDependency` 包装）与 ②（`Save` 核键与对象一致）**归 SA owner 一笔小票或随下一张 SA 票收**（推送方派 SA 下一票时写进派单）；③ `inserted_at` 归同笔；Spec ①（判据 2 措辞）本条记为「技术未形成 = 结果不翻、只留续办引用，与 02 同形」，票面判据句不改；② CONTEXT-MAP 括注归 11 落地那笔；③ 词形窄于格记为已知。**下游**：票 11 硬阻已解（仍 draft 等 PP owner 裁入口形状）；判据 4 等 11。

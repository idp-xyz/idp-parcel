...
| [09](issues/09-sa-consumes-duty-payment-verification-envelope-into-advance-recovery.md) | SA inbox 消费付款核对信封 → `AssessAdvanceRecoveryHandler`（05 裁「本目录加一张」）——**resolved（作者完工，2026-09-11 16:1x）**：接为 UC-SA-001 步 2「结算输入采用」一格（`AdoptDutyPaymentVerification`，登记册只存核对版本引用 + 采用时刻，`AdvanceOutcome` 不动；票面做法 1「译成 AssessAdvanceCommand 交 Assess」在代码上走不通，通道 1 15:5x 裁改口）；分支 `mcp5-sacc09` 代码 tip `17167aa8`、注释笔 `cb04084e`、清点 `11da0858`；等非作者评审与推送方重放进 main | SA | 无（05 已进 main） |

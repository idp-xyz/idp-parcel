...
Category: chore
Status: ready-for-agent——产品已裁「补」，**拆两笔**（2026-09-02，owner）
...
>
> **第一笔已完成（`85c1c7f`）**：封闭集增至十值、`valid()` 与 `String()` 已跟上、迁移 `0019`
> 把 `object_kind` CHECK 放宽到 1..10。**只拓宽不接线**——领域类型与正文表属票 `05`，别把
> 「进了封闭集」读成「能用了」。
>

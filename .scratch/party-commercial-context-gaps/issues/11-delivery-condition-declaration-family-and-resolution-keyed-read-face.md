...
- **评审 ← 通道 1（推送方自跑 /code-review，非作者）· 钉 `3e513732`（基 `d9a1f571`，代码 tip `7f18b156`）· 13:4x**。通道 3 / 4 仍未重起、2 / 6 在票上、5 是作者，按 09-08 规矩推送方自跑；两轴串行、互不引用。**Standards**：阻断 0；非阻断 1——`CommercialPublications.SaveDeliveryConditions` 以 error 表达「产品层不在册 / 放宽」的业务拒件（判断题 1）：ADR-0031 给具名 Save 的是重放 / 冲突 / 已存三格算术，这是仓内第一处第四种落点；作者理由成立（核要产品层在手、用例刻意不读库、替代路要给 `PublicationRegistry` 加读口 + 三处替身），**归 owner 复核**——若认可，ADR-0031 补一句「输入对着册上另一份立不住时 Save 返回 error，与构造门拒件同落点」或立 supersede；不挡合入。核过：注释全中文；跨文件引用皆有标签（ADR-0133 决定二 / 四、ADR-0080 决定四、ADR-0062 决定三、ADR-0120 决定七、open-decisions D-4），无行号 / 计数 / 序位；读写两条 SQL 均带 `tenant_id = $1`（ADR-0003），`requireOwnedVersion` 再核一次租户；迁移 0030 LF 无 BOM，`object_kind IN (1, 2)` 与按层钉 `tightens_*` 两列的 CHECK 都在，子表 FK ON DELETE CASCADE；读回经 `DeclareProductDeliveryConditions` / `DeclareContractDeliveryConditions` 构造门、坏行 error 不折「没有」（ADR-0028 的 PC 形，同 0007 / 0012 / 0027）；`TightensWithin` 先核「正是所指名那一版（同租户同对象同版本）」再核子集，产品层调它答类别错误；smell baseline 无可报项（`loadDeliveryConditionLayer` 父 LEFT JOIN 子一次取回、`sameDeliveryConditions` 逐格比稳定序方式集合，都是最直的写法）。**Spec**：阻断 0；非阻断 1——「服务产品版本未被采用」不单列一格（判断题 5）：ADR-0133 决定二第一格字面是「闭包在场且采用了客户合同版本与服务产品版本、其上有交付条件声明 → 引用」，实现里产品未采用而合同层有声明也答「有」；ADR 只为「未采用合同」立了 error 哨兵、没为「未采用产品」立，作者读作「那一层只是没有可读的声明」，**归 owner 复核**（若要分格，`DeliveryConditionReferenceFor` 加一哨兵 + 读口头注一句，形不动）；不挡合入。要建什么 ①–⑤ 逐项对上：① 两层同形、正文三格、合同层只许产品层已有方式（`ErrDeliveryConditionWidened`）、无默认不内置「本人签收」、**开放引用**（理由在完成记录，与 PAR-NET-09 实例半边一致，CONTEXT / GLOSSARY 因此不动——认可）；② `pcports.DeliveryConditionView.LoadDeliveryConditionReference(ctx, tenant, ResolutionID) → (DeliveryConditionReference, found, err)` 四格、只答有没有、引用即回指本身（ADR-0133 决定一）；③ `SaveDeliveryConditions` 先读回再判重放 / 冲突、`DeliveryConditionChannel` 一格、批文 `deliveryConditions` 一节、三处替身跟随、`declarationsDocument` 一节；④ 迁移 0030 父表带层级列 + 子表方式行；⑤ CONTEXT 未动（开放引用）。完成判据 1–4 逐项 ✓（领域五例含放宽 / 冒名 / 零方式；读口四格领域一例 + 真库六例含跨租户与未采用合同；端到端一例批文 → 0030 → 固定闭包 → 读口答引用；带 DSN 23 包 ok）。红线三条 ✓：夹具全合成串、读口不认识包裹、三份 ADR 正文未动。判断题 2（合同层必指名 `tightens`）/ 3（两条规则引用两层必填）/ 4（未折进 PCC-1，归 awf 票同笔）/ 6（读口今天无进程装配，tf/14 接线那天装）/ 7（一父表带层级列 + 子表）认可。**给 tf/14 的两句**（写进派单）：集运单元在 TF 适配器直接答 `RequirementMissing` 不调 PS（psr/07 评审那条）；`pcpostgres.NewDeliveryConditions(db)` 由 tf/14 在 `cmd/parcel-api` 装配。**结论：两轴无阻断，进 main。**
- **进 main 记录（推送方 · 通道 1 · 13:4x）**：隔离树 `%TEMP%\idp-replay-pc11` 在 `c0cdebba`（= 当时 origin/main，psr/07 刚进）之上 cherry-pick 六笔：`d9a1f571→d656b99b` / `582e9f12→545b2483` / `d7bd4132→e0edc701` / `697c9dc5→1ad8d81f` / `7f18b156→08084300` / `3e513732→92a059dc`（第一遍漏了基底那笔 `d9a1f571`，票面笔冲突，重置后按序重来——冲突成因是漏 pick，不是内容）；作者清点笔 `aff5537d` 与 main 同文件冲突，**跳过**，推送方在 tip 重生成为 `fce1faa9`（单独一笔）。本票十六件与分支逐字相同。`fce1faa9` 上 `gofmt -l` 零、`go build ./...` / `go vet ./...` 0、`./internal/architecture/...` ok、迁移 0030 LF 无 BOM；占 55432 广播后**带 DSN** `go test -p 1 -count=1 -v ./...`：102 ok / 0 FAIL / 15 无测试，`--- PASS` 7939 / `--- SKIP` 1（仅 `pgtest.TestHelperTemplateOwnerProcess`）/ `--- FAIL` 0 / 0 cached，122 s，探针 PASS，释号广播。本条 Comments 随推送方簿记一笔落在 `fce1faa9` 之上，ff 后 `push <sha>:main`，SHA 在推后广播里。tf/14 的两个 Blocked by（本票 + psr/07）至此都在 main。

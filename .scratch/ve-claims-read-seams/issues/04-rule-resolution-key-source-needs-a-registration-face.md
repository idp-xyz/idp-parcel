...
- **2026-09-11 23:3x · 通道 1 推送方中途裁（通道 3 中途报，A 类，测试夹具路径）**：PS 解析键登记面在 `262e8c0a` 上**不收** `CUSTOMER_SERVICE_RULE`——迁移 `0007` 立、`0008` 重加的 CHECK `commercial_resolution_key_registration_bases_closed` 白名单无此格，PS `adapters/partycommercial/commercial_resolution_keys.go` `commercialKindFrom` 名集亦无（通道 1 复核：`git grep CUSTOMER_SERVICE_RULE -- migrations/parcel_shipment internal/parcelshipment/adapters/partycommercial` 零命中）。ADR-0136 越权风险点 2 的前提「可登、只是不必登」不成立。**裁 (a)**：「已登记」态的闭包由测试直给 PC（重建门 / 真解析器）造、经 PC 真解析库落，PS 真委托仓储 Decide → Save 一份接受委托、决定上的回指指向它，VE 读路径三段全真库；「未采用客户服务规则 → 未登记」态真走 PS 登记面（只含客户合同，白名单收）；判据 3 原句划掉留痕改口如上；PS 两道闸 + 「是否必登」另立 [ps-port-remainder/09](../../ps-port-remainder/issues/09-resolution-key-face-does-not-accept-customer-service-rule.md)（23:32 进 main `aa2e6a07`，ready）；ADR-0136 本票不改，越权风险点 2 要补一句 `262e8c0a` 实测归 owner（候选后继）。不取 (b) 挂起：机制半边（VE 读回指 → 闭包 → 正文）与 PS 开闸互不依赖，挂起只是把已验的机制压在别人的票后面。
...
3. 装配测试对真库钉两态，~~且 SYN 解析键那一行的必需依据含 `CustomerServiceRuleObject` 与 `CustomerContractObject`~~ **改口（2026-09-11 23:3x 通道 1 推送方中途裁 (a)，见「裁决」末条）：且「已登记」态的闭包采用依据含 `CustomerServiceRuleObject` 与 `CustomerContractObject`——键直给 PC 解析 / 重建门造闭包、经 PC 真解析库落，不经 PS 解析键登记面；原句在 `262e8c0a` 上做不到，PS 登记面两道闸不收 `CUSTOMER_SERVICE_RULE`，归 [ps-port-remainder/09](../../ps-port-remainder/issues/09-resolution-key-face-does-not-accept-customer-service-rule.md)；「未采用客户服务规则 → 未登记」那一态真走 PS 登记面（只含客户合同）**；生产装配不变那一格照旧。

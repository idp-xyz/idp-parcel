...
- **评审 ← 通道 1（推送方自跑 /code-review，非作者）· 钉 `3df82d62`（基 `84e89dc7`，代码 tip `f66af0d0`）· 15:0x**。通道 3 / 4 仍未重起、2 / 5 在票上、6 是作者，按 09-08 规矩推送方自跑；两轴串行、互不引用。**Standards**：阻断 0；非阻断 1——迁移 0021 受限项两列 CHECK 写成「同在或同缺」而非票面字面「受限行两列非空」（判断题 7）：与同一句「存量行 NULL 如实读回、不补不拒」只能这样同时成立，领域 `AdoptedDisposition()` 对同缺答缺席、`AwaitsAuthorizedDisposition` 对未采用答否 → 按 ADR-0125 过渡口径译`未通过`——认可，记一句是因为票面字面与落地形不同，后来者读票面别以为 CHECK 漏了。核过：注释全中文；跨文件引用皆有标签（ADR-0132 决定一 / 二 / 三 / 四、ADR-0125、ADR-0094 决定四、ADR-0086 决定一、ADR-0106、`f822e1d` 三值逻辑纪律），无行号 / 计数 / 序位；**UC-PS-001 那句「四者对应……四个等待态」改成「各自对应……一个等待态」正是 AGENTS.md 禁计数那条的落地**；0021 LF 无 BOM，`resume_path` / `task_waiting_on` 两条 CHECK 与领域 `ResumePath` 封闭集对齐第五格、部分索引带 `tenant_id`、`failure_disposition` 只认两值；队列读口 SQL 带 `tenant_id = $1`（ADR-0003）；重建门收处置记录并拒拼出来的行（ADR-0028）；`NewAdvanceFinancialControlJudgmentHandler` 加参的六处调用点同笔、开窗通知 12:3x 广播过；两份基线文件未动而 `./internal/architecture/...` 在重放 tip 绿。smell：四个 `Disposition*Reference` 类型与主动拒绝那一组同形不复用（Duplicated Code 的判断题）——作者理由「处置权与拒绝权在类型上分得开」成立，`formActiveRejection` 处逐格包 `requiredValue` 转成拒绝留痕是包内一处，接受。**Spec**：阻断 0；非阻断 1——做法第 2 步「同目录另加处置授权的翻译适配器」未建、只建 `UnconfiguredAuthorizedDispositionAuthorizer`（判断题 4）：PC 授权动作封闭集今天没有「授权处置」一格（ADR-0132 越权风险点 2），翻译适配器的全部意义在动作守卫上而守卫要比对的词不存在，照 `UnconfiguredSourceDataAmendmentAuthorizer` 先例——认可；**PC 那一票落词那天同笔换成翻译适配器**，本记录点名。裁决四问逐条对上：第 1 问 `AuthorizedDispositionChoice` 封闭两值 `REJECT` / `CUSTOMER_SUPPLEMENT`，放行不在集内，`拒绝`复用 `formActiveRejection`、`交客户补充`只转 `waitingOn`，两去向都在命令事务里按 `OccupationFormed` 释放（`releaseOccupation`）；第 2 问独立等待态 `ResumeByAuthorizedDisposition`，`undecidedDisposition` 加格 → 提交入账，保存护栏照 ADR-0086 决定一，不新增信封；第 3 问在 `AdvanceFinancialControlJudgmentHandler` 读处置采用到受限项再记录，适配器走回指 → 闭包 → 已采用结算政策 `ChargeScope` → 控制策略版本 → `ItemsFor` 与 SA 同源，对不上 `CONTROL_DISPOSITION_NOT_FORMED` 停内部续办不折去向（判断题 8 认可：SA 刚按同一份正文执行完，读不到只可能是换版竞争或坏数据）；第 4 问责任引用随受限项作采用引用落库、队列读面透出、拒绝决定 Basis 缺席经采用结果回指（判断题 6 认可）。硬句守住：`TestADispositionRecordNeverPassesTheFinancialControlGroup` 钉「处置记录从不让接受前财务控制那一组通过」，`FinancialControlCheckFor` 的`无法判定`说的是接受判断待去向、`业务限制`原样保留在采用结果上。`Decide` 次序（判断题 1：客户补充 > 授权处置 > 运营登记 > 内部重试）认可——处置的一个去向本就是交客户补充；判断题 2（已处置版本再判留在`等待受控补充`）认可——一版一次；判断题 3 / 5 / 9 / 10 / 11 / 12 认可，无补充。完成判据逐项 ✓（含 `task_waiting_on = 5` 可查、入账不重投、两去向闭环真库场景 1 / 3、存量 NULL 读回、未授权不碰聚合、`AUTHORITY_RULES_NOT_CONFIGURED`）。红线 ✓：SA 一格未动、PC 正文形状未动、`apps/` 未动、ADR-0132 / 0094 / 0086 正文未动（ADR-0125 只加 owner 复核记录一条）、无真实合同 / 策略取值。**结论：两轴无阻断，进 main。**
- **进 main（推送方 · 通道 1 · 15:0x）**：隔离树 `%TEMP%\idp-replay-sa04` 在 `62c87e73`（= 当时 origin/main，pc-gaps/11 刚进）之上 cherry-pick 九笔全干净：`e4883039→6a5535e2` / `8c05797e→3c1f54bf` / `ca2c44e3→0ce6461e` / `f686e4bd→a974e6b3` / `33fbfb6d→b995f683` / `1aa5b6e3→1e589f66` / `5f34f67b→f1644897` / `f66af0d0→1f45af10` / `3df82d62→779f841e`；作者清点笔 `99ddc8f5` 与 main 同文件冲突，**跳过**，推送方在 tip 重生成为 `fb38e66a`（单独一笔）。本票 56 件与分支逐字相同。`fb38e66a` 上 `gofmt -l` 零、`go build ./...` / `go vet ./...` 0、`./internal/architecture/...` ok、0021 LF 无 BOM；占 55432 广播后**带 DSN** `go test -p 1 -count=1 -v ./...`：102 ok / 0 FAIL / 15 无测试，`--- PASS` 8011 / `--- SKIP` 1（仅 `pgtest.TestHelperTemplateOwnerProcess`）/ `--- FAIL` 0 / 0 cached，125 s，探针 PASS，释号广播。本节随推送方簿记一笔落在 `fb38e66a` 之上，ff 后 `push <sha>:main`，SHA 在推后广播里。tf/12（通道 2）与 tf/13（通道 5）都要动 `cmd/parcel-api` 接线，本票先进，它们推前对着新 main 干跑。

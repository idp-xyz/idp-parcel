...
| [05](./issues/05-estimate-endpoint-backend.md) | 试算后端：编排、补齐读数共用件与 `POST /pricing-estimates` | 走并行会话那条路 | — | resolved |
| [06](./issues/06-estimate-admin-page.md) | 管理台试算页 | 前端切片 | 05 | resolved |
| [03](./issues/03-customer-account-onboarding-readiness.md) | 开户就绪清单 | 先建模：按客户账户做一次不落账的解析 | product-strategy-boundary/17 | draft |
| [04](./issues/04-price-card-template-import-per-adr-0101.md) | 落地 ADR-0101 价卡导入 | 已拆为 [`price-card-import`](../price-card-import/spec.md) 子票 01–06 | — | in-progress（跟踪容器） |

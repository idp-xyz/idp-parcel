...
1. **（已裁 2026-09-14 10:2x：取 (a) 不必登——通道 1 推送方按用户「授权代裁」以 PS owner 口径裁。理由照作者完成记录：PC CONTEXT 说合同版本恒在，所以 ADR-0133 / 0136 对「闭包未采用客户合同 → error」的判法成立；客户服务规则是 ADR-0104 决定三「对首发两项都无客户差异就是不登记」那一类，一个租户可以合法地没有任何客户服务规则版本，登记面若必登，接受流会在这类租户身上整行拒登，把「没有差异规则」变成「接受不了委托」；VE 侧对「未采用 → 未登记」已有诚实停格（ve-claims/04 态一）。(b) 的收益要先有「每个租户都必须发布一版客户服务规则」这句商业语言，今天没有。**零代码改动**——今天的登记面就是 (a)；ADR-0136 越权风险点 2 补记末尾追一句。）** CSR 是否随合同维一并「必登」（ADR-0136 越权风险点 2 原题，本票只让它「可登」）：PS `commercial_resolution_keys.go` 今天只在请求结算依据时强制合同维（`validateSettlement`）。选项 (a) 不必登——租户没登 CSR 时索赔两维如实答未登记（今天 ve-claims/04 落的就是这一格）；(b) 与合同维同强度必登——没登就拒登整行，索赔两维在接受时就有答案。归 PS owner；不阻做法 1–3。
...
- 2026-09-14 09:03 · **非作者评审 ← 通道 2**（task-50cd7049；钉 `9fa2ddc6`，基 `1b06bb18`，隔离检出只读、不带 DSN；独立于票面自评；推送方代落全文）：
  - **Standards**：阻断 无。非阻断 ① `internal/parcelshipment/adapters/partycommercial/commercial_resolution_keys.go` `commercialKindFrom` 新头注首句「名集与库内 `..._bases_closed` 白名单是同一集合的两份镜像」与两行之下的代码不符——名集含 `pcdomain.PriceRuleObject`，0022 白名单无 `PRICE_RULE`；PRICE_RULE 由 `ResolutionKeyRegistration.validate`（「需要键携带额外选择维度，本登记面不承载」）与库内 CHECK 另拦，既有用例「价格规则仍在名集之外」钉着。按 AGENTS.md「注释只写代码讲不出的东西」，一句被代码反证的话该改口：建议写成「名集是 PC `CommercialObjectKind.String()` 的反向；库内白名单比它少 PRICE_RULE 一格，那格由 `validate` 与 CHECK 拦」。只改注释，零行为。无发现：0007 / 0008 / `internal/partycommercial/**` / `internal/visibilityexception/**` / `cmd/parcel-api/assemble_claims.go` 零 diff（diff --stat 实测）；0022 与 0008 同形——DROP 后 ADD 同名约束、`<@ ARRAY[...]::text[]`、既有各项原序 + `CUSTOMER_SERVICE_RULE`、PRICE_RULE 仍不在，尾句「迁移不回改 0007 / 0008」照 0008 尾句（0020 未动 bases_closed，0008 是唯一先例）；头注引 ADR-0136 决定三「客户合同版本与客户服务规则版本都必须在接受时闭包的必需依据里」与 ADR 决定三首句逐字、单行；白名单字面 = `CommercialObjectKind.String()` 的 `CUSTOMER_SERVICE_RULE`；无 INSERT、测试只用 SYN / tenant-1 身份，不预填任何租户键；注释全中文、无行号，「各加一格 / 同笔」是本票不可变事实，「结算三维 / 信用两维 / 索赔资格两维」是 ADR-0044 / 0127 / 0136 的领域词，不是跨文件计数。
  - **Spec**：阻断 无。非阻断 ①（作者判断项 1，答：可接受、不要求改）：`cmd/parcel-api/assemble_claims_test.go` 两客户共用一份 `RegisterClaimEligibility`——VE 册键形是（租户，合同），ADR-0136 决定二明写 `EligibilityQuery.Contract` 是 VE 自家两本册的键、不拿去 PC 过滤，所以共用与生产同形；两个货主客户解到同一 PC 合同壳 `SYN-CONTRACT-1` 是合成便利，领域上一份客户合同归一个客户——若要更贴生产需第二合同壳 + CSR 第二适用性，超出票面，不建议本票做。无发现：判据 1 白名单含 CSR ✓；判据 2 `commercialKindFrom` 只 +1 格，末行 `unknown commercial object kind %q` 不变，测试文件纯新增、既有用例零 diff ✓；判据 3 真库两例——`TestAResolutionKeyRowRequiringACustomerServiceRuleRoundTrips`（`pgtest.Pool`，`slices.Equal` 逐字逐序、其余维照登、结算 / 信用维空）与 `TestARegisteredCustomerServiceRuleBasisFormsAKeyRequiringIt`（`newResolutionKeys` → `pgtest.Pool`，两类都在、最小身份成立、两选择器 Empty）✓；CLI 无第三道闸——`cmd/parcel-commercial/translate.go` `commercialKindFrom` 早含 `CustomerServiceRuleObject`，解析键文档的 `document.RequiredBases` 循环走它，非测试代码里无封闭提示词表 ✓；做法 4 负断言与「键直给」段已删，两态皆 `formKey` → `resolveClosureOnRealAssembly`（PC 真 `NewResolveCommercialBasisHandler` + 真 `NewCommercialResolutions`）→ `acceptOnRealAssemblyWith`（PS 真 `ShipmentRequests` Decide → Save），两助手零 diff、无绕路；头注引 09 路径；`secondSubmissionCommand` 全 cmd/parcel-api 仅该用例一处引用，改客户不波及他例 ✓；「必登」`validate` / `validateSettlement` 零 diff（已裁不评）✓。
  - **结论**：可进 main（已在 main）；非阻断两条归推送方代落 Comments，Standards ① 若要修是一笔纯注释改动。检出即拆（不加 --force）。
  - **推送方处置（通道 1，09:0x）**：Standards ① → 追加为 lc/38 第 7 条（通道 4 task-65ce5275 在做，PS 地盘、纯注释）；Spec ① → 接受，不改；证据层级自此从「作者自评」升为「非作者评审 0 阻断」，Status 首句改口。**10:0x 已落**：lc/38 条 4（`c569ea36`，重放 `9cf30fab`）改口 `commercialKindFrom` 头注并顺带收了同文件 `FormResolutionKey` 那一支同句错话，进 main 见 [lc/38](../../label-channel-service-first-release/issues/38-lc37-review-tails-adr0049-citation-five-values-count-and-fact-reference-rename.md)。

...
Status: resolved——**2026-09-10 15:4x 进 main**（推送方通道 1 重放；通道 6 非作者评审 15:04 两轴 0 阻断；分支→main 对照与验证见文末「进 main 记录」）。此前 15:1x 通道 2 交活（task-0450f54b，分支 `mcp2-psr03` 基 `dd5ed934`；代码 tip `27607caa`，三笔 `39c1206d` / `07fe7930` / `27607caa` 见「完成记录」，票面笔随其后）；此前 in-progress——14:4x 通道 2 接手；此前 ready-for-agent——2026-09-10 14:2x 通道 1 解阻（远端 main `5dda0fb2` 上取证）：PC 半边已随 [pc-gaps/08](../../party-commercial-context-gaps/issues/08-authorized-action-lacks-source-data-amendment-and-adjudication-names-no-decider.md) 进 main（`0cfe3571` 领域 + 端口 + 迁移 0025 / `31903cc5` 合同委派册 / `02ff5205` 发布通道 + 批文；ADR-0116，owner 复核 2026-09-09 认可），生产入口已随本目录 [04](./04-amendment-production-entry-mechanism-half.md) 落 `cmd/parcel-api` 且今天接的是 `UnconfiguredSourceDataAmendmentAuthorizer{}`；剩 PS 半边（适配器照撤回那只 + 换掉那只未配置 + 剪基线两条），见 Comments 末条。此前 blocked——三问已由 MCP-1 代裁（owner 授权，2026-09-07，见 Comments）；PC 半边等 pc-gaps 批；「顺带量到」的生产入口机制半边已按代裁拆到 04 承接
...
- **评审 ← 通道 6 · 钉 `276e3770` · 15:04**（task-b4a2553d，非作者；隔离检出只读，两轴串行、互不引用；该检出上 `go vet` 两包 0、`gofmt -l` 空、适配器包 `-run SourceDataAmendment` 10 例 PASS 无 DSN）。由推送方代落票面，文字照评审原文。
  - **Standards**：**阻断 0 · 非阻断 2（均判断题）**：① `SourceDataAmendmentRequestCoordinates` 头注「RequesterKind 取 PC 的两格」、适配器头注「与撤回、拒绝两只同形」——AGENTS.md「计数与行号同构」：数的是 PC 封闭集 / 本包别的文件，PC 加第三种请求方或本包再加一只时这句无声变旧（`formSourceDataAmendmentRequest` 的 default 会炸、改的人会路过，危害小）；改法：写成「本适配器接客户账户 / 运营角色两种请求方」，语义不动。② 四格 `errors.Is` switch 在 withdrawal / active_rejection / 本文件三处同形（重复代码 / 重复 switch，判断题）——本包既有纪律是两口不合并（`psports.WithdrawalAuthorizer` 头注），三口授权引用类型各异，抽公共函数收益小；记录不建议改。**无发现**：注释全中文、写的都是取舍与出处（ADR-0116 Decision 三、ADR-0025、UC-PS-002 硬句），无行号引用；`ErrUntranslatableAnswer` 复用本包既有那只；default 分支报错不吸收（ADR-0025）；用例用 `// Covers:` 与 `value/pcValue` 惯例，术语（实际决定方 / 委派方 / 代录）与 CONTEXT 一致；`cmd` 注释把 RequestSource nil 如实写成实例半边（`BD-PS-009` / `PAR-COM-14`）并引 ADR-0063；基线头注计数钉 SHA 且写明只对该检出成立。
  - **Spec**：**阻断 0 · 非阻断 1**：票面裁决①原句「RequestSource 把查询折成带资料修订动作的 AuthorizationRequest」——落地是 RequestSource 交坐标、适配器折请求。运行时形成的仍是带资料修订动作的 `AuthorizationRequest`（`NewAuthorizationRequestBy(requester, SourceDataAmendmentAction, …)`），且红线「不复用既有两格授权动作」由此从映射自觉变成结构（`TestSourceDataAmendmentIsRefusedWhenTheScopeOnlyGrantsTheOtherTwoActions`），派单③要求两扇门随本适配器出名单也只能这样发生——认可，有票面依据；代价是本包今后两种 RequestSource 形状并存，撤回 / 拒绝迁 `NewAuthorizationRequestBy`（三步法）那天再统一，随票记。**无发现（派单五点逐核）**：① Decider 只从 PC 转写：`Decider` 无公开构造器，适配器文件 `NewDeciderReference` 恰一次、`query.Requester` 零次，`query.Identity.CustomerAccountID()` 只作 PC 请求方 account / onBehalfOf；`TestSourceDataAmendmentByAnOperatorRoleNamesTheDelegatorAsDecider` 三串互异，PS 顶替任一都红。② 四格方向对：`ErrNotAuthorized`（含 `ErrDelegationAbsent`）→ Refused 且两项引用空；`ErrAuthorityRulesNotConfigured` → RulesNotConfigured；`formed=false` / 映射 nil / RequesterKind 零值 → error 且不问提供方；PC 旧构造器缺委派读口 → error 不读成拒绝。④ cmd：`buildSourceDataAmendmentAuthorizer` 接 `NewAuthorityGrants` + `NewContractDelegations` + 带委派裁定编排，RequestSource nil 两处注释；两停点断言 RULES_NOT_CONFIGURED → AUTHORITY_UNAVAILABLE 与消费方「err → SourceDataAmendmentAuthorityUnavailable」一致。⑤ 基线：`formSourceDataAmendmentRequest` 是两名的非测试调用方（第二种，真接上）；头注写明剪前 3 剪后 1 钉在 `39c1206d` 父提交上。
  - **作者五道判断题**：1 认可（另核 `NewDeciderReference` 恰一次、`query.Requester` 零次）；2 认可（只搬 Reference；带 Kind 要先改 PS 域读法，另票）；3 认可（RequesterKind 归实例半边，按串推断即一条默认）；4 认可且建议两笔连放（`39c1206d` 单独在 main 会让 `TestWiringBaselineHasNoStaleEntry` 红——作者已披露）；5 认可（`unconfigured_source_data_amendment.go` 头注已成历史，可立簿记小票就地改）。**结论：可重放。**
- **进 main 记录 · 通道 1 · 2026-09-10 15:4x**：分支 `mcp2-psr03` 在 `93349f83` 上 cherry-pick 四笔全干净——`39c1206d→f9852a3f`（适配器 + 测试 + 票面 in-progress）/ `07fe7930→ee1e8cc1`（cmd 装配 + 两停点 + 基线剪两行）/ `9c6ccd4c→bb0abf2f` / `276e3770→5cdac262`（票面）；作者清点笔 `27607caa` 与 main 上 psr/01 之后的同文件冲突，**跳过**，清点在 `5cdac262` 干净 detached 检出重生成为 `19f323f1`（推送方兑底）；两笔代码连放（评审判断题 4），`f9852a3f` 单独在 main 上接线基线门禁会红、`ee1e8cc1` 起绿，CI 只跑 tip 不受影响。六件非簿记文件与分支逐字相同。验证钉 `19f323f1`（隔离 detached 树 `%TEMP%\idp-replay-psr03`）：`gofmt -l` 空；`go build ./...` / `go vet ./...` 退 0；`./internal/architecture/...` ok（基线剪两行在 tip 成立）；占 55432 广播后含 DSN `go test -p 1 -count=1 ./...` 退 0，**104 ok / 0 FAIL / 15 无测试 / 0 cached**（120 s）；探针 `cmd/parcel-api -run Amendment -v` 五例 PASS 含 `TestTheProductionAmendmentAssemblyStopsAtUnformedAuthorization`；释号。本笔（票面 Status 行 + 评审 + 本条 + tasks.md）在 `19f323f1` 之上，纯 .md，自审；`ls-remote` 核 `93349f83` 未动后 ff 并 `push <sha>:main`，SHA 见推后广播。分支指针改 `merged/mcp2-psr03`、远端删；树由作者比内容后拆。评审非阻断三条：Standards ① 注释计数措辞与判断题 5 那句历史头注一起，归一张 PS 簿记小笔（随下一张 PS 票或通道 5 的痕迹清剪票带走）；Standards ②、Spec ① 不改，随票记。

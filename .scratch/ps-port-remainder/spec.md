...
Category: chore
Status: in-progress——十问已由 MCP-1 代裁（owner 授权，2026-09-07，逐票 Comments）：01、03 转 blocked 等 PC 半边（pc-gaps 批，MCP-3）；02 的 PS 半边中不依赖 PC 的那段与新拆出的 04（资料修订生产入口机制半边）由通道 2 按 task-b77525c9 在分支 `mcp2-ps-ports` 实施中。此前 draft（通道 2 于 2026-09-07 按 task-0c472fed 立三票：每口一次 `/domain-modeling`，机制半边的头一半都在 PC，PS 单方面无可落代码）
...
| [04](issues/04-amendment-production-entry-mechanism-half.md) | （入口，不是端口） | 资料修订编排的生产入口机制半边：端点 + `UnconfiguredIntake{}` + 两层边界壳，接上后编排如实停在授权未决 / 矩阵未登记 | PS（通道 2 在做） |

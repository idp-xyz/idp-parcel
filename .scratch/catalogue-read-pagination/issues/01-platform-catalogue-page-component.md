...
### 评审 ← 通道 3 · 钉 `f23f16e6`（基 `51d2548a`，只读，门禁未重跑）· 2026-09-24 15:1x（推送方自通道 3 来信代落原文）

引通道 2 在 `f23f16e6` 实跑：cataloguepage 92 PASS 含 -race、architecture ok、全仓 build / vet 退 0。评审人不是代码作者，是 ADR-0144 起草人。

**Standards** — 阻断：无。无发现：注释全中文、引 ADR 决定号不引行号、不数别处；只导入标准库，包内无任何上下文词（ADR-0144 决定七）。

**Spec** — 阻断：无。无发现（实核）：决定一——`after` 不透明（base64url JSON）、内含排序 / 末行值 / 多段标识 / 摘要，解不开或摘要、排序不符一律 `staleCursorReason`（即决定一原句），只向后；决定二——包内无页大小参数，`NewPage` 的 size 由调用方从 Intake 拿；决定三——`sort` 单维、`-` 倒序、集外即拒、多给即拒，缺省序由各册 `Spec.DefaultSort` 声明（`-registeredAt` 的缺省落在票 02 / 03）；决定四——筛选维名即声明名、重复值规整为维内或、词表外即拒、选择器不兑筛选维且与保留键同名即装配报错、`q` 去首尾空白且空即缺席、超 `MaxKeywordRunes` 即拒、未知键即拒；决定五——`Page{Size, Next *string, Total *int64}`，json 名对得上。

判断项裁定（以 ADR 起草人身份判）：③ 摘要多覆盖册名与选择器取值**不越出决定一**——决定一的理由是「换了条件还拿旧游标，翻出来的是另一份列表的中段」，册名与 family 正是「哪一份列表」，覆盖它们是这条理由的本意，ADR 不必改。① 游标不签名可接受：Integer / Instant 只认规整写法，伪造在接入面答 400；Text 值走参数化查询，篡改最多跳到同一份列表的另一位置，不越授权范围。②④⑤⑥ 接受。

非阻断：
1. **「多取一行判下一页」没有共用件**（作者已点明未做）。票 02 / 03 会各写一遍「limit + 1 → 截断 → 末行编游标」，决定七要的是一处定义。建议在票 02 开工前补一个泛型小函数（如 `Trim[T any](rows []T, size int) (page []T, more bool)`），或在票 02 里顺手加并让 03 照用。

结论：**可接受**（Standards 0 / 0 · Spec 0 / 1）。

**处置**（推送方 · 通道 1）：评审无阻断，按 parallel-sessions「别人分支上的活怎么进 main」重放。分支 `mcp2-crp01` 的七笔在 `3953a9ec` 上 cherry-pick 为
`52f983ce`（← `5537db0c`）/ `1167ea02`（← `c048746a`）/ `b9b04675`（← `91d932d6`）/ `62a04eac`（← `f23f16e6`）/ `d2f284d6`（← `33fd352d`）/
`69f6e5d1`（← `5ed8abcd`）/ `7f594a92`（← `db96d561`），内容与分支 tip 逐字一致（`git diff db96d561 <重放 tip> -- internal/platform/cataloguepage` 与本票面均为空）。
非阻断 1 转作者：票 02 开工前补共用的「多取一行判下一页」，或票 02 顺手加、03 照用。票面转 resolved 由作者按推送方广播办。

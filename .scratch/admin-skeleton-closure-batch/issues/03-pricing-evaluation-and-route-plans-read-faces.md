...
- **2026-08-31 MCP-3（阶段二已交，票闭）**：基 `ffbc2d8`（批 07 装配第三笔，MCP-1 点名换基，
  含装配线 A `de09f55` 全部内容；变基零冲突——线 C 六笔无 admin-web 改动）。计价侧：词表
  `3a5dd8a`、api 出口 `c4fceb1`、页面重写 `957c8a7`——登记册检索列面
  （引用/封闭五格状态/双摘要/规范化版本/登记时间），旧骨架快照列（对象、方向、目的、基准
  时点、金额、清单）全撤且注明理由，金额列连 0 顶替的机会也不留；空态句「读取入口已配置，
  但评价登记册为空……册空是墙拦不是缺陷」。网络侧：词表 `91db053`、api 出口 `e6dd730`、页面
  重写 `1c25e07`——两册 chip 切换（形取网络目录页），「无当前有效路由」占完整结论格，适用性
  组逐态呈现（successor/basis 在场才显），候选缺席显「不评估」、改路缺席显「未评估」；空态
  句同上。`liveIds` 加 `'pricing-evaluation'`、`'route-plans'`（`5d9022d`）。`tsc --noEmit` 绿；
  既有已接线四页与 `catalogue-api`/`catalogue-view` 零改动。

...
- **2026-08-31 MCP-3（阶段二已交，票闭）**：基 `ffbc2d8` 交付，MCP-1 序移落地于 `1f5d7ae`
  ...
  计价侧：词表 `0bde129`、api 出口 `47337c2`、页面重写 `fd172b0`——登记册检索列面
  （引用/封闭五格状态/双摘要/规范化版本/登记时间），旧骨架快照列（对象、方向、目的、基准
  时点、金额、清单）全撤且注明理由，金额列连 0 顶替的机会也不留；空态句「读取入口已配置，
  但评价登记册为空……册空是墙拦不是缺陷」。网络侧：词表 `e94a3f7`、api 出口 `94ae07e`、页面
  重写 `9641413`——两册 chip 切换（形取网络目录页），「无当前有效路由」占完整结论格，适用性
  组逐态呈现（successor/basis 在场才显），候选缺席显「不评估」、改路缺席显「未评估」；空态
  句同上。`liveIds` 加 `'pricing-evaluation'`、`'route-plans'`（`528b1fd`）。`tsc --noEmit` 绿；

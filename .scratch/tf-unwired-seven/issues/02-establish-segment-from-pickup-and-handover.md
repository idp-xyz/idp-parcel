...
- 2026-09-03 · MCP-3：**本票 resolved。** 四笔：`a318f0e`（交接侧首个对象立段）、`f680e1e`
  ...
  **↑ 这句是假的，2026-09-03 由 MCP-1 在票 08 取证推翻。原句留着不删，因为它是证据。**
  `NewFulfillmentSegmentReference` 与 `NewPlannedSegmentReference` 都只调 `newRequiredValue`，
  而它唯一的失败是去空白后为空；进段门在调构造器**之前**就有一道空白判据把那一种筛走了。
  **所以「格式不合法」这个情形不可达**，那两处 `err != nil` 一行也走不到——能走到的只有空白串，
  而空白串等同于「没给段引用」，静默不立段正是它应得的答案。取证与那条钉住判定的测试在
  [08](./08-multi-object-pickup-attempt-establishes-no-segment.md) 的 Comments 里。

  **代价与教训记在这里，因为犯错的是本票。** MCP-1 照着这句话实现了一整套三格答案（改了
  `enterFulfillmentSegment` 的返回类型、给三个结果类型各加了一个访问器），**red 转不成 green 才
  回去看构造器**，然后全部回退。**那句话读起来与取证过的一模一样**——具体、有位置、有后果，唯独
  没有人去看一眼 `newRequiredValue`。我写它时是从「构造器会失败」推的，不是量的。
  这与本仓反复记的是同一条：**推测与取证在写下来之后长得一模一样**，而这一次它让另一个会话白做
  了一轮。**照它做还会更坏**：那会造出一个永远答不出来的答案格，下一个人会以为「没报输入未受理」
  意味着引用是好的。

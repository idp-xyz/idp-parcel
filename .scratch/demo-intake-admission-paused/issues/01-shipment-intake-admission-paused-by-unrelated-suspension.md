...
1. **随评审固定候选组（归治理 owner 复核）。** 评审要引已在册的候选版本组（`RecordStageReviewHandler` 的候选组引用完整性），
   而生产路径此前没有任何地方形成它。分三点：
   - **生产路径仍然只有这一处形成候选组。** 本 CLI 是唯一入口；接入渠道或别的编排若也要形成候选组，须另议，本票不开。仍只开票面
     裁决的一个子命令，没有另开候选组登记口。
   - **CLI 输入换来的是什么。** 输入多一段 `candidateSet`，评审的范围版本与候选组引用改为取自这段、不再另给（见第 2 条）。代价是
     一次命令同时承载「固定组」与「记决定」两件治理事实，留痕只记一笔 `stage-review`。
   - **同一事务的回滚语义。** `FixCandidateSetHandler` 先行：同内容重放答 `ALREADY_FIXED` 照常往下；同标识异内容答
     `CONTENT_CONFLICT`（组不可扩张）或读写失败答 `UNDECIDED` 时不进评审。评审答冲突、未受理或未决时整笔撤回，候选组一并不落，
     也不留痕。评审已落而覆盖关系撞上在册相悖边（重放路才会遇到）时，决定照落、答续办引用；若追加是因库错失败，同一事务里的留痕
     随之失败，整笔翻成未决，重跑同一命令续办。

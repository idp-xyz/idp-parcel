...
- **owner 复核 2026-09-09 认可**（用户经 IDP 队列通道 1 授权代裁，越权风险点 ①–④ 逐条）：① 失效版本继承离场三件——失效版本与替代版本同是链上一个版本，ADR-0112 决定三那条规则说的是「版本」不是「替代」，用到失效格是同一条规则不是新规则；② 凭新控制事实再进已失效的段只能经更正从失效版本长出替代版本——「已结束不重开」是 CONTEXT 生命周期句，失效是结束的一种；TF CONTEXT「履约参与关系」补的那半句连同认可，它是领域规则不只是裁决；③ 退掉 CORRECTION_WITHDRAWS_CONTROL——本票就是让「撤控制的更正」从被拒变成使参与失效，被拒那格没有对象了，HTTP 面零引用（评审核过）；④ 失效链尾答 OBJECT_NOT_IN_SEGMENT——与裁决 4 对 EndFulfillmentParticipationHandler 同句，ADR-0114 决定二加了一条复核记录说明「被替代或已离场」自本票起覆盖已失效，正文字面不改。

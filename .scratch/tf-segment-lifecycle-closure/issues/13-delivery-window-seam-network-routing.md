...
- **评审 ← 通道 1（推送方自跑 /code-review，非作者）· 钉 `dab85498`（基 `0c9b846f`，代码 tip `9a068d2d`）· 16:0x**。通道 2 在 tf/14 上、6 余量将满、3 / 4 未重起，按 09-08 规矩推送方自跑；两轴串行、互不引用。**Standards**：阻断 0；非阻断 1——`ports/delivery_requirement.go` 头注改写后仍说「parcel-shipment 那一侧读哪个读面归票 12……各与所有者对齐后才落适配器」，而本分支基的 `0c9b846f` 已含 tf/12 的 `adapters/parcelshipment.DeliveryPlaceSource`，那半句在 rebase 后过时了；tf/14 落地时同一段头注要再改（三条都接），届时一并改准，不单独开笔。核过：注释全中文；跨文件引用皆有标签（ADR-0131 决定一 / 二 / 三、ADR-0114 决定一 / 三、ADR-0004、ADR-0025、ADR-0003、ADR-0029，越权风险点用引文）；`adapters/networkrouting` 只 import `nrdomain` + `tfdomain` + `tfports`，窄口 `PlannedLegWindowSource` 一法、不 import NR ports / application（ADR-0025，头注把「不拿到改计划的能力」说成类型上的话，好）；租户原值传过（ADR-0003）；基线剪 `ParsePlannedLegReference` 一行带成因三分 + 「在 c0cdebba 上 4 → 3」两数同锚一棵树，推送方在重放 tip `1cb827b0` 实测 **3**，一致；端口拓宽走三步法三笔各自全仓编过（expand `30a49640` / migrate `0d3dd6a3` / contract `e400e8ac`）；smell 基线无可报项（`present bool` 一格是显式缺席而不是零值判，作者判断题已述）。**Spec**：阻断 0；非阻断 0。做法 1–7 逐项对上：② 对象入参**不保留**（票面留给本票定，定了）、头注按引用取 / 缺席由适配器答；③ 五种输入各一格无 default（缺席 → MISSING 不调 NR / 解析失败 → MISSING / found=false → MISSING / found=true → RESOLVED 带 Earliest·Latest / error 原样上抛），不读 `PlanApplicability`，「四态不分支不是漏 default」写在头注里；④ 执行器把 `participation.PlannedSegment()` 交端口、不重复判缺席；⑤ 测试六例含「已被替代版本的段仍 RESOLVED」钉裁决②；⑥ 生产入口归 tf/12，本票只填 `Deps.Windows`，装配测试停点后移到 `DELIVERY_CONDITION_SOURCE_NOT_WIRED`；⑦ 完成记录含 ADR-0114 决定四那句「已由 ADR-0131 决定三答」。完成判据四条 ✓；红线四条 ✓（`internal/networkrouting` 零改动、无真实时间窗、不从计划推事实、四态一律交内容）。作者八道判断题全部认可，无补充。**结论：两轴无阻断，进 main。**
- **进 main 记录（推送方 · 通道 1 · 16:0x）**：分支已由作者 rebase 到 `0c9b846f`（= 当时 origin/main，tf/12 刚进），隔离树 `%TEMP%\idp-replay-tf13` 在其上 cherry-pick 六笔全干净：`4e5d34a9→b8c3d0c1` / `30a49640→0363b088` / `0d3dd6a3→31a71b22` / `e400e8ac→eb18af94` / `9a068d2d→0c99d362` / `dab85498→636366c3`；作者清点笔 `bdb1224c` 与 main 同文件冲突，**跳过**，推送方在 tip 重生成为 `1cb827b0`（单独一笔）。本票十件与分支逐字相同。`1cb827b0` 上 `gofmt -l` 零、`go build ./...` / `go vet ./...` 0、`./internal/architecture/...` ok、基线实测 3；占 55432 广播后**带 DSN** `go test -p 1 -count=1 -v ./...`：104 ok / 0 FAIL / 15 无测试，`--- PASS` 8048 / `--- SKIP` 1（仅 `pgtest.TestHelperTemplateOwnerProcess`）/ `--- FAIL` 0 / 0 cached，124 s，探针 PASS，释号广播。本条随推送方簿记一笔落在 `1cb827b0` 之上，ff 后 `push <sha>:main`，SHA 在推后广播里。tf/14（通道 2）随后 rebase 到它之上填 `Deps.Conditions`。

...
- **owner 复核 2026-09-09 认可**（用户经 IDP 队列通道 1 授权代裁，四条逐条）：① 参与表主键换四元——0006 头注「不需要版本维」是立表时的判断，CONTEXT 生命周期句后半（更正长出替代版本）要求版本维，头注让位于 CONTEXT；② 「在场」= 未离场且无人回指——链尾才是在场，与 tf/11 的 `Active()` 同一条谓词；③ 段已关闭仍插替代版本——替代继承终点，段不重开，tf/11 用例钉住；④ `CORRECTED_START_AFTER_INHERITED_END` 第三格——已在 ADR-0112 加复核记录，正文不改。

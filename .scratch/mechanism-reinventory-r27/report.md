...
**认可追补（2026-08-28 · MCP-1 受托认可轮）**：用户批复「你作为业务和系统专家，根据你的
建议」委托裁断，上列留待清单逐项审毕**全数认可、无一翻案**，产品就绪宣布成立；逐项理由
与横向核验记录在[开发计划复核记录](../development-plan-2026-08-20.md)末条，基线「机制半边
现状」状态列同笔更新（非重盘，代码事实仍锚 `f6f0029`）。
...
## 追补（2026-08-28 · MCP-1）：CI 门禁自 08-20 起因 Actions 账务停摆

收口后的例行核验撞出一条系统性事实：GitHub Actions 在 `main` 上的最后一次成功运行是
**2026-08-20T12:18:52Z**（历史成功共 371 次）；此后每次 push 的运行都在 3–4 秒内以同一
注解失败——「The job was not started because recent account payments have failed or your
spending limit needs to be increased」（`gh` 实测 2026-08-28，样本含 `f6f0029`/`86cc5d8`/
`b52e8e4` 三笔与更早约百次，最近百次运行零成功）。影响与不影响各说清：

- **不影响本轮判据读数**：第四节的可复算证据本就是本机 WSL 真库锚定实跑（80 包全 ok、
  零竞态、锚 `f6f0029`），不引 CI；r24–r26 各轮证据同型（各轮报告均以本机实跑为据）。
- **影响的是基线「持续保证」那半句**：`.github/workflows/ci.yml` 的门禁配置仍在且正确，
  但自 08-20 起没有实跑过——「真库适配器在 CI 上实跑而不是跳过」这七天里是配置事实而非
  运行事实。基线该句已同笔加注运行事实（见提交记录）。
- **恢复动作在用户侧**：GitHub「Billing & plans」结清欠费或调高支出限额；机器侧无可代劳
  项。恢复后首个 push 的绿跑即可把「持续保证」接回，届时基线加注可删。

...
> **顺手记一个不属本票、但取证途中撞到的小缺陷（2026-08-21，对 `3df0160`）：**
> `envelope_partition_gate_test.go` 的文件头注释写着「与本包**另三套**门禁同一约定」，
> **今天已经差一**——除它自己之外带 `...CanActuallyCatchAViolation` 自测的是**四**份
> （`enum_exhaustiveness`、`identity_prefix`、`rehydration`、`transaction_closure`；
> `boundaries_test.go` 无此类自测）。成因是**新增一份门禁不会去改别人文件头的那句计数**，
> 于是它无声过期。**我按那句注释向 MCP-3 报过「四套」，差点进 ADR-0071**，是 MCP-3 去数了
> 文件才拦下。这与「跨文件引用不用行号」是同一回事的另一面：**行号会指错，计数也会数错，
> 两者都失效无声。** 未改该注释——`internal/architecture` 非本票地盘、本票零代码；记在这里
> 是因为本节大量引用该文件，读到这里的人正是会去读它注释的人。
...
**丙类能不能被守住，不取决于它多隐蔽，取决于它有几个入口**（这条判据由 MCP-3 提供，材料是同一个
`assemble.go` 里一处**丙类被成功拒绝**的先例）：`intakeQualificationEvidence` 对「填了表却没认领
段」当场报错，注释诊断的正是同一件事——「放过去就是一张永远问不到的表，判断结果与真的没配置一
模一样，而要人做的事相反」。

**它拒得成功靠的不是更仔细，是那条路只有一个工厂。** 所有消费者从它过，检查写一次就够，且有用例
钉住（`TestAQualificationRegistryWithoutAClaimedAuthorityIsRefused`）。而租户那条路每个消费者各建
各的包装，于是检查各抄一份、无一有用例钉。

所以丙类可操作的判据是：**入口收拢到一个 → 退化成一次普通的运行期检查（可守）；散在 N 个装配点
→ 只剩改类型一条路（守不住，得动签名）。** 这也解释了为什么 ADR-0071 裁的是方法签名而不是加一道
检查——签名是它那 N 个入口唯一的公共上游。

...
- **评审 ← 通道 6 · 钉 `b1d8bc40`（代码 tip `2d19bc32`，基线 `76932b38`，5 笔）· 2026-09-10 19:3x**（task-0509b519，非作者，隔离树 `%TEMP%\idp-review-lc29`；由推送方代落）。评审侧验证（无 DSN——本票不动 postgres）：`gofmt -l ./internal/parcelshipment` 空；`go vet ./internal/parcelshipment/...` 退 0；`go test -count=1` PS domain / application / adapters/partycommercial / adapters/parcelpricing / internal/architecture 五包 ok；未跑全仓。
  - **Standards（红线 / PS CONTEXT / ADR-0025 / 领域包不依赖 HTTP、pgx / 注释规矩）**：**阻断 无**。**非阻断 2（都是判断）**：(1) `adapters/partycommercial/channel_selection_basis.go` `NewChannelSelectionBasisTranslator` 头注只为三个源论证了 nil = 显式未配置，但两个 PC 读口 `Authorizations` / `Contents` 不是实例半边，nil 会在 `authorizationOf` 的 `LoadLatest` 处 panic 而非具名停下；同包 `NewCommercialResolutionKeys` 拒 nil、`NewCommercialBasisAdapter` / `NewChannelCandidateAssembler` 不拒——与 deps-struct 同伴一致不算违规，但头注已在区分「可 nil」与「必填」，必填那两口宜在构造期拒。(2) `authorizationOf` / `agreementOf` 在 `AllowsUseAt` / `SupportsProcurementAt` 答否后用 `!query.At.Before(revokedAt)` 自分「已撤销 / 不在有效期」，这条边界谓词与 PC `channel_account_use_authorization.go` `AllowsUseAt`、`supplier_agreement.go` `SupportsProcurementAt` 内部那句字面相同（本 SHA 核过）但是第二份，PC 改边界时这里不报；另 `AllowsUseAt` 因 `status == Invalid` 答否时会被标成 NotEffective——只影响分格标签不影响放行，PC 侧出一个「为何不允许」访问器可消掉。**无发现**：文件级导入仅 psdomain / psports / pcdomain / pcports（判据 5），不导 PP、不导 PC adapters/postgres；三源接口留在适配器包（ADR-0025）；`domain/` 新文件只导 `errors`；注释全中文、无行号、跨文件引用用符号名；无任何候选 / 账号 / 映射 / 价卡 / 协议版本 / 接受时解析取值，三源未配置各具名停、无默认放行、不拿「最新一条」顶替；PC / PP 任何包零改动。
  - **Spec（完成判据 1–7、「做法」与「裁决」）**：**阻断 无**。**非阻断 3**：(1) `adapters/parcelpricing/cost_bridge.go` `withRateReference` 不在票面「地盘」四项之内，但是裁决 (3)「费率由择优步带出」的必经之处（价卡版本只在 PS→PP 桥上有），且「不动 PP 任何包」指 `internal/parcelpricing`——范围说明；(2) 完成记录称「源未装与源答未配置同一格」，但 `TestEachTranslationStopIsNamed`「授权源未装」子用例用的是零值替身（答未配置）而非 nil，`accountUseFor` / `agreementVersionFor` / `resolutionFor` 三处 `deps.X == nil` 分支无用例走到——三行平凡代码；(3) `Handle` → `Select` 窄面等价核过：并列 / 无人参选时先 `recordDecision` 再返 err、候选零值与旧路同；正路多一步 `selectedCandidateOf`，赢家不在成本表按 `ErrChannelCostsIncomplete` 报——`coversEveryCandidate` 已保证这支实际不可达，属诚实兼容非行为变化。**无发现（判据逐项）**：1 ✓ `domain/selected_channel_basis.go` Spec 构造、七格任一空白拒（`TestASelectedChannelBasisRefusesAnyBlankReference` 八子用例）、Evaluation 可缺席，头注两句在；2 ✓ `Select` expand、`Handle` 旧签名不动、两格从 `Costs` 那份取，`TestASelectionHandsOverTheWinnersEvaluationAndRate` 证同源；3 ✓ 十二格 `TestEachTranslationStopIsNamed` 逐格 + 费率缺席、译不过去各一条，全部 `errors.Is`，正路 `TestASelectedCandidateTranslatesIntoAllSevenReferences` 七格过 `EstablishLabelTransaction` 门、服务方与结算相对方同值；4 ✓ 只改头注；5 ✓；6 ✓；7 ✓ 逐笔 SHA、七格取处对照「取证」表、类型名与 expand 形状均在。
  - **重点核 ①–⑦** 全属实（④ `selectedChannelBasisOf` 头注写明「同源是首发实例事实不是类型同义」；费率只读 `SelectedChannelCandidate.Rate()` 不问 PP；`ResponsibilityBasis` 填接受时 `CommercialResolutionID`；⑦ 三条越权风险点字面内——diff 不碰 PC、未立第二格、未在 PC 侧另记）。**⑥ 三道判断题全同意**：(1) `ChannelAccountUseSelection{Authorization, Grantee}`——PC 无「租户的运营企业是哪个 party」概念，它是实例半边且与选法同源，单开一口配置只多一处可不一致的登记；(2) 停点多四格——不在册 / 范围不符 / 费率缺席各是不同续办，按恢复动作分格是本仓一贯口径（ADR-0029 精神）；(3) `ErrUntranslatableAnswer` 六处保留、不补用例——具名包裹是诚实的 fail-loud，要走到它得绕 PC 构造门造空白对象，比未覆盖更坏，完成记录已如实记。
  - **结论**：两轴各 0 阻断；Standards 最重是必填读口不在构造期拒 nil，Spec 最重是 cost_bridge 超地盘字面的范围说明（有裁决支撑）。可重放进 main。
- **进 main 记录（通道 1 推送方，2026-09-10 19:34，簿记笔 `32778486`；本条、评审条与 Status 行原写 20:0x，19:4x 按 git 提交时刻改正）**：隔离 detached 树 `%TEMP%\idp-replay-lc29` 先叠在 lc/31 链 `037d88b8` 上 pick 五笔全干净（作者清点笔 `03ce7c1b` 不带，tip 重生成 `85b5898b`）；本票文件对分支 tip 零差。**验证钉 `85b5898b`**：`gofmt -l` 空；`go build ./...` / `go vet ./...` 退 0；带 DSN `go test -p 1 -count=1 ./...` **105 ok / 0 FAIL / 15 无测试 / 0 cached**（19:24:19→19:26:25，126 s）；PS adapters/partycommercial `-v` PASS 189 / SKIP 0。lc/31 进 main（`03474d87`）后 `rebase --onto 03474d87 037d88b8` 零冲突，代码树对 `85b5898b` 逐字节同：`68c3e72a→df6d486b`、`18f79dda→2e8b7f48`、`944e97b7→74ee7ca9`、`2d19bc32→e53db985`、`b1d8bc40→50910105`、清点 `58c59daf`。分支 `mcp3-lc29@b1d8bc40` 作封存出处、改名 `merged/`。评审非阻断五条随票记、不挡合入（Standards 1 与 Spec 2 是同一处的两面：构造期拒 nil 顺带补三处 nil 分支用例，可随 lc/28 组合根装配时一并做；Standards 2 归 PC 出「为何不允许」访问器，另票）。

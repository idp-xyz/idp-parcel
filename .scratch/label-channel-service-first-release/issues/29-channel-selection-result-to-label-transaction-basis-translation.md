...
Status: draft——通道 3 于 2026-09-10 17:3x 按通道 1 裁决（[`28`](./28-channel-selection-composition-root-and-call-entry.md)「要裁的」2 另立，task-bc04bfc2）立票，取证锚远端 main `062f5228`；**只写票面，未动代码。** 「七类引用各从 PC 哪个对象取」以代码为准逐格取证（见「取证」），四格答得出、三格答不出——答不出的四条列在「要裁的」，各带默认；裁完转 ready-for-agent
Blocked by: 无（裁决在本票「要裁的」；与 `28` 并行——`28` Blocked by 本票）
...
- **端口**（PS `ports`，编排看得见）：`ChannelSelectionBasisTranslator.TranslateSelectedCandidate(ctx, query psports.ChannelSelectionQuery, candidate psdomain.ChannelCandidateID) (psdomain.<择优结果对象>, error)`——交回 `28` 判据 1 那个「择优结果」对象（候选 + 七类引用 + 评价痕迹引用）；对象本身由 `28` 定形，本票只填它。停下的格各自具名：授权未配置 / 协议未配置 / 授权已撤销或不在有效期 / 协议不在有效期 / 译不过去（照 `ErrUntranslatableQuery` / `ErrUntranslatableAnswer` 的方向区分）。
- **适配器**（`adapters/partycommercial/channel_selection_basis.go`）：把 `psports.ChannelSelectionQuery` 与候选译成 PC 键（照 `providerKeysOf`）；经两个实例半边源问「这个候选适用哪条账号使用授权、哪一版供应商协议」（要裁的 1 默认）；用 PC 既有点读口取回授权与协议；核授权 `channel` 等于候选、`grantee` 是运营企业、`scope` 与 `effective` 覆盖 `query.At`、状态未撤销——不满足即停，不换一条；把 `account` / `grantor` / `Supplier()` / `Version()` 译成 PS 的四个引用类型；`Rate` 与 `ResponsibilityBasis` 按要裁的 3、4 的裁决取。
- **不做的**：不在本包判候选该不该赢（那是择优的事，票 `01` 裁决）；不复制映射、授权或协议的任何规则；不读凭据（ADR-0039：授权记录不拥有凭据）。
...
- 不填任何账号、持有人、供应商、协议版本、价卡、范围取值（实例半边 `PAR-INT-02` / `PAR-COM-10` / `PAR-SET-03`）；两个实例半边源未配置即停，不改成默认放行、不拿「最新一条」顶替「适用的那条」。
...
1. 端口与「择优结果」对象的接法与 `28` 判据 1 一致（谁先落地谁定形，后落的对齐；两票同期时在频道对一次）。
2. 四条「要裁的」各有裁决写进 Comments 或本节；默认被推翻的那条按裁决改写「做法」。
3. 适配器对每一格停点各一条测试（合成替身、合成串）：授权未配置 / 协议未配置 / 授权渠道不等于候选 / 授权不在有效期 / 授权已撤销 / 协议不在有效期 / 译不过去；一条正路：两源配上 → 七格齐 → 与 `Establish` 的构造门逐格过。
4. 架构门禁：本包仍只导入 `psdomain` / `psports` / `pcdomain` / `pcports`（`Rate` 若取 PP，PP 那半落 `adapters/parcelpricing/`）；`go vet`、`./internal/architecture/...` 全绿。
5. `gofmt -l` 空、`go build ./...` / `go vet ./...` 退 0；`go test -count=1` PS `adapters/partycommercial` + `adapters/parcelpricing`（若动）+ `./internal/architecture/...`。
6. 完成记录逐笔 SHA、逐格写「从哪个 PC 对象、经哪个读口取」，与本票「取证」表对照，改了口径的写明。

## 地盘

`internal/parcelshipment/adapters/partycommercial/`（新文件 `channel_selection_basis.go` + 测试）；`internal/parcelshipment/ports/`（一个端口，只加不改）；若要裁的 3 取 (a) 且不由 `28` 的对象带出，另加 `internal/parcelshipment/adapters/parcelpricing/`。**不动**：PC 任何包（要裁的 1 取 (b) 才会动，那时另立 PC 票）、`operate_label_transaction.go`（`28` 的地盘）、`cmd/`（`28` 的地盘）。

...
Status: resolved——**已进 main，2026-09-14 15:1x**（通道 1 推送方：分支基 main 当时 tip `7d664915`、main 未动，直接 ff——三笔 SHA 不变 `2ff3cbe5` / `35257c3c` / `b293a921` + 本簿记笔；清点在 `b293a921` 重生成**零差**、不出清点笔；`b293a921` 带 DSN 全仓 `-p 1 -count=1` 110 ok / 0 FAIL / 16 无测试 / 0 cached（122 s），判据 2 真库例与 `Save` 版本例 `-v` PASS。**非作者评审 ← 通道 3** 钉 `b293a921`：Standards 0 阻断 / 3 非阻断 · Spec 0 阻断 / 2 非阻断，全文与 Spec ① 的读法见 Comments「评审」「进 main 记录」）；此前 resolved——2026-09-14 14:0x 通道 1（接手方；作者为前一任通道 1 会话，崩于 13:4x，用户 14:0x 令本会话接手）：分支 `mcp1-lc35` 基远端 main `7d664915`（分支就在 tip 上，重放为纯 ff），两笔——之一 `2ff3cbe5`（做法二 + 甲，判据 1、2）、之二 `35257c3c`（「归本票」五条，判据 3）；带 DSN PS 四包 + `internal/architecture` + 反向依赖 `cmd` 三包 ok（判据 4）；**待非作者评审进 main**（本通道换会话仍算作者一侧，评审须在通道 1 以外）；完成记录见 Comments 末。此前 ready-for-agent——2026-09-14 10:2x 通道 1 按用户 10:1x「授权代裁」（PS owner 口径）裁「要裁的」1：**取甲**（建立重放先查交易已存在则跳择优），乙不做、留作候选后继；做法二「先做」与三的五条「归本票」照旧；裁决全文见文末「裁决」。取证锚仍是票面的 `9ddbafcf`——作者开工先在 main 重量「缺口」两段点名的文件是否被 lc/36 / lc/37 / lc/38 改过（lc/38 只改了 `assemble.go` 注释与 PS 收寄路，与本票地盘无交集，但要核）。此前 draft——2026-09-10 22:0x 通道 4 立票（按通道 1 派单 task-d00c5556；lc/28 非作者评审 Spec 非阻断 ③ / 判断题 (c)(d) 与 Standards 非阻断 ① 的后继，顺带收 lc/26 / lc/29 两票评审里仍未修的非阻断）。要裁的一条（重放对账二选一）归 PS owner。只写票面未动代码；取证锚 main `9ddbafcf`
...
- 2026-09-14 14:0x · 通道 1（接手方；作者为前一任通道 1 会话，崩于 13:4x，用户 14:0x 令本会话接手）——**完成记录，转 resolved，待非作者评审进 main。** 分支 `mcp1-lc35` 基远端 main `7d664915`（分支就在 tip 上，重放为纯 ff；`origin/mcp1-lc35` 已同步到 tip），两笔：
  1. `2ff3cbe5` feat(lc/35 之一)——做法二 + 甲。做法二：`Select` 交回 `ChannelSelectionResult`（`ChannelSelectionOutcome` 三格 SELECTED / COST_TIED / NO_QUALIFIED_CANDIDATE，三格都是比较发生过、决定已记的业务答案），两个领域哨兵只在 `Select` 一处译成结果格；`Handle` 窄面删（无生产调用方）；`ChannelSelector` 随之改；`transactionalChannelSelection` 不再认任何领域错误——不返 error 就提交、返 error 就回滚，组合根那段业务分类连头注一起删。**判据 1** 于 `35257c3c` 复核：`git grep 'ErrChannelCandidateCostTied\|ErrNoQualifiedChannelCandidate' -- internal/parcelshipment/application cmd/ ':!*_test.go'` 只剩 `select_channel_candidate.go`（同一函数里两个 `case`）。甲：`EstablishSelectedLabelTransactionDeps` 加 `LabelTransactionLookup`（`ports.LabelTransactionRepository` 的 `FindByID` 即它，单列只读窄面、不扩写面），`Establish` 先问交易在不在，已在直接答 `ESTABLISHED` + `ALREADY_APPLIED`（建立步的现名，不新造格）、不择优不记决定，选中候选与择优结果缺席；组合根接 transactions 的读半边，不加规则。**判据 2**：真库 `cmd/parcel-api` 同 `TransactionID` 两次 `Flow.Establish` → 决定册仍一条 SELECTED、第二次 ALREADY_APPLIED（落 `assemble_label_channel_test.go`）。
  2. `35257c3c` feat(lc/35 之二)——「归本票」五条（**判据 3**）：lc/26 S(3) `LabelTransactionRepository.Save` 交回 `ports.LabelTransactionSaveResult`（结果格 + 落库后版本，UPDATE 加 `RETURNING revision`），`judgmentBeat` 用答复版本、`operate_label_transaction.go` 无 `Revision() + 1`，`pspostgres.LabelTransactions` 与应用层替身一并改，真库用例加钉答复版本 = 读出那一代加一且与库里一致；lc/29 S(1) `NewChannelSelectionBasisTranslator` 改返 `(*T, error)`，`Authorizations` / `Contents` 为 nil 构造期拒（形照 `NewCommercialResolutionKeys`），`assemble_label_channel.go` 那一处接错误，三个实例半边源仍允许 nil；lc/29 Spec(2) `TestTheTranslatorRefusesNilReadersAndNamesEachUnwiredSource`——两口 nil 构造拒 + 三源各为 nil 停在各自具名格；lc/26 S(1) `assemble.go` `labelTransactionJudgmentUndecidedSentinels` 头注与 `judge_parcel.go` 两处「四个读口之一」→「读口之一」——票面点名的另一处「判断编排的六口」在本票开工前已由 lc/36（`481e5af4`）于 main 去掉，本票不再动；lc/26 S(2) `parcelFinalAdoptionChain` 头注改句（两条链各得一份 handler，共用的是 `FinalOutcomeStore` 那一处终局）。原句「四个读口之一」「判断编排的六口」「各建一份 handler 会让」于 `cmd/` `internal/` 下 `.go` 零命中（rg，UTF-8 读，非 PowerShell 管道）。
  **二选一取甲**，理由见上「裁决」节（通道 1 10:2x 代裁）：不加列、不改 `ChannelSelectionDecisionSpec` / `SelectedChannelBasis` / `LabelTransaction` 任一形状、不动票 14 / 23「只追加」；乙留作候选后继归 PS owner。
  **判据 4**（于 `35257c3c`，树上无未提交改动）：`gofmt -l` 空（量暂存 blob，非工作副本）；`go build ./...` / `go vet ./...` 退 0；带 DSN `-p 1 -count=1`：PS application / adapters/partycommercial / adapters/postgres / adapters/labelfinal + `internal/architecture` + 反向依赖 `cmd/parcel-api` / `cmd/parcel-commercial` / `cmd/parcel-dispatch`（`go list` 反查 `parcelshipment/ports` 所得）8 ok / 0 FAIL；单跑 `TestAStaleLabelTransactionSaveAnswersRevisionConflict -v` PASS 非 SKIP；接线基线未响（未加导出领域工厂）。带 DSN 全量由推送方在重放 tip 上跑一次。
  **地盘核对**：`git diff --name-only 7d664915..35257c3c` 17 件全在票面地盘内（PS application / ports / adapters 三包、`cmd/parcel-api/assemble_label_channel{,_test}.go`、`cmd/parcel-dispatch/assemble.go` 四行注释）；端点表 / 探针 / 放行表 / `internal/architecture/*_baseline.txt` / PC / PP 零 diff。
  **接手方能力边界**：之一那笔由前一任写就、已提交并推分支，本记录按其提交信与 diff 复述，接手方未逐行重审；之二那 11 件是前一任留下的未提交现场（mtime 13:43–13:46，前一任最后一条广播为「占号 55432 跑 mcp1-lc35 工作区」、未报结果），接手方逐块核 diff 对上票面五条与「红线」后原样提交，未改一行。非作者评审须由通道 1 以外的通道做。
- **2026-09-14 14:57 · 评审 ← 通道 3 · 钉 `b293a921`**（task `819bdc5e`；推送方 14:46 点名、截止 14:50 前应答 4 个（通道 2 / 3 / 4 / 5），派通道 3——lc/28 评审人，对本票地盘最熟；通道 6 截至 14:50 未应答。基线 main `7d664915`，隔离检出 `$env:TEMP\idp-review-lc35` detached 干净；评 `2ff3cbe5` 逐行 + `35257c3c` 逐块，`b293a921` 纯簿记不评。以下为评审原文，推送方只把两个轴标题改成行内粗体）：
  **Standards** —— **阻断**：无。**非阻断**：① 跨文件计数（AGENTS.md「改文档」计数条，Go 注释同受约束）：`establish_selected_label_transaction_test.go` `TestAHalfWiredFlowRefusesLoudly` 头注「四口任一为 nil」数的是另一文件 `EstablishSelectedLabelTransactionDeps` 的字段（前身「三口」同病，本票 3→4 未去数）；`channel_selection_basis_test.go` `TestTheTranslatorRefusesNilReadersAndNamesEachUnwiredSource` 头注「两个 PC 读口 / 三个实例半边源」同类。生产文件里的「四口」「三个源」与被数 struct 同文件，可接受。建议改「任一口为 nil」。② 复述（「单一权威」/ Duplicated Code 判断题）：「壳只管事务：Select 不返 error 就提交、返 error 就回滚」一句在 `application.ChannelSelector` 头注与 `cmd/parcel-api` `transactionalChannelSelection` 头注各写一遍，会各自漂移；留一处、另一处引符号名。③ 段标缺席（ADR-0029 判断题）：`EstablishSelectedLabelTransactionHandler.Establish` 建立前那一问的读口故障只以字串前缀包出（`look up transaction: %w`），不像择优 / 翻译两段有 `ErrChannelSelectionStopped` / `ErrChannelBasisTranslationStopped` 段标。恢复动作与其它依赖故障同格，不必新格，但与本文件「停法各有段标」的惯例不齐。可不改。**无发现**：注释全中文、无行号引用（17 件 `+` 行正则抽验零命中）；`domain/**` 零 diff、`SelectChannelCandidateByCost` 未动；两个领域哨兵生产代码只剩 `select_channel_candidate.go` `Select` 同一 switch 两个 `case`（判据 1 grep 实测于 `35257c3c`）；`transactionalChannelSelection.Select` 无任何领域错误分类、组合根 `Lookup: transactions` 只接线；`Handle` 删净（build 过、`.Handle(` 零残余）；`NewChannelSelectionBasisTranslator` 拒 nil 与 `NewCommercialResolutionKeys` 同形；`ChannelSelectionOutcomeInvalid = iota` 零值格照 `LabelTransactionSaveOutcomeInvalid` 成例；ports 头注「Revision 只在 SAVED 格有意义」与 `pspostgres.LabelTransactions.Save` 冲突格返零 `Revision` 一致；`gofmt -l` 空。
  **Spec** —— **阻断**：无。**非阻断**：① 裁决 1 末句「那一问与随后的建立在 `transactionalChannelSelection` 同一事务内」字面未满足、且不可能满足：`Lookup: transactions` 接裸仓储，`FindByID` 走 `ReadExecutor` 在任何壳外自动提交；择优在 `transactionalChannelSelection` 一笔、建立在 `transactionalLabelTransactions` 另一笔——建立从来不在择优那只壳里，那半句自身不成立。「事务边界照旧」满足（既有两笔一根未动）。后果：同标识两次 `Establish` **并发**恰落在读与 `Insert` 之间时两次都过读、各记一条 SELECTED、Insert 一方答 ALREADY_APPLIED——「一笔交易一条 SELECTED」只对顺序重放成立；`Establish` 头注已明写认下这一格。READ COMMITTED 下把读挪进择优事务也堵不住（需锁），是裁决措辞题不是改法题。建议推送方在完成记录写明「并发窗口是认下的残差、裁决 1 末句按『边界照旧』读」，或 owner 补改一句裁决文字。② 判据 4 我未跑带 DSN 与 `internal/architecture`（只读、不占 55432），「接线基线不响」只从 `domain/**` 零 diff、无新导出领域工厂间接推得。**无发现（逐项）**：判据 1 ✓（三格都在 `recordDecision` 之后交回，「比较发生过、决定已记」成立；币种不齐等仍走 error 且不记决定）；判据 2 ✓（`assemble_label_channel_test.go` 「a replay of the same transaction…」用 `decisionsFor`→`ListBySubject` 真读决定册断 `len==1 && SELECTED`，并断第二次 ALREADY_APPLIED、带回既有交易、`Selected()` 缺席——不是只看答复；`pgtest.Pool` 每测新库，重跑不撞键）；判据 3 五条 ✓（lc/26 S(1) `assemble.go` + `judge_parcel.go` 三处「四个读口之一」→「读口之一」，「判断编排的六口」grep 零命中；S(2) `parcelFinalAdoptionChain` 头注改句、结构未动；S(3) `Save` `RETURNING revision` + `pgx.ErrNoRows`→冲突格、`judgmentBeat` 用 `afterSave` 收到的 `saved.Revision`、`operate_label_transaction.go` 无 `Revision() + 1`、`pspostgres`/替身/真库例齐改；lc/29 S(1) 构造期拒两口、`assemble_label_channel.go` 接错、三源仍允 nil；Spec(2) 三源各 nil 断在 `ErrChannelAccountUseNotConfigured` / `ErrSupplierAgreementNotConfigured` / `ErrAcceptanceResolutionNotConfigured` 各自具名格）；判据 5 ✓；裁决 1「现名不新造」✓（`SelectedLabelTransactionEstablished` + `LabelTransactionAlreadyApplied`）、「不扩写面」✓（`LabelTransactionLookup` 单列 `FindByID`）、「组合根不加规则」✓；红线四条 ✓（并列仍 `ChannelSelectionTied` 交人裁、无人参选仍不建立、决定册只追加——甲是不再多写；比较器未动；`domain/**` 零 diff；共享文件只动点名行）；地盘 ✓（`git diff --name-only 7d664915 35257c3c` 17 件全在票面地盘；PC / PP / `internal/architecture` / docs / migrations 零 diff）。
  **汇总**：Standards 0 阻断 / 3 非阻断（最重 ①）；Spec 0 阻断 / 2 非阻断（最重 ①）。可进 main；建议推送方把 Spec ① 那句写进完成记录。**跑了**：`go build ./...` 退 0、`go vet ./...` 退 0、`gofmt -l` 17 件空、`go test -count=1` `internal/parcelshipment/application` / `adapters/partycommercial` / `cmd/parcel-api` 三包 ok（无 DSN，真库例 skip）。**没跑**：任何带 DSN 用例、`internal/architecture`、`adapters/postgres` / `adapters/labelfinal` / `cmd/parcel-dispatch` 包；未逐例 `-v`。未改检出一行、未提交任何东西。
- **2026-09-14 15:1x · 进 main 记录（通道 1 推送方）**：分支 `mcp1-lc35` 基 `7d664915` = 当时 main tip，`fetch` 核 main = origin/main 未动 → 无需 cherry-pick，三笔 SHA 原样 `2ff3cbe5` / `35257c3c` / `b293a921`；清点在 `b293a921` 干净检出重生成（`tools/mechanism-inventory`）**零差**——本票改的 19 件全是既有文件，逐上下文文件面与端口面计数不动，不出清点笔；`gofmt -l` 空、`go build ./...` / `go vet ./...` 0；15:0x 先排队列再占号，带 DSN 全仓 `-p 1 -count=1` **110 ok / 0 FAIL / 16 无测试 / 0 cached**（122 s），判据 2 真库子例「a replay of the same transaction leaves a single SELECTED decision」与 `TestAStaleLabelTransactionSaveAnswersRevisionConflict` 单跑 `-v` PASS 非 SKIP；释号。评审 Spec ② 点名的带 DSN 与 `internal/architecture` 由这一跑补上（architecture 包在 110 ok 之内）。簿记一笔在其上 → ff → `push <sha>:main`。分支 → `merged/`，远端删，树拆。
  - **评审 Spec ① 收进本记录（推送方以 PS owner 口径、按用户既有「授权代裁」）**：裁决 1 末句「那一问与随后的建立在 `transactionalChannelSelection` 同一事务内」**按「事务边界照旧」读**——`Lookup` 接裸仓储、`FindByID` 走 `ReadExecutor` 自动提交，择优一笔、建立另一笔，与 lc/28 既有两笔一根未动，那半句的字面写错了边界，不改代码。后果如评审所量：同标识两次 `Establish` **并发**恰落在读与 `Insert` 之间时各记一条 SELECTED、`Insert` 一方答 ALREADY_APPLIED，「一笔交易一条 SELECTED」只对顺序重放成立；**并发窗口是认下的残差**（`Establish` 头注已明写；READ COMMITTED 下把读挪进择优事务也堵不住，要堵得上锁或走乙路的引用），归乙路候选后继、不在本票。裁决节原文不改写（历史），以本条为准。
  - **其余非阻断随票记、不挡合入**：Standards ①（两处测试头注「四口」「两个 PC 读口 / 三个实例半边源」计数）与 ②（「壳只管事务」一句两处复述）→ 候选尾巴票，与 lc/36 / 37 那类「注释去数换点名」同款，等下一批 A 类合票；Standards ③（那一问读口故障无段标）评审自判可不改，不立。

...
本票代码最终与本节同笔提交。中间绕了两趟，记下来是因为**这条竞态在
[并行会话](../../../docs/agents/parallel-sessions.md)里只记了一个方向**。

**第一趟：被别人卷走。** 本会话 `git add` 之后、`git commit` 之前，另一会话先跑了 `git commit`，
而 `git commit` 提交的是**整个索引**——本票暂存的文件被一并卷进它那一笔（当时为 `1b7bd0d`，
标题是它自己的「机制半边现状的三处接线叙述已失效」）。本会话随后那次 `git commit` 因索引已空
而扑空，输出是一句 `no changes added to commit`。

**第二趟：又被退回来。** 依上述如实记了 SHA 之后，对方 amend 了那一笔（`1b7bd0d` → `46c34a9`），
本票的文件随之**退回索引**、不再在分支上——刚写下的那个 SHA 当场作废。这一节因此改成现在
这样：不指任何一个会被改写的 SHA，只说清代码与本节同笔。

两趟都没丢内容（每一步都比过 `git diff`，工作树始终完好），丢的是两次可检索性。

**结论是命令换一个：`git commit --only -- <paths>`。** 它只对给定路径取快照、不理会索引其余
内容，正是共享工作树该用的形状。`git add` 加 `git commit` 这个组合在共享索引上先天带这条竞态，
而并行会话那份「精确暂存」记的是**卷走别人的**（`git add -A` 的方向），这一次是反过来——
**自己暂存好的被别人的 `commit` 带走**。同一个窗口，方向相反，`git add -A` 的禁令挡不住它。

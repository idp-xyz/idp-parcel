...
Category: chore
Status: resolved——第三类最后两笔各有归宿（2026-09-04，通道 6 经 owner 授权裁）：第 5 笔裁语义「main 的追加留痕成立、分支那半不合」（见[第五遍](#第三类的第五遍余两笔各归其位)）；第 13 笔那段操作警告已照原文补进 main（`cd21e68c`）。十四笔至此没有一笔仍是「未判的欠账」；本清单的职责——把没进 main 的活摆成可派的形态——已尽，不再有事可派
...
通道 6，锚 **`main = 512b419`**（本地共享树 HEAD，2026-09-04 18:2x；`origin/main` 落后，未用）。经 owner 授权
（task-f530ad56 裁决批口径：裁得下的裁，写明能力边界）。

**第 13 笔销案：已照原文补进 main（`cd21e68c`）。** 只加 `2f99bd3` 那四行 `#` 注释，逻辑一字未动；补前
`git grep -i UNDERFUNDED -- scripts/demo-seeds/seed.sh` 于 `512b419` 零命中，与第四遍的判定一致。分支
`cr04-readface` 上其余差异是分支旧于 main 的部分，不是欠账，不合。

**第 5 笔裁语义：main 的 `channel_execution`（追加）成立，分支的 `controlled_channel_execution`（按登记幂等）不合。**
第四遍判它「一个没人做过的取舍」——这一遍打开两侧看，取舍其实已经做过了，只是没有人把它写成裁决：

| | 分支 `4b5432a`（08-21） | main（`0004_channel_execution.sql`） |
|---|---|---|
| 写法 | `Record`，PK =（登记种类，登记身份），`ON CONFLICT DO NOTHING` | `Append`，PK = 自增 `execution_id`，无幂等约束 |
| 行上带什么 | OS 用户、主机、执行时刻 | 另带 **`command` 与 `outcome`**（登记册当时的答案） |
| 生产调用方 | 无 | `cmd/parcel-governance-register`（`tracer.Append`，与登记同笔事务） |
| 被沿用为先例 | 无 | **visibility-exception 照抄同形**：`0020_channel_execution.sql` + `cmd/parcel-ve-register` 两处 `Append` |

判据是留痕要回答的问题。受控通道留痕证的是「这条登记经受控通道执行过」，而「同一登记被执行过几次、每次
登记册答了什么」正是审计要看的：重放一次答 `EXISTING`，与首次答 `RECORDED`，是两件不同的事实。按登记键
幂等的 `Record` 把它们折成一行，等于把「重放过」从证据里抹掉——main 那份迁移的头注原句已经写下这一条
（「留痕做幂等等于抹掉『同一登记被执行过几次』这件事实」）。分支那版没有 `outcome` 列，也没法区分这两次。
所以两套不是并列可选，是一前一后：分支是 08-21 的首稿，main 是 09-01 起被两个上下文各自采用的定稿。

分支唯一多出的一格是 `registration_kind` 的封闭 CHECK（`authority-interval` / `suspension` / `resumption`）；
main 用 `command` 文本不封闭。要不要给 `command` 加封闭集归 `pilotgovernance` owner 另裁，它不构成合入
分支的理由——合进来得到的是一张少两列、把重放折掉的表。

**能力边界**：读了两侧的迁移、适配器与 `ports.go` 差异，以及 `cmd/parcel-governance-register` / `cmd/parcel-ve-register`
的调用点；**未读**票 12 当年的裁决原文（`.scratch/syn-wall-door-audit/issues/12`）以核「身份双轨第①轨」在两版之间
有没有措辞上的差异——本裁决只判留痕语义，不判双轨划分。

分支指针 `t12-governance-register` 照惯例保留，不删。

**第三类至此清零。** 十四笔的去处：五笔换基座重放（第三遍）、四笔重新落笔或独立重写（第二遍）、一笔整体采回
（二之二）、两笔按证据强度移出（第四遍）、一笔裁语义不合（本遍）、一笔补进 main（本遍）。**编号仍不重排。**
...
- 2026-09-04 通道 6（task-f530ad56 裁决批余量，owner 授权；锚 `main = 512b419`）：跑[第五遍](#第三类的第五遍余两笔各归其位)，第三类清零。第 13 笔照 `2f99bd3` 原文补进 main（`cd21e68c`，只加注释）；第 5 笔裁「main 追加留痕成立、分支幂等留痕不合」，理由与能力边界写在正文。状态行改 resolved——本清单原句「不自行转 resolved」的前提是两笔的去留归别人，如今两笔都有了归宿，清单没有剩余可派的事。未合并、未拆树、未删分支；`.scratch/` 之外只动了 seed.sh 那四行注释（另笔）。

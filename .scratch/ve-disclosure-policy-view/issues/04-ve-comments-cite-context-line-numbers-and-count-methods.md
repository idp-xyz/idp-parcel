...
Category: chore
Status: resolved——**已进 main，2026-09-11 23:2x**（通道 1 推送方：基 `609e334b` 纯 ff 不换号，`1e2eab1c` / `67be7173`；非作者评审 ← 通道 1 两轴 0 阻断；`67be7173` 带 DSN 全量 110 ok，见 Comments「进 main 记录」）；此前 resolved——2026-09-11 23:1x 通道 3 作者完工（task-7309fe70；分支 `mcp3-vedisc04-2` 基远端 main `609e334b`，认领 `1e2eab1c`，代码 + 本完成记录同笔，SHA 见完工报）：38 处 `硬句 NNN` 与同症 13 处裸行号全部换成 CONTEXT 原词引文、10 处方法计数改点名；零行为（diff 只有注释行）；判据 1–5 全过，真库未跑。**实测：38 处行号在 `609e334b` 的 CONTEXT 上 38 处全部指错行**，其中 3 处连原始编号也指错（见「完成记录」）。等非作者评审 → 推送方重放进 main。此前 in-progress——2026-09-11 22:5x 通道 3 认领（隔离树 `D:/tops/idp-parcel-mcp3-vedisc04`；21:2x 前会话的认领笔只在 salvage 分支上、零代码，不用）。此前 ready-for-agent——2026-09-11 14:2x 通道 1 推送方立票并直接转 ready（要裁的为零；收下 12:0x 节「VE 注释『六法 / 八法 / 七法』计数一族归 VE owner」那条候选后继，扩到同族行号引用）；取证锚 main `0b027ab8`
...
- **评审 ← 通道 1（推送方，非作者；无他人空闲）· 钉 `67be7173` · 23:1x**。对象 `git diff -U0 main 67be7173 -- internal/`，隔离检出 `%TEMP%\idp-replay-vedisc04b` 上脚本核 + 抽读；**取数一律走 UTF-8 文件与 `-f` 模式文件，不走 PowerShell 管道**——派单里那句「10 处 / 9 文件」就是通道 1 自己用管道数 `git grep` 中文输出数出来的错数（GBK 解码把多行并成少行，[workflow.md 的本机环境] 点名过的坑），作者纠正为 38 是对的，本条记下出处。
  - **Standards**：阻断 无。非阻断 ① 判断项 1 那几处跨文件计数（「七件」「六件必备」「五个装载口」「两组表」）仍在——基线已有、每处紧跟逐项点名、票面做法 2 只点了方法数，作者不动是对的；归 VE owner 与 02 评审 S②（`CatalogRegistry` 跨包同名）合成下一张 VE 小票。② 判断项 2「八个方法…三个接口」数的是本接口自己（同文件）——AGENTS.md 那条禁的是数别处的东西，同文件计数增减时改的人就在眼前，不算违反，留着可以。无发现：181 条 `+`/`-` 行逐条以 `//` 起头（非注释改动 0）；19 件全在 `internal/visibilityexception/**` + 本票面，`adapters/partycommercial/` 与 `CONTEXT.md` 零；`+` 行上 49 条「」去重后 44 条逐字命中 CONTEXT 当前文本，其余 5 条是票面完成标准名（「limit 非正即拒」「空册如实答空不折成未配置」「租户隔离」）与普通词（「不限量」「稳定序」），不是 CONTEXT 引文；无一条「」折行；注释中文。
  - **Spec**：阻断 无。非阻断 ① 判据 4「真库未跑」由推送方全量闭合（见下）。② 同症 13 处补收超出票面「缺口」列的 38 处，但同一病、同一红线（只注释）、同一判据形——接受，票面「完成记录」已如实分开记「38 处之内」与「同症补收」。无发现：判据 1 / 2 与同症两式在 `67be7173` 上用 UTF-8 模式文件重跑全零，同一式在 `main` 上 38 命中（模式本身有效）；判据 3 ✓；判据 5 ✓（38 / 38 指错行、偏移分四族、3 处原始编号即错、2 处引文不逐字，逐处写了真正要引的句）；判断项 3「无规则不符」接受。
- **进 main 记录（通道 1 推送方，23:1x–23:2x）**：`merge-base(main, 67be7173) = main 609e334b` → **纯 ff、零 cherry-pick、SHA 不换**。`%TEMP%\idp-replay-vedisc04b` detached `67be7173`：`gofmt -l` 空、`go build ./...` / `go vet ./...` 0；23:10 占号，带 DSN 全量 **110 ok / 0 FAIL / 15 无测试 / 0 cached**（23:10:55→23:13:07）；清点 tip 重生成 porcelain 空（不增删文件）；23:13 释号。簿记一笔在其上（本票 Status + 评审 + 本记录、tasks.md 节）；`ls-remote` 核 `609e334b` 未动 → 共享树 `merge --ff-only` → `push <sha>:main`。`mcp3-vedisc04-2` → `merged/mcp3-vedisc04-2`（指针留，远端删），`D:/tops/idp-parcel-mcp3-vedisc04` 比内容后拆。**候选后继（归 VE owner）**：判断项 1 的四处跨文件计数 + 02 评审 S② 跨包同名，一张小票。

...
注释里引另一个文件，照 [改文档](#改文档) 那一条办：用符号名或引文，不用行号。理由与实测都在那里，此处不重复。
...
- 引另一个文件的某句或某段 → 优先用符号名，无符号可指时用引文，一律不用行号。行号失效时仍然指得很稳，只是指错了，没有任何东西会报；引文至少失配可见，但中文注释一折行就搜不中。ADR 尤其要守——它引的常是它自己要改的那份代码，Context 会在决定落地那一刻失效。**Go 注释里的跨文件引用同受此约束**，理由一字不差地成立：全仓抽验六处行号引用四处已经指错（实测于 `1dbc680`），而往被引文档头部插五行之后 `go build`、`go vet` 与全仓 `go test` 零信号——`internal/` 下没有任何测试读过 `.md` 的内容，这一类**结构上守不了**，只能靠不写。

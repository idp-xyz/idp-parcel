...
// entities/13 第 1 / 3 条抬出）。四份 *-form.ts 各自只留本册的形状（草稿、载荷键、认领路径、修订建议），跨册同一判的
// 规则住在这里；此前 revisionOf 住在 business-party-form.ts 里导出、另两份反向依赖兄弟模块（票 10 评审 N3），建议值
// 顶格与落地判据则在四份表单组件里各写一遍（票 10 评审 N1）。组件那半的壳是 party-registration-fields.tsx 的
...
/** 带修订号格的草稿：四册的登记都是「一笔一个修订」，这一格四份表单同形。 */

...
- [项目现状、业务说明与全流程图集（2026-09-16，第 2 版）](./overview/project-overview-2026-09-16.html)：面向人的单文件 HTML 总览——产品定位、十四阶段主链、十个上下文、接单/履约/关务/追踪/结算五条线的流程与状态机、技术架构与运行时、首发范围、进度读法、演示动线、试点治理、风险与收费模型形态，附 26 幅 Mermaid 图并逐图标注取证来源。它是**带日期与 `main` SHA 的快照**，只组织既有权威文档，不另立规则；进度数字以取证时点为准，与上文任何权威文档冲突时以权威文档为准，可由代码算出的数以[机制半边清点](./product/MECHANISM-INVENTORY.md)为准。再出新版时在本目录另起一份带新日期的文件，不覆盖旧版。
- [项目完成情况审查快照（2026-09-22，钉 `e0d3f89d`）](./overview/completion-assessment-2026-09-22.md)：按开发主线自身判据（产品就绪 = 八切片机制半边达标；试点就绪归租户）对现状的一次只读审查——本机 `-race` + 真库与 `admin-web` 三道门实测、远端 CI 状态、文件级规模与对 09-04 快照的差量、可枚举的剩余机制缺口、归用户的裁决清单与实例半边墙。同上一条一样是**带日期与 `main` SHA 的快照**，只组织既有权威文档与生成物，不另立规则、不裁断任何在办票；再审时另起新日期文件，不覆盖。
...
- [IDP Parcel 商业主体管理 UX/UI V1.0（参考）](./reference/IDP_Parcel_Commercial_Party_UX_V1.0.md)：外部生成的商业主体（Commercial Party）管理界面方案——「主体主档 + 按票绑定角色」（发件 / 出口 / 卖方 / 付款…）、五步登记向导、按国家的注册号校验、主体详情页、去重合并与批量导入；**非权威、非规格、非验收依据**。整体模型不采纳：把交易各方收成一个主档并按票绑定角色，会改参与方、客户账户与委托各方的领域语言，今天没有触发它的需求，要做就单独走 `CONTEXT` / ADR。其中「Tax & Registration」「Country-aware」「Addresses 必须有 Validation 状态」「Contacts」几节只作责任法人业务属性建模的起草输入。原位于 `docs/ux/commercial-party.md`（未跟踪过），2026-09-24 用户授权自决后迁入。

...
- **`gh` 装不了 winget，认证也不能照搬 git 那份凭据。** winget 直连 `github.com` 取 MSI，不吃仓库级 `http.proxy`，实测卡满两分半毫无进展；经代理拉 zip 只要九秒，`Invoke-WebRequest -Uri "https://github.com/cli/cli/releases/download/v<ver>/gh_<ver>_windows_amd64.zip" -OutFile "$env:TEMP\gh.zip" -Proxy "http://127.0.0.1:7897" -UseBasicParsing`，解压到 `%LOCALAPPDATA%\Programs\gh` 再把其中 `bin` 加进用户 PATH，全程不需要管理员。认证是另一个坑：GCM 里那份 token **缺 `read:org`**，`gh auth login --with-token` 正是以这条理由拒绝，而 `GH_TOKEN` 不做这道校验、同一份 token 直接能用——因此不必另存一份密钥，每次把 `git credential fill` 输出里 `password=` 那行喂给 `$env:GH_TOKEN` 即可，token 仍旧只待在 Windows 凭据管理器里。`gh` 出网同样要 `HTTPS_PROXY=http://127.0.0.1:7897`，与上一条 git 用的是同一个代理。

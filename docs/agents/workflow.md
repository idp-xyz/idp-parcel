...
2026-09-21 用户裁定。适用条件一条：**这张票的 diff 全部落在 `apps/admin-web/**` 与它自己的票面里。** 碰到 Go / SQL / `ci.yml` / 领域文档的任何一行，整张票回到[并行会话](./parallel-sessions.md)那条路。

另开一条路的理由：那条路上的每一样——隔离树、`-p 1` 全量、占号、cherry-pick 重放与 SHA 对照、`merged/` 改名、tasks.md 叙事——病根都在后端（六个会话共享一只 55432 互相锁等、未提交现场被共享树抹掉）。admin-web 一件都不沾：三道门十秒、node:test 纯逻辑、CI 的 `admin-web` job 与 Go 分片互不相干。09-20 至 09-21 用那条路落五张前端票的实测：集成时延中位 25 分/票；wsform/03 的接线写完到进 main 隔了 15 小时，其中没有一分钟在改代码。一个 agent 在 `main` 上顺序做，比五个通道加一个推送方快。

1. **就在共享树的 `main` 上做**——不 `worktree add`、不另起分支；票面 Status → in-progress 随第一笔一起提。 
 **完成**：第一笔已推。
2. **做一条提一笔，提了就推**（`git push origin main`）。这是从旧路保留的唯一纪律，理由是事故不是流程：wsform/03 那 15 小时的直接原因是作者写完接线没 `commit` 就掉线。 
 **完成**：`git status` 干净，`main` 与 `origin/main` 同 SHA。
3. **门** = 三道门（命令与口径见[本机环境](#本机环境)「三道门禁与 CI `admin-web` job 同口径」那条）+ 票面自己的完成判据。要证事件路径（keydown → state → 重渲）用 `apps/admin-web/scripts/dom-probe.mjs`，探针源放仓外，结论写进完成记录。 
 **完成**：三道门退 0；判据逐条标 ✅ / ◑ / ❌ 并附证据。
4. **CI 是第二双眼睛**：push 后看 `gh run list -b main -L 1`；红了在 `main` 上再提一笔修。 
 **完成**：最新 run `success`。
5. **评审只在碰共享面时要**——`templates/*`、`shell/*`、`components/*`：一份 Spec 轴，非作者优先，无人就自审并在票面写「自审」。业务页（`pages/**`）自查判据即可。 
 **完成**：共享面票的 Comments 有评审或「自审」两字。
6. **收工**：票面 Status → resolved + 一段完成记录（落点 / 判据逐条 / 未验），spec 子票表同步，随最后一笔提。台账就是 `git log`。 
 **完成**：票面与子票表状态一致。
...
- **`apps/admin-web` 的 `pnpm build` 若断在 PostCSS 装载阶段，坏的是这台机器上的那次安装，不是源码。** 症状是 `Loading PostCSS Plugin failed: Cannot find module '@alloc/quick-lru'`（另一次是 `picocolors`），都是 tailwind 的传递依赖、lockfile 里都有，断点在**编译任何应用源码之前**，所以对谁的改动都一视同仁，别读成自己刚写的页面有问题。修法是删掉 `node_modules` 后 `pnpm install --frozen-lockfile`——2026-09-16 起这一步在任何全新检出上都能过：`@idpxyz/ui-*` 七个 tarball 随仓放在 `apps/admin-web/vendor/idpxyz-ui/`，`package.json` 的 `pnpm.overrides` 钉到那些文件（此前 lockfile 指向别的宿主上的 `file:/home/tops/idp-ui-tgz/*.tgz`、而 `package.json` 没有 overrides，`--frozen-lockfile` 因 `ERR_PNPM_LOCKFILE_CONFIG_MISMATCH` 装不了，本机 `node_modules` 只能是半残的一份）。**不带 `--frozen-lockfile` 的 `pnpm install` 仍会重写 lockfile**，那是[并行会话](./parallel-sessions.md)点名的「扫当前树再写回去」的全仓派生态命令——只在有意换依赖时跑，且换 `@idpxyz/*` 版本要连 tarball、overrides、lockfile 三件一起换（见 `apps/admin-web/README.md`）。
  **三道门禁与 CI `admin-web` job 同口径**：`node node_modules/typescript/bin/tsc -b --noEmit`、`node scripts/run-tests.mjs`、`pnpm exec vite build`。tsc 走入口文件而不是 `pnpm exec tsc`——`.bin/tsc` 垫片在半残安装下会缺失，报 `Command "tsc" not found`，看着像没装 TypeScript（实际装着 5.6.3）；`npx tsc` 更坏，缺包时会落到同名占位包上，退 0 却什么都没编，不能当证据。
- **Cursor 的子代理（`Task` 工具，`/code-review` 的隔离评审子代理走的就是它）自 2026-09-14 起在本机一律 `Authentication error. If you are logged in, try logging out and back in.`**，09-16 11:5x 用一个不读不写的最小 explore 子代理烟测仍原样复现，三天里 sa-cc/11、12、13、32 四张票的评审门都因此退到推送方自审。这是 Cursor 账号会话层的事，仓里没有任何东西能修它。识别标志是子代理在第一步就死、transcript 只有那一行——不要据此怀疑 skill 装没装或票面写错了什么。**用户 2026-09-16 裁定：本仓以后一律不用子代理**，不必再去修它、也不要拿它烟测。skill 正文里凡写「起隔离子代理」的步骤（`/code-review` 两轴、`/implement` 的评审段、`/research` 的独立 worker）都改在主会话里串行做；评审门按[并行会话](./parallel-sessions.md)办——有空闲通道就点名派非作者通道评，没有就推送方自审并在票面如实写「推送方自审，不算非作者评审」。
...
- **写多行提交消息用 `git commit -F` 加**单引号**here-string，双引号那种会吃掉反引号。** `@" … "@` 是可扩展 here-string，反引号在里面是转义符：`` `resolutionOrder `` 里的 `` `r `` 被当成回车写进消息，整行从此断在那里；`` `CommercialBasisQuery `` 之类没撞上转义名的只是丢掉两个反引号。2026-09-04 又中了一次，形状是后一种：MCP-1 一条提交的标题里 `` `等待运营登记` `` 两侧反引号全没了，整条消息一个反引号都没剩，已 amend 为 `86ba464`（原提交 `15b90b0` 只在对象库里可查，两份标题按字节对照只差那两个反引号）。而本仓的提交消息几乎必然引到代码符号，反引号是默认写法。用 `@' … '@`（单引号 here-string，字面量、不做任何替换），再 `[System.IO.File]::WriteAllText($path, $msg, (New-Object System.Text.UTF8Encoding $false))` 写盘、`git commit -F $path`。**写盘那一步同样别用 `Out-File -Encoding utf8`**：Windows PowerShell 5.1 写的是带 BOM 的 UTF-8——与 `Set-Content -Encoding utf8` 那条是同一个 BOM——而 `git commit -F` 把它原样当作标题首字收进去。同日 MCP-1 另一条提交的标题以 `EF BB BF` 三个字节开头，`git log --oneline` 看不出来，按字节读提交对象才露出 U+FEFF，已 amend 为 `8ab9835`（原提交 `40f5f4d`）。**别用 `git commit -m` 写多行中文**：换行处会另有一处损坏。识别标志是消息里出现半行截断或凭空多出的换行，而 `git commit` 本身退 0——反引号那桩 `git log --oneline` 还能看见，BOM 那桩连这一格也没有。
...
- **`gofmt -l .` 在本机会误报，而误报的方向是「有活要干」。** 它读**工作副本**，而本机新写或未经检出的文件带 CRLF；`.gitattributes` 是 `*.go text eol=lf`，入库仍是 LF，ubuntu 上检出即 LF，CI 的 `test -z "$(gofmt -l .)"` 不会红。**临时 worktree 里不会误报**，因为那是一次全新检出——这是「推前在临时 worktree 里验」的第二个理由，与它免疫于别人未提交改动那条并列。它与「与宿主无关」一节 `gofmt -l` 退出码那条不同属：那条说的是**它不用退出码说话**，这条说的是**它说出来的那句话本身可能不成立**。
  ...
  **CRLF 那一半的来源点得出名：Cursor 的 `Write` 工具在本机新建文件时逐行写 `\r\n`。** 探针实测于 2026-09-04：用它在仓库外新建一份 `.go` 与一份 `.sql`，字节里 CR 数与 LF 数相等、无 BOM；`gofmt -l` 点名那份 `.go`，`gofmt -d` 的删行数与增行数都等于文件行数——整文件差，而内容一字未动；`gofmt -w` 之后 CR 归零。同一天在临时 worktree 上实测，`eol=lf` 让这台机器上的**检出**也是 LF，所以主树 `git ls-files --eol` 里凡是 `w/crlf` 的，都是被工具写过而没再检出的那些，不是 Windows 检出的常态。

  顺带一格不属这一族但同源：**`.sql` 写成 CRLF 是真问题，不是误报**。迁移校验和按字节算，CRLF 会让已施加的库在干净检出下报漂移；`migrations/eol_guard_test.go` 的 `TestEmbeddedMigrationAssetsCarryNoCarriageReturnOrBOM` 读的是嵌入进二进制的工作副本字节，所以 `Write` 新建的迁移文件不归一就在本机直接红，BOM 一并拦。`.sql`、`.md` 这些 `gofmt` 管不到的文件，归一用与 `Set-Content` 那条同一套读写——`[System.IO.File]::ReadAllText($p, (New-Object System.Text.UTF8Encoding $false))` 读、``-replace "`r`n","`n"`` 、`WriteAllText($p, $text, (New-Object System.Text.UTF8Encoding $false))` 写回，只碰文件，不动索引。2026-09-04 当天的新文件都这么过了一遍：MCP-1 的 `operator_registration_queue.go` 与 `0013_operator_registration_queue.sql`、MCP-6 新建的每一份 `.sql`、MCP-3 的 `0016_audited_payable_and_credit_note.sql`；事后 `git ls-files --eol` 对它们给的是 `i/lf w/lf`。

...
- **Cursor 的子代理（`Task` 工具，`/code-review` 的隔离评审子代理走的就是它）自 2026-09-14 起在本机一律 `Authentication error. If you are logged in, try logging out and back in.`**，09-16 11:5x 用一个不读不写的最小 explore 子代理烟测仍原样复现，三天里 sa-cc/11、12、13、32 四张票的评审门都因此退到推送方自审。这是 Cursor 账号会话层的事，仓里没有任何东西能修它；处置只有登出再登入 Cursor（或核账号对子代理的权限）。识别标志是子代理在第一步就死、transcript 只有那一行——不要据此怀疑 skill 装没装或票面写错了什么。修好之前，评审门按[并行会话](./parallel-sessions.md)「没有可用通道时推送方自己审」办，并在票面如实写「推送方自审，不算非作者评审」。

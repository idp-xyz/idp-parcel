...
- **`apps/admin-web` 的前端门禁在本机只剩一道，而缺的那道不是坏在源码上。** `pnpm build` 必然失败，报 `Loading PostCSS Plugin failed: Cannot find module '@alloc/quick-lru'`，断在 tailwind 的 PostCSS 装载阶段——**在编译任何应用源码之前**，所以它对谁的改动都一视同仁，别读成自己刚写的页面有问题。判它是环境而不是源码：在未改动的树上同样复现，且 `node_modules\.pnpm\@alloc+quick-lru*` 实测不存在。**而 lockfile 里是有的**（`@alloc/quick-lru@5.2.0`，作 tailwind 的传递依赖），所以坏的是这台机器上那次安装，不是依赖声明。
  **修它没有便宜路子，别顺手跑一句 `pnpm install`。** `--frozen-lockfile` 会被 pnpm 直接拒绝（`ERR_PNPM_LOCKFILE_CONFIG_MISMATCH`）：`apps/admin-web/pnpm-lock.yaml` 的 `overrides:` 段把 `@idpxyz/ui-*` 六个包指向 `file:/home/tops/idp-ui-tgz/*.tgz`——**Linux 绝对路径**，那份 lockfile 是在别的宿主上生成的；而 `apps/admin-web/package.json` 里根本没有 `overrides`，两边对不上。不带 `--frozen-lockfile` 装则会**重写 lockfile**，把那几行私有包来源换成这台机器解析得到的东西。那是[并行会话](./parallel-sessions.md)点名的「扫当前树再写回去」的全仓派生态命令，共享树上不得单方面跑：先约定谁跑、什么时候跑，并且要有人认领「换掉那六行 `file:` 来源之后私有包还取不取得到」这个问题。
  **因此 `tsc --noEmit` 是前端仅剩的门禁，而它自己也有一个坑**：`node_modules/.bin/tsc` 垫片缺失，`pnpm exec tsc` 报 `Command "tsc" not found`，看着像没装 TypeScript（实际装着 5.6.3）。走入口文件：`cd apps/admin-web; node node_modules/typescript/bin/tsc --noEmit`。

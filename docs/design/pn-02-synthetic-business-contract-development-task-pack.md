...
- 控制请求只能消费 `UNIQUE_RESOLVED` 的商业依据。预付/账期模式、币种、范围、策略版本和当前修订必须与该依据完全一致。结算账户不由商业依据携带：[`settlement-accounting`](../domain/settlement-accounting/CONTEXT.md) 的结算账户固定责任法人、结算相对方、收付方向和结算币种，由结算侧按该依据解析并回校，[`party-commercial`](../domain/party-commercial/CONTEXT.md) 的结算政策只规定币种、方式与允许归集范围。调用方不能覆盖模式、指定或借用其他范围的账户，也不能使用客户级默认值。

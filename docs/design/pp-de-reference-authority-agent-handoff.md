...
状态：**本交接已收口。** 路径 A（supersede ADR）、B（抬升已定案 OPEN 项）、C（誊金样例）、D（scrub 活文档，含批量抹名）均已执行完毕，参考树已于 2026-08-10 经人类确认后删除。本文件转为历史记录，保留是为了让后来者能查到删除时依据的是什么核验。本文件是工作摘要，不是新权威规则。规则仍以各 `CONTEXT.md` / ADR / 用例为准。

日期：2026-08-07（A+B+C+D 于同日执行，通道 `idp-mcp-1`）  
删除参考树：2026-08-10（通道 `idp-mcp-3`）  
上一会话通道：`idp-mcp-4`  
仓库：`idp-parcel`
...
参考树已删除，该禁令随之失效；下面第 7 节保留了删除时逐条核验的记录。
...
6. **删参考树前验证** — **已完成，参考树已于 2026-08-10 删除**  
   - ✅ 案例保真比对已在删除前复跑，这是删掉之后再也执行不了的核验。按本仓转录本自述口径（两边案例按 `case_id` 排序、规范化为紧凑 JSON 后逐例比对）：136 例对 136 例全部一致、0 处差异，四条 `SRC-DISC` 声明一致，与转录当时记录的结果相同。比对程序为一次性工具，写在仓库外，未入库。  
   - ✅ 无活文档可点击进参考树（仅存 ADR-0011 正文那三条，已成断链，属预期）；`go vet` 与 `go test ./...` 绿；人类确认已取得。  
   - ✅ 参考树下无未跟踪文件，删除不丢失任何未入库内容；未转录的 `fixtures` 已归档在 `docs/archive/reference-design-rate-card-extraction-v1.0.1.json`。  
   - 目录的真实路径是 `foo/`，删除指令按真实路径执行，这不是抹名遗漏。28 个文件全部经 git 跟踪，删除进入历史而非丢失。
...
| 参考树（已删） | 原 `foo/`（28 文件）；案例与价卡抽取均已落库，于 2026-08-10 删除，内容见 git 历史 |
...
A 至 D 与删除参考树均已完成，本主题没有待办。下一任若接手相关工作，只需知道两件事：
...
2. 仍然开着的只有本主题之外的两项：`SRC-DISC-001` 至 `004` 的业务裁决，以及吸收台账登记的两条「未识别」能力（周期结算的佣金、追溯调价的影响分析）。两项都不依赖已删除的参考树。

...
状态：**本交接已收口。** 路径 A（supersede ADR）、B（抬升已定案 OPEN 项）、C（誊金样例）、D（scrub 活文档，含批量抹名）均已执行完毕。参考树**不删除**：产品决定保留一份可回查的一手交付物，2026-08-10 从根目录 `foo/` 移入 [`docs/reference/reference-design/`](../reference/reference-design/) 并改名，目录内 `README.md` 承担「非权威」抬头。本文件转为历史记录，保留是为了让后来者能查到当时依据的是什么核验。本文件是工作摘要，不是新权威规则。规则仍以各 `CONTEXT.md` / ADR / 用例为准。

日期：2026-08-07（A+B+C+D 于同日执行，通道 `idp-mcp-1`）  
参考树迁址改名：2026-08-10（通道 `idp-mcp-3`）  
上一会话通道：`idp-mcp-4`  
仓库：`idp-parcel`
...
| 判断 | 说明 |
|---|---|
| 参考树定性 | 外部参考输入，不是规格、不是权威；定性见 [ADR-0012](../adr/0012-parcel-pricing-context-within-idp-parcel.md) |
...
不要擅自删除参考树。产品已决定保留它，只是迁址改名；下面第 7 节保留了当初为删除所作的逐条核验。
...
6. **删参考树前验证** — **核验已全部完成；产品最终决定不删，改为迁址改名**  
   - ✅ 案例保真比对已复跑。按本仓转录本自述口径（两边案例按 `case_id` 排序、规范化为紧凑 JSON 后逐例比对）：136 例对 136 例全部一致、0 处差异，四条 `SRC-DISC` 声明一致，与转录当时记录的结果相同。比对程序为一次性工具，写在仓库外，未入库。  
   - ✅ 无活文档可点击进参考树（仅存 ADR-0011 正文那三条，红线禁止改写，迁址后仍是断链，属预期）；`go vet` 与 `go test ./...` 绿。  
   - ✅ 参考树下无未跟踪文件；未转录的 `fixtures` 已归档在 `docs/archive/reference-design-rate-card-extraction-v1.0.1.json`。  
   - **结论改变：删除的内容前提已全部成立，但产品选择保留一份可回查的一手交付物。** 目录从根目录的 `foo/` 移入 `docs/reference/reference-design/`，`foo` 这个代号自此在工作树中消失。保留的代价是必须靠目录内 `README.md` 的非权威抬头挡住误读——本条门槛因此从「删除前核验」转为「长期维持抬头有效」。
...
| 参考树（保留，已迁址） | `docs/reference/reference-design/`（28 文件 + 非权威抬头 `README.md`）；原路径为根目录 `foo/`，2026-08-10 迁入并改名 |
...
A 至 D 已完成，参考树的去留也已定案（保留并迁址），本主题没有待办。下一任若接手相关工作，只需知道三件事：

1. 本文件已是历史记录，不要按它派活。参考设计的定性以 [ADR-0012](../adr/0012-parcel-pricing-context-within-idp-parcel.md) 为准，案例语料以[转录本](./pp-golden-case-transcript.md)为准。  
2. 参考树仍在仓库里，位置是 [`docs/reference/reference-design/`](../reference/reference-design/)。它非权威这一点不因保留而松动：任何规则若只能靠它读懂，仍属缺陷，处置办法是把内容补进权威文档。  
3. 仍然开着的只有本主题之外的两项：`SRC-DISC-001` 至 `004` 的业务裁决，以及吸收台账登记的两条「未识别」能力（周期结算的佣金、追溯调价的影响分析）。两项都不依赖参考树。
...
- 未修改参考树内任何文件。迁址改名只动路径，28 份交付物内容一字未改。  
- 未取得 `SRC-DISC-*` 业务签字。  
- 未改变证据层级或试点 `Go/No-Go`。  

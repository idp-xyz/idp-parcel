...
`PBC-08` 的行为面已在全部持久化写口上收口取证（`ccbf79e`，bento-gate-reeval 票 02）：每个写方法——方法体引用 `RequireExecutor`，或经 `EnqueueOnce` 写出——在同包测试里有「引用 `ErrTransactionRequired` 且以该类型接收者调用该方法」的负向证据；无事务 context 上返回框架的 `ErrTransactionRequired` 而非改用连接池，被拒的写入不落库。该性质随适配器逐个成立，行为本身没有静态门禁能替它把关，因此**每新增一个持久化适配器都要自带这条证明**——而「证明在不在场」这一格由 `internal/architecture` 的 PBC-08 行为面门禁常驻核对（归属按接收者变量类型解析，捆绑夹具不再产出假缺），漏带证明的新写口会让全仓测试变红，不再依赖一次性审计脚本。

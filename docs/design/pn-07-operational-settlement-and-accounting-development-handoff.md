...
- 使用同一版本的商业价格政策、SELL/BUY 价卡、计价策略、输入快照和 `PricingEvaluation` 验证评价可复算、BUY/SELL 隔离、费用项目映射、解释和回放；`foo` 的 Golden Case 只有在 Schema、源价卡文件名和哈希证据修复后才能作为正式回归基线。
...
`foo` 导入的合成案例只能用于稳定骨架和隔离 `S` 验证（包括重放确定性）；没有真实历史来源时不得标记为 `R`。当前 Golden JSON 的 Schema 指针和部分源价卡文件名/哈希存在不一致，41 个 `SOURCE_RATE_CARD` 案例必须隔离，修复前不得把任何金额样例标记为生产验收证据。

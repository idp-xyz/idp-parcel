...
- `parcel-pricing → settlement-accounting`：计价提供不可变 `PricingEvaluation`、计价输入/版本清单、费用行、计价重量结果和解释；结算据此按费用确认条件和责任范围形成计费重量采用快照、费用、预期成本、应收应付、调整、对账和核销，不能把评价金额直接当作金额责任，也不能发布或改写价卡。

...
**四、不改解析结果的判定。** 产品缺席不使解析从`唯一解析`退化为`无适用依据`。

两条理由。浅的一条是波及面：那会改变每一个解析产品的既有闭包（含 `parcel-shipment` 的接受闭包）的结果，远大于要修的缺口，而「绝不默认」这条保证由第三条与 `ServiceProductForm` 零值不合法共同交付，不需要靠退化解析来实现。

深的一条是**一份讲可观察性的记录不该顺手改掉判断结果**。本仓的规矩是发布后不覆盖、变化形成新版本；把「让形态看得见」与「让解析在新条件下失败」捆进同一份 ADR，将来回溯的人分不清被接受的是哪一条。真要改判定，那是另一份记录的事。

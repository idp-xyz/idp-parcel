...
- 过渡态解除 2026-09-10（票 [sa-preacceptance-policy-view/04](../../.scratch/sa-preacceptance-policy-view/issues/04-authorized-disposition-flow-and-failure-disposition-read.md) 落地，[ADR-0132](./0132-authorized-disposition-decides-the-destination-of-a-restricted-request-as-its-own-wait-state-and-never-passes.md)）：`RESTRICTED` 不再一律拒绝——受限项在形成控制判断那一步经 PS 自己的商业缝读回正文登记的失败处置与责任引用、作为采用引用落库，`FinancialControlCheckFor` 按它分路（任一 `REJECT` → 未通过照旧；全部 `AUTHORIZED_DISPOSITION` → 无法判定 + `等待授权处置`）。决定一「受限项只带自己的原因」的形因此扩了两格采用引用（ADR-0132 越权风险点 3），本正文不改，以 ADR-0132 决定三为准；决定五留下的读路已建在 `internal/parcelshipment/adapters/partycommercial` 的处置读口。上一条「认可为过渡」自此失效，不删只记。
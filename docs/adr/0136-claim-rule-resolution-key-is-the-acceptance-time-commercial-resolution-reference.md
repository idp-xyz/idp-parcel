...
2. **PS 解析键登记面对两类依据的校验强度。** `commercial_resolution_keys.go` 今天只在请求结算依据时强制合同维（UC-PC-002「请求结算依据时还必须包含能够区分合同…的业务维度」）；按 PC CONTEXT「委托被接受时固定其适用的…客户合同版本」合同应恒在，客户服务规则则「可登」。要不要把两者改成登记面上的「必登」归 PS owner；本记录只要求「不在闭包里就按各自那一格答」，不改 PS 登记面。**补记 2026-09-14**：上句「可登」在 `262e8c0a`（2026-09-11，ve-claims/04 落地基线；下述两道闸自本记录接受的 `2a788853` 起到那时零 diff）上实测**不成立，是不可登**——迁移 `parcel_shipment/0007` 立、`0008` 重加的 CHECK `commercial_resolution_key_registration_bases_closed` 白名单没有 `CUSTOMER_SERVICE_RULE`，PS `adapters/partycommercial` `commercialKindFrom` 名集也没有 `CustomerServiceRuleObject`，行进不了库、进了库接受时成键也报 unknown kind（实测出自票 [ve-claims/04](../../.scratch/ve-claims-read-seams/issues/04-rule-resolution-key-source-needs-a-registration-face.md)「裁决」5）。两道闸已由 [ps-port-remainder/09](../../.scratch/ps-port-remainder/issues/09-resolution-key-face-does-not-accept-customer-service-rule.md) 开：迁移 `0022_resolution_key_bases_accept_customer_service_rule.sql` 重加同名约束把 `CUSTOMER_SERVICE_RULE` 加进白名单、`commercialKindFrom` 加一格（`9fa2ddc6`，2026-09-12 进 main）——自那之后「可登」成立；「必登否」仍归 PS owner，未裁。本补记只订正前提，不改本条的决定。**补记 2026-09-14（二）**：「必登否」已裁 (a) 不必登（用户授权推送方以 PS owner 口径代裁，理由见 [ps-port-remainder/09](../../.scratch/ps-port-remainder/issues/09-resolution-key-face-does-not-accept-customer-service-rule.md)「要裁的」1）——一个租户可以合法地没有任何客户服务规则版本（ADR-0104 决定三），登记面不因此拒登；本条「不在闭包里就按各自那一格答」照旧。

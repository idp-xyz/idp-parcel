...
**本节是快照，盘于 `0410e2f`（2026-08-11）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，此后按下述触发条件反复重盘，本轮为第九轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
- **应用层十例分布在四个上下文，端口无生产实现。** `parcel-shipment` 七例（提交、可达性与财务控制两条腿的推进、形成决定、主动拒绝、客户撤回、资料修订）、`party-commercial`（`UC-PC-002` 第一阶段）、`network-routing`（`UC-NR-002`）与 `settlement-accounting`（`UC-SA-002` 接受前财务控制分支）各一例，`parcel-pricing` 仍只有 `domain` 包；`judgment_continuation.go` 是 `parcel-shipment` 包内共用的续办摘要，不是用例，不计入。端口是纯接口，今天唯一的实现是测试用替身，PostgreSQL 适配器仍被 ADR-0017 的持久化闸门挡住。前九例复现同一套编排形状：租户显式入参而不从 context 里补、最小身份先于任何权威读取、判断时间由应用层从时钟取且与业务时点分开、依赖失败形成本上下文自己的未决结果而不上抛技术错误、未决结果携带封闭原因与续办引用。样本翻倍没有推翻这五条，只添了一条**并列**而非例外：决定已成立而随附补偿未完成时，补偿续办引用与判断续办引用分开交回——前者续的是补偿，后者续的是尚未形成的判断，合成一个会让调用方分不清该重判还是该重放。第九例（客户撤回）同样没有推翻它们，却改正了其中一条此前三个决定处理器都做错的：未决结果里的委托状态原先硬写`已提交`，而走依赖答不出那条路时委托已经取到手，真实状态可能是`已接受`或`已撤回`。「未决要交回已知事实」意味着状态也得是读到的那一个，取不到委托时传零值而不是编一个。第十例（资料修订）是头一个在这五条上**不齐**的：依赖答不出时它直接上抛技术错误，而另外九例都形成本上下文自己的未决结果并带续办引用。这是缺口不是分歧——`UC-PS-002` 的业务未决结束明写「规则或授权无法确定」属未决，它没有另一套领域语言要表达，只是还没做。
...
- **四条编排出口是技术错误而非业务取值**：形成决定、主动拒绝、客户撤回与资料修订在委托查不到时上抛，后两条各有两处，重放路径上走的是同一出口，因此共六处。这是有意的——指名一份查不到的委托是调用方的错，给它业务取值会让编程错误混进未决统计。但接 HTTP 时它必须映射为 CONTEXT 的`统一不可见结果`，因为 `FindBySourceIdentity` 的否定结果不区分「不存在」与「属于别的租户」；六处 `!found` 分支里只有三处旁边就地记下了该约束，另三处（撤回与资料修订的重放路径、资料修订的主路径）没有，补 HTTP 面时容易漏掉。

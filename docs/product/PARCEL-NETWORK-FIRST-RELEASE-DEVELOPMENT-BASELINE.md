...
**本节是快照，盘于 `90fe12a`（2026-08-11）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，此后按下述触发条件反复重盘，本轮为第十二轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
**受控案例可复算——三条里最接近满足的一条。** `parcel-pricing` 有 26 份测试，覆盖规范化摘要回放、合成契约与逐案例断言；`parcel-shipment` 有 27 份，含 `SYN-CHAIN` 联检与 `UC-PS-001` 编排的逐结果断言；`party-commercial` 有 22 份，其中两份是契约测试（数它时当心 `customer_contract_test.go`：那是客户合同这个领域概念的测试，不是契约测试，按文件名数会多出一份）；`settlement-accounting` 有 5 份、`network-routing` 有 4 份。CI（`.github/workflows/ci.yml`）在每次 push 与 PR 上跑 `gofmt`、`go vet`、`go test -race` 与 `go build`，因此可复算有持续保证，不是一次性事实。
...
- **HTTP 面只有 `/healthz` 与 `/version`**（`internal/platform/httpapi/router.go`），没有任何业务端点。补这一项时必须一并考虑 [ADR-0021](../adr/0021-frontline-operations-client-is-part-of-the-product.md)：一线作业客户端的离线与弱网容忍会约束端点的幂等、冲突合并语义与重放形状，那是设计不出来就补不上的一类要求。**四项里只有这一项现在没有闸门挡着**——ADR-0017 说它「在应用层成型后自然解除」，而应用层已成型到十例。另两项仍卡在 Bento 持久化闸门。

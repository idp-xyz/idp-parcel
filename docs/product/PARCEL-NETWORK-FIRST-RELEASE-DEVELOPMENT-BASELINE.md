...
- **无事件机制**，无 outbox 或发布侧实现。发布侧此后有了第一个端口接口——`UC-PS-002` 步骤 9 的 `SourceDataVersionHandoff`，按 ADR-0017「端口接口属机制半边」定义，唯一实现仍是测试替身。它不解除本项：机制半边缺的从来不是接口而是**事务发布**，而「发布意图与业务结果同一提交」这条要求 `UC-PS-001`、`UC-SA-002`、`UC-TF-004`/`006`、`UC-CC-009` 全都写着，无一处得到证明。第一条缝的形状（一份结果发一份意图、意图由结果标识认领、重放重发同一份）已有两个样本：`SourceDataVersionHandoff` 与 `UC-PS-001` 决定发布的 `AcceptanceDecisionHandoff`（`AT-PS-013` 的意图半边），第二个样本一字未改地沿用了第一个的形状。两个样本仍都在 `parcel-shipment` 之内，故不在此升为跨上下文口径；第三个上下文采纳时再议。

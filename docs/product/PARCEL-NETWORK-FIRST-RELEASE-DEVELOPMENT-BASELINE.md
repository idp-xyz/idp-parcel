...
|---|---|---|---|
| PN-02 | `UC-PC-001/002`、`UC-PS-001/002/005`、`UC-NR-002` | 商业对象发布、两阶段唯一解析、接单机制、建单前来源重放/冲突、最小委托候选、决定前撤回和同租户提交批次边界已形成；`已提交`聚合与归属端口已实现，`UC-PS-001` 走完到接受决定，`UC-PS-005` 决定前撤回与客户原始资料版本已有代码但都不完整——撤回缺步骤 1 的来源保全（`AT-PS-069` 因此落不了地），资料版本只有领域层、尚无应用编排；生产持久化和事件事务尚未实现，真实商业版本、规则包、财务策略、角色、时点和试点参数仍待提供 | 按 [`PN-02` 真实参数取证与开发交接](../design/pn-02-real-parameter-evidence-and-development-handoff.md)并行落实稳定骨架、参数未配置和真实取证，保持“商业唯一解析 → 可达/财务判断 → 接受、拒绝或撤回”的决定边界 |
...
**本节是快照，盘于 `950c1f3`（2026-08-10）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，本节此后已按下述触发条件重盘五轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。

...
**本节是快照，盘于 `e86c90b`（2026-08-13）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，此后按下述触发条件反复重盘，本轮为第十九轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
| PN-07 | `parcel-pricing`、`settlement-accounting` | 部分 | `parcel-pricing` 规则模型七步全部有执行器、特征来源十项闭合，并有评价编排（同标识冒名冲突用纯函数重算比语义摘要、失败评价照样入册交付、编排零算术）；`settlement-accounting` 全链领域面就位：接受前冻结/信用暴露、供应商预期成本（发生项+协议+BUY 评价三方合成、跨币种换算只采评价内步骤）、客户费用三段演进与调整封闭二值（另有确认编排：条件未配置不默认转正、确认不改金额）、供应商账单（到达≠应付、匹配五格、审核成应付、贷项不净额）、账单接收编排、客户对账单（截单快照、发布密封、后续纳入不回填）、外部资金映射与核销（映射须显式依据、分配守恒、撤销一次）、赔付与追偿金额（责任结论成额、认可≠到账）、成本分摊与经营结果（无规则不分摊、指标只派生）、代垫评估与客户回收。编排四例对应 `UC-SA-002/004`；`UC-SA-001/003/005/006/007` 五票领域件在而编排为零（第十八轮曾误记七 UC 全编排，本轮文件级核对更正）；剩该五票编排与三闸 |
| PN-08 | 跨上下文治理编排（`pilotgovernance`） | 部分 | 记录能力与编排两例同 PN-01 行；候选版本组、阶段决定、权威区间、暂停恢复、接管与在途盘点全部有代码、编排与测试。真实候选、证据包与阶段决定属实例半边 |

**八个切片全部为部分，未开始清零。** 十个上下文全部有生产代码与应用编排（`internal/` 下 `parcelshipment`、`partycommercial`、`networkrouting`、`parcelpricing`、`settlementaccounting`、`nodeoperations`、`transportfulfillment`、`visibilityexception`、`customscompliance`、`pilotgovernance`，另有 `platform` 与 `architecture` 两个非业务目录）。`docs/application/` 下四十八个 UC（另有一份 `UC-PS-001` 配套商业简报不是独立 UC）中四十二个已有对应编排触点；剩六个是「领域件在而编排为零」：`UC-SA-001` 代垫回收、`UC-SA-003` 对账单、`UC-SA-005` 收付核销、`UC-SA-006` 成本分摊、`UC-SA-007` 赔付追偿与 `UC-TF-001` 监管处置承接——第十九轮曾把这六个误记为已清零（凭上一段会话摘要而未做文件级核对），本轮评审文件级对照后更正。十个跨上下文适配器落在消费方侧（第十个把 PC 的收寄资格与终局规则声明译成 PS 的两个规则视图）。除该六票外，没有一个切片达标的原因收敛为同一组横切缺口：持久化、事务发布与 HTTP 准入三道闸（ADR-0017/ADR-0022）加实例参数——那正是机制半边与实例半边的分界线。
...
- **无事件机制**，无 outbox 或发布侧实现。发布侧此后有了第一个端口接口——`UC-PS-002` 步骤 9 的 `SourceDataVersionHandoff`，按 ADR-0017「端口接口属机制半边」定义，唯一实现仍是测试替身。它不解除本项：机制半边缺的从来不是接口而是**事务发布**，而「发布意图与业务结果同一提交」这条要求 `UC-PS-001`、`UC-SA-002`、`UC-TF-004`/`006`、`UC-CC-009` 全都写着，无一处得到证明。这条缝的形状（一份结果发一份意图、意图由结果标识认领、重放重发同一份）在第三个样本（`network-routing` 的 `ReachabilityJudgmentHandoff`，`AT-NR-030` 意图半边）跨出单一上下文后已升为口径：[ADR-0043](../adr/0043-publish-intent-claimed-by-result-identity.md)，新采纳点引用它而不再各自复述理由。

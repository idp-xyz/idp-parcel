...
**本节是快照，盘于 `eaf6fee`（2026-08-10）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，本节此后已按下述触发条件重盘三轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
**受控案例可复算——三条里最接近满足的一条。** `parcel-pricing` 有 26 份测试，覆盖规范化摘要回放、合成契约与逐案例断言；`parcel-shipment` 有 16 份，含 `SYN-CHAIN` 联检与 `UC-PS-001` 编排的逐结果断言；`party-commercial` 有 22 份，其中三份是契约测试；`settlement-accounting` 有 3 份、`network-routing` 有 4 份。CI（`.github/workflows/ci.yml`）在每次 push 与 PR 上跑 `gofmt`、`go vet`、`go test -race` 与 `go build`，因此可复算有持续保证，不是一次性事实。

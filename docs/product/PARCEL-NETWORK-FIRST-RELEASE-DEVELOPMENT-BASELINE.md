...
**本节是快照，盘于 `03b0234`（2026-08-11）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，本节此后已按下述触发条件重盘六轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
**受控案例可复算——三条里最接近满足的一条。** `parcel-pricing` 有 26 份测试，覆盖规范化摘要回放、合成契约与逐案例断言；`parcel-shipment` 有 26 份，含 `SYN-CHAIN` 联检与 `UC-PS-001` 编排的逐结果断言；`party-commercial` 有 22 份，其中两份是契约测试（数它时当心 `customer_contract_test.go`：那是客户合同这个领域概念的测试，不是契约测试，按文件名数会多出一份）；`settlement-accounting` 有 5 份、`network-routing` 有 4 份。CI（`.github/workflows/ci.yml`）在每次 push 与 PR 上跑 `gofmt`、`go vet`、`go test -race` 与 `go build`，因此可复算有持续保证，不是一次性事实。
...
- **应用层九例分布在四个上下文，端口无生产实现。** `parcel-shipment` 六例（提交、可达性与财务控制两条腿的推进、形成决定、主动拒绝、客户撤回）、`party-commercial`（`UC-PC-002` 第一阶段）、`network-routing`（`UC-NR-002`）与 `settlement-accounting`（`UC-SA-002` 接受前财务控制分支）各一例，`parcel-pricing` 仍只有 `domain` 包；`judgment_continuation.go` 是 `parcel-shipment` 包内共用的续办摘要，不是用例，不计入。端口是纯接口，今天唯一的实现是测试用替身，PostgreSQL 适配器仍被 ADR-0017 的持久化闸门挡住。九例复现同一套编排形状：租户显式入参而不从 context 里补、最小身份先于任何权威读取、判断时间由应用层从时钟取且与业务时点分开、依赖失败形成本上下文自己的未决结果而不上抛技术错误、未决结果携带封闭原因与续办引用。样本翻倍没有推翻这五条，只添了一条**并列**而非例外：决定已成立而随附补偿未完成时，补偿续办引用与判断续办引用分开交回——前者续的是补偿，后者续的是尚未形成的判断，合成一个会让调用方分不清该重判还是该重放。第九例（客户撤回）同样没有推翻它们，却改正了其中一条此前三个决定处理器都做错的：未决结果里的委托状态原先硬写`已提交`，而走依赖答不出那条路时委托已经取到手，真实状态可能是`已接受`或`已撤回`。「未决要交回已知事实」意味着状态也得是读到的那一个，取不到委托时传零值而不是编一个。
...
- **无事件机制**，无 outbox 或发布侧。
- **HTTP 面只有 `/healthz` 与 `/version`**（`internal/platform/httpapi/router.go`），没有任何业务端点。补这一项时必须一并考虑 [ADR-0021](../adr/0021-frontline-operations-client-is-part-of-the-product.md)：一线作业客户端的离线与弱网容忍会约束端点的幂等、冲突合并语义与重放形状，那是设计不出来就补不上的一类要求。**四项里只有这一项现在没有闸门挡着**——ADR-0017 说它「在应用层成型后自然解除」，而应用层已成型到九例。另两项仍卡在 Bento 持久化闸门。
- **两条编排出口是技术错误而非业务取值**：形成决定与主动拒绝在委托查不到时上抛。这是有意的——指名一份查不到的委托是调用方的错，给它业务取值会让编程错误混进未决统计。但接 HTTP 时它必须映射为 CONTEXT 的`统一不可见结果`，因为 `FindBySourceIdentity` 的否定结果不区分「不存在」与「属于别的租户」；两处 `!found` 分支旁已就地记下该约束。

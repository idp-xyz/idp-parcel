...
|---|---|---|---|
| PN-02 | `UC-PC-001/002`、`UC-PS-001/002/005`、`UC-NR-002` | 商业对象发布、两阶段唯一解析、接单机制、建单前来源重放/冲突、最小委托候选、决定前撤回和同租户提交批次边界已形成；`已提交`聚合与归属端口已实现，`UC-PS-001` 走完到接受决定，`UC-PS-005` 决定前撤回与客户原始资料版本仍无代码；生产持久化和事件事务尚未实现，真实商业版本、规则包、财务策略、角色、时点和试点参数仍待提供 | 按 [`PN-02` 真实参数取证与开发交接](../design/pn-02-real-parameter-evidence-and-development-handoff.md)并行落实稳定骨架、参数未配置和真实取证，保持“商业唯一解析 → 可达/财务判断 → 接受、拒绝或撤回”的决定边界 |
...
**本节是快照，盘于 `cc65176`（2026-08-10）。** 它与本文其余部分性质不同：其余部分说明产品长期必须保持什么，本节只说明某一刻站在哪里。它记录的对象是代码，每次提交都可能使它过期——立节当天即被并行落地的提交反复带假，本节此后已按下述触发条件重盘四轮。**读到与代码不符时以代码为准**，不要据本节推断某项能力不存在。
...
**受控案例可复算——三条里最接近满足的一条。** `parcel-pricing` 有 26 份测试，覆盖规范化摘要回放、合成契约与逐案例断言；`parcel-shipment` 有 23 份，含 `SYN-CHAIN` 联检与 `UC-PS-001` 编排的逐结果断言；`party-commercial` 有 22 份，其中三份是契约测试；`settlement-accounting` 有 5 份、`network-routing` 有 4 份。CI（`.github/workflows/ci.yml`）在每次 push 与 PR 上跑 `gofmt`、`go vet`、`go test -race` 与 `go build`，因此可复算有持续保证，不是一次性事实。
...
- **应用层八例分布在四个上下文，端口无生产实现。** `parcel-shipment` 五例（提交、可达性与财务控制两条腿的推进、形成决定、主动拒绝）、`party-commercial`（`UC-PC-002` 第一阶段）、`network-routing`（`UC-NR-002`）与 `settlement-accounting`（`UC-SA-002` 接受前财务控制分支）各一例，`parcel-pricing` 仍只有 `domain` 包；端口是纯接口，今天唯一的实现是测试用替身，PostgreSQL 适配器仍被 ADR-0017 的持久化闸门挡住。八例复现同一套编排形状：租户显式入参而不从 context 里补、最小身份先于任何权威读取、判断时间由应用层从时钟取且与业务时点分开、依赖失败形成本上下文自己的未决结果而不上抛技术错误、未决结果携带封闭原因与续办引用。样本翻倍没有推翻这五条，只添了一条**并列**而非例外：决定已成立而随附补偿未完成时，补偿续办引用与判断续办引用分开交回——前者续的是补偿，后者续的是尚未形成的判断，合成一个会让调用方分不清该重判还是该重放。
...
- **无事件机制**，无 outbox 或发布侧。
- **HTTP 面只有 `/healthz` 与 `/version`**（`internal/platform/httpapi/router.go`），没有任何业务端点。补这一项时必须一并考虑 [ADR-0021](../adr/0021-frontline-operations-client-is-part-of-the-product.md)：一线作业客户端的离线与弱网容忍会约束端点的幂等、冲突合并语义与重放形状，那是设计不出来就补不上的一类要求。**四项里只有这一项现在没有闸门挡着**——ADR-0017 说它「在应用层成型后自然解除」，而应用层已成型到八例。另两项仍卡在 Bento 持久化闸门。

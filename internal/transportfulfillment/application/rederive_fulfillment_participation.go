...
// 领域正当拒绝两格各自答出去（无可替代、更正后的起点晚于继承的终点），其余领域错误（租户不符之类本编排
// 自己造不出的输入）不出声。撤回控制的交接更正不再是拒绝：领域门在链上长失效版本，它与替代版本同样经
// `Supersede` 落库、成功时两格都空（ADR-0112 决定四，票 tf-segment-lifecycle-closure/11 裁决 5）。
//

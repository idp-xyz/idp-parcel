...
var ErrInvalidOperationsQueryScope = errors.New("transport fulfillment: invalid operations query scope")
...
// OperationsQueryScope 是运输履约运营查阅的授权作用域（ADR-0077 Decision 二）：
// （作用域引用，租户）两维，两样必须非零。
//
// 没有班次维、也没有承运商维——不是「那两维可选」，是它们在这里不是查阅方身份：
// CONTEXT 把班次、容量池、运输方定为履约判断与事实的归属维，那是登记内容；运营
// 查阅的授权边界只有租户（租户是最高数据隔离边界，ADR-0003）。把归属维当授权维会
// 得到一个看起来更严实、实际把「查得到哪些事实」与「事实由谁承运」混成一格的作用域。
//

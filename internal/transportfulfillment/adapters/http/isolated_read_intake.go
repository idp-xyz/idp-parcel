...
// CatalogueQueryIntake——交付命令面的 DeliveryIntake 本类型刻意不实现，放行装不进
...
func NewIsolatedOperationsReadIntake(
	...
	if err != nil {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("transport fulfillment http: isolated read intake: %w", err)
	}
	...
	if limit <= 0 {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("transport fulfillment http: isolated read intake: limit must be positive, got %d", limit)
	}

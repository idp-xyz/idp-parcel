...
// 本文件承载运输履约查阅页的目录查阅接入形状（ADR-0077，票
// admin-skeleton-closure-batch/05）。查阅不建班次、不订舱、不改交接结果、不登交付
// ——查询端点只消费存储读面，不接应用编排（ADR-0077 Decision 一）；命令面的交付
// 两端点在 register_effective_delivery.go，两面共用本包的传输层错误码与答复助手，
// 但接入形状各自成型：命令要从请求体收设备签发的事实身份与时间（ADR-0023），查阅
// 对请求零采信。
//
// CatalogueQuery 是一次已授权的运输履约目录查阅（ADR-0077）：作用域来自认证与授权
// 结果，授权边界只有租户——班次、容量池与运输方是履约判断与事实的归属维，不是查阅
// 方身份（见 domain.OperationsQueryScope）；页大小由接入面按渠道契约裁决。两样都不
// 采信调用方自报。
...
//
// 它是接口而非解析代码：请求方身份与租户必须同时核对，运营接入面的认证归
// 操作者渠道（ADR-0100），其真 Intake 未就位；采信自报租户会穿透 ADR-0003 的隔离边界（ADR-0077 Decision
...
// 缺席是真话不是缺陷：不为「未更正」编造一个零时刻——零时刻是合法时间值，用它兼表
// 没发生会让两态在报文上分不开。

...
//   - 收件地点未定 → **取甲**（做法第 2 步，ADR-0130 越权风险点 1）：译成 RequirementMissing。**所有者说的是「未定」，
//     不是「没有」**——PS 此刻说不出该送哪一版（两条修订分叉等人来并），端口今天没有「未定」那一格，任务同样保持待形成、
//     续办引用非空；拍到分叉并掉之后同一拍重跑就拿到引用。要给端口加第三格得先证明时间窗与条件两条缝也用得上（票面
//     第 2 步的默认判据），届时改这里一格与 TF CONTEXT「派送要求」那句。

...
// **今天对全部快照两段都答「要素缺席」**：提交版本不携带地址要素（寄收件 name/value 条目只进 PayloadDigest），内容落库
// 归 pp-seams/05；那票落地后基线格开始带值，本口与答格不动。已采用版本锚那一格只交锚不交要素，理由同
// DeclaredMeasurementView（客户原始资料版本今天只留痕不留内容）。
//
// 值原样交：邮编格式与前缀粒度属实例半边（ADR-0109「随首份真实分区表定」），本上下文不校验、不规范化。五格里没有

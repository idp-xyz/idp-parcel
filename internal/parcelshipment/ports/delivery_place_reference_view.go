...
// 不拓宽 ShipmentRequestRepository 一类写口，判据同 ADR-0077 决定五（伴生读端口另立）；**不预设按委托或按引用
// 反查的方法**——按锚解析回地址内容的第二个消费方是 `parcel-pricing`，它的口是 AddressElementsView（pp-seams/03
// 裁决 4）：同样按（租户，包裹身份）问、不过授权查询作用域，只交邮编与国家 / 地区码两格值 + 锚、不交地址文本；本口
// 的四格答法与 DeliveryPlaceReference 四段形不因它而动。
//

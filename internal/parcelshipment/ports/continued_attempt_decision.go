...
// 货主、渠道服务方或代理商只能提出请求（CONTEXT Rules），它不带来任何授权，**也不进提供方**——适配器折
// 提供方请求时，请求方取的是实例坐标里的运营角色代询问身份上的客户账户（`formContinuedAttemptDecisionRequest`），
// 本字段今天没有任何适配器读它。它的用处在本上下文的写面：落在决定上，重开时核「与原关闭请求方同一
// 货主账户」。关闭这一格允许缺席，重开与关闭同型只为一口。
//

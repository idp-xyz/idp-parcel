...
// 或确定不适用），还是根本没得出答案。
//
// 它刻意不是 party-commercial 结果代数的副本。那边的非唯一取值有六种——`无适用依据`、
// `适用冲突`、`解析未决`、`输入未受理`、`已失效`、`依据未解析`——那是它的语言；本上下文只需要
// 接受条件矩阵自己的那两列，确定性不通过与无法判定。究竟是哪一种由原因引用带过来，不在
// 这里重新声明一套口径。
//
// 压缩只发生在校验结果这一层。编排另按原因取各自的未决原因与续办引用，否则一次越权探测
// （`AT-PC-028`，ADR-0029 之后落在`依据未解析`）与一次权威读不到会共用一个引用，续办时催的
// 也是同一个人。

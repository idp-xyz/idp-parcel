// Package labelfinal 拥有面单渠道服务终局判断三路触发（TF 首次有效收寄到达、面单交易定案 / 后续动作
// 两拍、受控关闭 / 重开决定生效）**共用的处理方核**与那一套消费结论翻译（ADR-0134 决定二）。三路只有
// 译码与取事实那一层不同：面单交易那一路（lc/26）不带收寄事实，关闭 / 重开那一路（lc/27）也不带；
...
// ParcelJudgmentCore 是三路共用的核：按（租户 + 包裹）经当前已接受投影反查目标委托 → 折

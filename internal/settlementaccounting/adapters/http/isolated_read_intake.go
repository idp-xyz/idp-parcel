...
// CatalogueQueryIntake——本包没有命令面，将来长出命令 Intake 时本类型刻意不实现，
// 放行装不进命令端点由编译期决定。
//
...
func NewIsolatedOperationsReadIntake(
	...
	if err != nil {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("settlement accounting http: isolated read intake: %w", err)
	}
	...
	if limit <= 0 {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("settlement accounting http: isolated read intake: limit must be positive, got %d", limit)
	}

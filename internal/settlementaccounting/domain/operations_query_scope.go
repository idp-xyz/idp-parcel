...
var ErrInvalidOperationsQueryScope = errors.New("settlement accounting: invalid operations query scope")
...
// OperationsQueryScope 是结算与核算运营查阅的授权作用域（ADR-0077 Decision 二）：
// （作用域引用，租户）两维，两样必须非零。
//
// 没有结算账户维、也没有责任法人维——不是「那两维可选」，是它们在这里不是查阅方
// 身份：CONTEXT 把责任法人、结算账户、币种定为一份余额与一本对账册的归属维，那是
// 账上的登记内容；运营查阅的授权边界只有租户（租户是最高数据隔离边界，ADR-0003）。
// 把归属维当授权维会得到一个看起来更严实、实际把「查得到哪些账」与「这些账归谁」
// 混成一格的作用域。
//
// 它是本上下文语言的一部分，不从别的上下文导入共享作用域类型：共享类型让边界在类型
// 上互相依赖——「每上下文自立，各自成形互不参数化」（ADR-0077 Decision 二）。

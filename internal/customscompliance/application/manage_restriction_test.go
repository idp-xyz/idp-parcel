...
// Covers: UC-CC-011 的编排面——同 ID 重放返原限制不重立（`AT-CC-345`「相同限制形成请求和相同来源、范围、动作、依据再次到达」→「返回已有限制结果，不创建第二限制或重置开始时间」）；
// 同 ID 异约束集是冒名冲突
...
// Covers: CC CONTEXT「只有作用于当前对象和拟执行动作的全部阻断性限制均已解除，相应动作才可继续」的编排面——两条限制同时约束出库时部分解除仍阻断（清单列全）；全部

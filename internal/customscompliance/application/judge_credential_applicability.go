...
// 凭证适用性判断——UC-CC-003 步 7「核验监管凭证身份、适用性、有效期和截至当前的可用依据」的判断半边
// （票 mechanism-executor-triage/07 CC-a 的消费侧）。它是凭证登记册的
...
// 责任流程形成有效依据后重新评估（UC-CC-003 门禁表「4. 监管凭证」行）。把两者压成一格，租户上线
// 前每一次判断都会读成「凭证不适用」。

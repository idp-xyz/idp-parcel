...
var ErrInvalidOperationsQueryScope = errors.New("customs compliance: invalid operations query scope")
...
// OperationsQueryScope 是合规规则库运营查阅的授权作用域(ADR-0077 Decision 二,形状同
// ADR-0076 Decision 二):(作用域引用,租户)两维,两样必须非零。没有货主客户账户维——
// 不是「客户维可选」,是这个维不存在:规则登记册是租户内部的登记对象,运营查阅的授权
// 边界只有租户(租户是最高数据隔离边界,ADR-0003)。
//
// 它是本上下文语言的一部分,不从 visibilityexception 导入共享作用域类型:共享类型让
// 边界在类型上互相依赖——「每上下文自立,各自成形互不参数化」(ADR-0077 Decision 二)。

...
// Covers: CC CONTEXT「监管接收、业务受理、监管过程决定、监管核定税费、放行结果和监管处置决定必须分层保存……不能使用一个“清关成功”状态覆盖各层事实」
...
// Covers: CC CONTEXT「与同层现有事实冲突时，不得据此猜测提交、补造缺失层次或按最后到达直接改变当前判断」——同层同版本同范围异语义即冲突（独立哨兵，双方保留

...
// 在构造时由装配点给定，进请求路径后不读任何授权输入。它只实现规则库查阅的
// CatalogueQueryIntake——外部结果接收的 ResultIntake 是命令面，刻意不实现，放行装不进
// 命令端点由编译期决定。
//
...
func (intake IsolatedOperationsReadIntake) IntakeCatalogueQuery(context.Context, *http.Request) (RuleCatalogueQuery, error) {
	return RuleCatalogueQuery{Scope: intake.scope, Limit: intake.limit}, nil
}

...
func authenticator(t *testing.T, verifier identity.OperatorCredentialVerifier, register operatorRegister) *ccaccess.OperatorRegistryAuthenticator {
	t.Helper()
	...
	built, err := ccaccess.NewOperatorRegistryAuthenticator(minter)
	if err != nil {
		t.Fatal(err)
	}
	return built
...
func TestRegistryGradesTranslateIntoCustomsGrades(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"issuer parameters unset": {identity.UnconfiguredOperatorCredentialVerifier{}, writer, "t", customshttp.ErrAccessChannelNotConfigured},
		"token missing":           {operatorVerifier{}, writer, "", customshttp.ErrOperatorCredentialRejected},
		"only a read grant":       {operatorVerifier{}, operatorRegister{faces: []identity.CapabilityFace{identity.CapabilityMasterDataAndOperationsRead}}, "t", customshttp.ErrOperatorNotGranted},
		"issuer key set down":     {operatorVerifier{err: identity.ErrCredentialVerifierUnavailable}, writer, "t", customshttp.ErrIdentityDependencyUnavailable},
		"operator register down":  {operatorVerifier{}, operatorRegister{err: errors.New("down")}, "t", customshttp.ErrIdentityDependencyUnavailable},
	}
	grades := []error{customshttp.ErrAccessChannelNotConfigured, customshttp.ErrOperatorCredentialRejected, customshttp.ErrOperatorNotGranted, customshttp.ErrIdentityDependencyUnavailable}

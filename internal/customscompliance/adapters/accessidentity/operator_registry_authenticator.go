// Package accessidentity 是关务合规对共享接入身份能力（internal/accessidentity）的消费侧适配器：铸造操作者信封，
...
// OperatorRegistryAuthenticator 实现 customshttp.OperatorRegistryAuthenticator：以登记册配置写能力面铸信封。
// 不带准入要求——登记一个版本不形成生产事实，ADR-0055 决定五两项对登记写面不适用（ADR-0100 决定四）。
type OperatorRegistryAuthenticator struct {
	minter Minter
}

var _ customshttp.OperatorRegistryAuthenticator = (*OperatorRegistryAuthenticator)(nil)

func NewOperatorRegistryAuthenticator(minter Minter) (*OperatorRegistryAuthenticator, error) {
	if minter == nil {
		return nil, errors.New("customs accessidentity: operator registry authenticator needs a minter")
	}
...
// answer 把共享接入身份能力的格译成 customshttp 的格：处理器只认本上下文的哨兵。
func answer(err error) error {
	...
	case errors.Is(err, identity.ErrCredentialRejected):
		return fmt.Errorf("%w: %w", customshttp.ErrOperatorCredentialRejected, err)
	case errors.Is(err, identity.ErrOperatorNotGranted):
		return fmt.Errorf("%w: %w", customshttp.ErrOperatorNotGranted, err)

...
//
// **这本册子不是接入认证。** CONTEXT 要求「提交授权请求、批准、拒绝、撤销、申报渠道账号使用和实际提交权限必须分别表达」，并明写「能够登录系统或持有账号凭据均不能自动取得其他权限」
// ——接入渠道认证（`PAR-INT-01`）回答的是「这个请求来自哪个租户」，
// 本口回答的是「这个申报单元当前有没有有效的提交授权依据」。把后者接到前者上就等于
// 让登录直接换来报关权，正是该硬句要禁的合并。签名本身也证得了这一点：租户是入参，
// 而接入认证的产物才是租户。
//
// 与 ReadinessView 同形三态、分表存放：两条轨分别形成和失效（CONTEXT「提交授权与就绪判断分别形成和失效」），就绪在场
// 顶替不了授权。

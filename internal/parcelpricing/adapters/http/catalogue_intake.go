// Package pricinghttp 承载 parcel-pricing 主数据目录查阅的 HTTP 适配器(ADR-0077):
// 价卡目录与计价参考序列两个查询端点。查阅不做评价、不择优、不解析期次——本包只
// 消费存储读面,不接应用编排,与 /shipment-request-views 同一条分界。
package pricinghttp
...
var ErrMalformedRequest = errors.New("parcel pricing http: malformed request")
...
// PricingCatalogueIntake 把一次已认证的目录查阅请求翻译成查询。
//
...
type PricingCatalogueIntake interface {

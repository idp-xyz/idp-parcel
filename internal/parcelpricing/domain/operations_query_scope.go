...
var ErrInvalidOperationsQueryScope = errors.New("parcel pricing: invalid operations query scope")

// OperationsScopeReference 指名共享身份能力交来的那份运营授权作用域判断,供审计与
// 续办回溯;它不是权限内容本身,权限内容由认证与授权结果拥有。
type OperationsScopeReference struct{ identifier }

func NewOperationsScopeReference(value string) (OperationsScopeReference, error) {
	identifier, err := newIdentifier("operations scope reference", value)
	return OperationsScopeReference{identifier: identifier}, err
}

// OperationsQueryScope 是主数据登记目录查阅的授权作用域(ADR-0077 Decision 二,形状
// 同 ADR-0076):(作用域引用,租户)两维,两样必须非零。没有货主客户账户维——不是
// 「客户维可选」,是这个维不存在:登记册是租户内部对象,运营查阅的授权边界只有租户
// (租户是最高数据隔离边界,ADR-0003)。按方向或种类过滤可以是查询条件,但那是过滤器
// 不是授权边界。
//
// 它是本上下文语言的一部分,不从 visibilityexception 导入共享类型:共享作用域会让
// 两个上下文的边界在类型上互相依赖(ADR-0077 Decision 二,各自成形互不参数化)。

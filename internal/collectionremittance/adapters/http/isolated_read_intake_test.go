...
func TestIsolatedOperationsReadIntakeGrantsInjectedScope(t *testing.T) {
	intake, err := collectionhttp.NewIsolatedOperationsReadIntake("SYN-SCOPE-ISOLATED-READ", "SYN-TENANT-01", 25)
	if err != nil {
		t.Fatalf("构造:%v", err)
	}

	request := httptest.NewRequest(http.MethodGet, "/collection-subledgers?tenant=TENANT-9", nil)
...
func TestNewIsolatedOperationsReadIntakeRejectsBrokenInjection(t *testing.T) {
	if _, err := collectionhttp.NewIsolatedOperationsReadIntake("", "SYN-TENANT-01", 25); err == nil {
		t.Fatal("空作用域引用被接受")
	}
	if _, err := collectionhttp.NewIsolatedOperationsReadIntake("SYN-SCOPE-ISOLATED-READ", "", 25); err == nil {
		t.Fatal("空租户被接受")
	}
	if _, err := collectionhttp.NewIsolatedOperationsReadIntake("SYN-SCOPE-ISOLATED-READ", "SYN-TENANT-01", 0); err == nil {
		t.Fatal("非正页大小被接受")
	}
}

...
func NewIsolatedOperationsReadIntake(
	...
	if err != nil {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("collection remittance http: isolated read intake: %w", err)
	}
	...
	if limit <= 0 {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("collection remittance http: isolated read intake: limit must be positive, got %d", limit)
	}

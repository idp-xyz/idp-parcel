...
var ErrInvalidOperationsQueryScope = errors.New("collection remittance: invalid operations query scope")
...
// OperationsQueryScope 是代收分户账运营查阅的授权作用域(ADR-0077 Decision 二,形状
// 同 ADR-0076 Decision 二):(作用域引用,租户)两维,两样必须非零。没有货主客户账户
// 维——不是「客户维可选」,是这个维不存在:分户账是租户内部的受托保管登记对象,货主
// 客户是账上的登记内容不是查阅方身份,运营查阅的授权边界只有租户(租户是最高数据
// 隔离边界,ADR-0003)。
//

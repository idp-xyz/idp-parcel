...
func RehydratePublicationContent(canonicalization string, document []byte) (PublicationContent, error) {
	...
	if decoded.AuthorizationRule != nil {
		body, err := decoded.AuthorizationRule.body()
		if err != nil {
			return none, fmt.Errorf("rehydrate publication content: authorization rule: %w", err)
		}
	...
	if decoded.PricePolicy != nil {
		body, err := decoded.PricePolicy.body()
		if err != nil {
			return none, fmt.Errorf("rehydrate publication content: price policy: %w", err)
		}
	...
	if decoded.PreAcceptanceFinancialControlPolicy != nil {
		body, err := decoded.PreAcceptanceFinancialControlPolicy.body()
		if err != nil {
			return none, fmt.Errorf("rehydrate publication content: pre-acceptance financial control policy: %w", err)
		}

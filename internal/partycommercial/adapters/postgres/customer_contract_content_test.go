...
func TestContentRulePackageMustAgreeWithTheVersionShell(t *testing.T) {
	...
	t.Run("壳上没指名", func(t *testing.T) {
		unnamed := effectiveContractWithoutNamedRulePackage(t, "contract-2", "v1", "digest-2")
...
// effectiveContractWithoutNamedRulePackage 造一份壳上没有指名规则包的已生效合同。
// 正文件列仍须自带引用；F-3 只在两处都在场时比对。
func effectiveContractWithoutNamedRulePackage(t *testing.T, objectID, label, digest string) domain.CommercialVersion {
	t.Helper()

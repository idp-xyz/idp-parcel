...
//
// **已知的收窄**：登记册的完整形状还含区间更正（ADR-0038）与价格/结算政策
// （ADR-0034/0044），而这三册今天都还没有持久化面——本仓也没有任何生产代码调用
// RegisterValidityCorrection / RegisterPricePolicy / RegisterSettlementPolicy。因此
// 本口现在交回的就是全部已落库的权威事实，不是「省略了几册」。这些册子拿到持久化面

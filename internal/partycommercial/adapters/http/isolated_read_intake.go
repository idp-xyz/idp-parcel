...
// CommercialCatalogueIntake（服务产品与商业策略两个端点共用，与该接口注释「分设只会
// 让装配点看起来能只配一半」同一条理由）。
//
...
var _ CommercialCatalogueIntake = IsolatedOperationsReadIntake{}
...
func NewIsolatedOperationsReadIntake(
	...
	if err != nil {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("party commercial http: isolated read intake: %w", err)
	}
	...
	if limit <= 0 {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("party commercial http: isolated read intake: limit must be positive, got %d", limit)
	}
	return IsolatedOperationsReadIntake{scope: scope, limit: limit}, nil
}

// IntakeCatalogueQuery 不读请求（参数匿名）：作用域整组来自注入，答复与请求内容及
// 自报身份无关。kind 分派参数属传输形状，由端点自己读，与本 Intake 无涉。

// Package veinbox 是 visibility-exception 的入站事件消费适配器（ADR-0025：适配器
// 在消费方侧）。事务舞步交给 platform/inboxconsume；本包只声明消费者名、事件类型与
// 译码。生产路由登记留给 fan-out 票（ADR-0049：接不住不登记）。
package veinbox
...
// nodeIntakeConsumerName 是本消费者在 inbox 键上的稳定名。必须与
// parcel-shipment/adopt-node-intake 分账：Inbox 键只由（消费者名 + 来源 + 事件 ID）
// 认领，共名会让一路把另一路的投递当重复跳过。改名等于换消费者。
const nodeIntakeConsumerName = "visibility-exception/derive-projection-from-node-intake"
...
// 结构；业务缺件（收寄还看不见、实物还没识别）走处理方哨兵，由消费门回滚重投。
var ErrPoisonEnvelope = errors.New("visibility exception inbox: poison envelope")
...
// FormedNodeIntakeHandler 是本消费者转交的处理方。真实装配接 DeriveOnNodeIntakeAdapter。

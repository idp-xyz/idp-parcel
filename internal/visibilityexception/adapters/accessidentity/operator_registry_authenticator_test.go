...
func authenticator(t *testing.T, verifier identity.OperatorCredentialVerifier, register operatorRegister) *veaccess.OperatorRegistryAuthenticator {
	t.Helper()
	...
	built, err := veaccess.NewOperatorRegistryAuthenticator(minter)
	if err != nil {
		t.Fatal(err)
	}
	return built
...
func TestRegistryGradesTranslateIntoVisibilityGrades(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"issuer parameters unset": {identity.UnconfiguredOperatorCredentialVerifier{}, writer, "t", visibilityhttp.ErrAccessChannelNotConfigured},
		"token missing":           {operatorVerifier{}, writer, "", visibilityhttp.ErrOperatorCredentialRejected},
		"only a read grant":       {operatorVerifier{}, operatorRegister{faces: []identity.CapabilityFace{identity.CapabilityMasterDataAndOperationsRead}}, "t", visibilityhttp.ErrOperatorNotGranted},
		"issuer key set down":     {operatorVerifier{err: identity.ErrCredentialVerifierUnavailable}, writer, "t", visibilityhttp.ErrIdentityDependencyUnavailable},
		"operator register down":  {operatorVerifier{}, operatorRegister{err: errors.New("down")}, "t", visibilityhttp.ErrIdentityDependencyUnavailable},
	}
	grades := []error{visibilityhttp.ErrAccessChannelNotConfigured, visibilityhttp.ErrOperatorCredentialRejected, visibilityhttp.ErrOperatorNotGranted, visibilityhttp.ErrIdentityDependencyUnavailable}

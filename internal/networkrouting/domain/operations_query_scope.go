...
var ErrInvalidOperationsQueryScope = errors.New("network routing: invalid operations query scope")
...
// OperationsQueryScope 是网络目录运营查阅的授权作用域(ADR-0077 Decision 二,形状同
// ADR-0076 Decision 二):(作用域引用,租户)两维,两样必须非零。没有货主客户账户维——
// 不是「客户维可选」,是这个维不存在:网络目录是租户内部的登记对象,运营查阅的授权

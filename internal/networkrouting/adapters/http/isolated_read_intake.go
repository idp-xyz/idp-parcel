...
// 在构造时由装配点给定，进请求路径后不读任何授权输入。它只实现网络目录查阅的
// CatalogueQueryIntake——七族登记命令 Intake 一个不带，放行装不进登记端点由编译期
// 决定（ADR-0085 Decision 二明文维持 ADR-0078 的这条排除）。
//
...
func NewIsolatedOperationsReadIntake(
	...
	if err != nil {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("network routing http: isolated read intake: %w", err)
	}
	...
	if limit <= 0 {
		return IsolatedOperationsReadIntake{}, fmt.Errorf("network routing http: isolated read intake: limit must be positive, got %d", limit)
	}
	return IsolatedOperationsReadIntake{scope: scope, limit: limit}, nil
}

// IntakeCatalogueQuery 不读请求（参数匿名）：作用域整组来自注入，答复与请求内容及
// 自报身份无关。family 分派参数属传输形状，由端点自己读，与本 Intake 无涉。
func (intake IsolatedOperationsReadIntake) IntakeCatalogueQuery(context.Context, *http.Request) (NetworkCatalogQuery, error) {
	return NetworkCatalogQuery{Scope: intake.scope, Limit: intake.limit}, nil
}
